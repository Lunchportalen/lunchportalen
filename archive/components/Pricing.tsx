...
export default async function Pricing() {
  const [basisLabels, luxusLabels, ppB, ppL] = await Promise.all([
    mealDisplayLabelsForPlan("basis"),
    mealDisplayLabelsForPlan("luxus"),
    getProductPlan("basis"),
    getProductPlan("luxus"),
  ]);
  const basisBullets = [...basisLabels, "Forutsigbar drift"];
  const luxusBullets = [...luxusLabels];
  const priceB = ppB?.price && Number.isFinite(ppB.price) && ppB.price > 0 ? ppB.price : 90;
  const priceL = ppL?.price && Number.isFinite(ppL.price) && ppL.price > 0 ? ppL.price : 130;
  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <Plan
          title="Basis"
          price={`${priceB} kr pr. kuvert`}
          bullets={basisBullets}
        />
        <Plan
          title="Luxus"
          price={`${priceL} kr pr. kuvert`}
          bullets={luxusBullets}
          badge="Mest valgt"
        />
      </div>
