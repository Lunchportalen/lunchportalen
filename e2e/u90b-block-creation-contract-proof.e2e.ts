/**
 * U90B — Sluttbevis: kanonisk block type definition styrer NYOPPRETTELSE (library → canvas → defaults → validation → dirty → lagre → reload).
 *
 * Kjør mot kjørende local_provider (én dev-instans), f.eks. port 3042:
 *   cross-env CI=1 LP_CMS_RUNTIME_MODE=local_provider PLAYWRIGHT_BASE_URL=http://localhost:3042 npx playwright test e2e/u90b-block-creation-contract-proof.e2e.ts --project=chromium
 */
...
const ARTIFACT_DIR = path.join(process.cwd(), "artifacts", "u90b-block-creation-contract-proof");
...
test("U90B block creation contract — new cta + cards, runtime proof + artifacts", async ({ page, baseURL }) => {
  ...
  const ctaMarker = `U90B-CTA-${Date.now()}`;
  await selectBlockByOrdinal(page, ordinalSimple);
  await openInnholdTab(page);
  ...
  await ctaEdit.locator('[data-lp-property-section="settings"] input').first().fill("U90B primær");

  const cardsMarker = `U90B-CARDS-${Date.now()}`;
  await selectBlockByOrdinal(page, ordinalStructure);
  await openInnholdTab(page);
  ...
  await cardsEdit.locator('[data-lp-property-section="structure"] input').nth(1).fill("U90B kort-tittel");
  ...
  await expect(ctaPersist.locator('[data-lp-property-section="settings"] input').first()).toHaveValue("U90B primær");
  ...
  await expect(cardsAfter.locator('[data-lp-property-section="structure"] input').nth(1)).toHaveValue("U90B kort-tittel");
  ...
  await writeFile(
    path.join(ARTIFACT_DIR, "u90b-block-creation-contract-proof.json"),
