...
| **V.W8** — icon consolidation | `e2e/week-icon-probe.e2e.ts` | Kalender: **locked / ordered / unavailable** `.ds-week-icon` — alle tre **samme resolved px** (bredde/høyde); `currentColor`; `aria-hidden="true"`; markør `font-size` **12px**; `getBoundingClientRect().height` **> 0**. **5.4-vakt:** valgt slot `::after` uendret (`content: "✓"`, 22×22px, top/right). **Slot-klokke:** `week-category-card__state-icon` på locked slot (V.W6, ikke `ds-week-icon`); `aria-hidden`; height **> 0**; label «Frist passert». **Determinisme:** slot-klokke-sub-probe etablerer egen locked-dag-precondition via `expect.poll` (Mon-tap til CUTOFF-slot mount) — full-suite 3× er gaten, ikke isolert steg | `playwright.week-row-probe.config.ts` |
