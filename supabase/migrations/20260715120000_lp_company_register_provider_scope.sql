-- P0 registration correctness: provider-scope canonical lp_company_register.
--
-- Bevist tilstand før denne migrasjonen (prod == repo-baseline 20260528000000):
--   - companies.provider_id og agreements.provider_id er NOT NULL uten default/trigger.
--   - lp_company_register satte ALDRI provider_id => INSERT INTO companies feilet
--     garantert med 23502 (not_null_violation) => offentlig registrering var død (500).
--
-- Regel (fail-closed):
--   - provider_id resolves KUN server-side fra postnummer via
--     public.lp_match_provider_by_postal_code (deterministisk ved flere treff:
--     ORDER BY provider_id ASC LIMIT 1 — dokumentert i match-RPC-en).
--   - Ingen match => RAISE 'PROVIDER_NOT_FOUND' (ingen rader skrives).
--   - Ingen Melhus-fallback. Ingen «første provider»-fallback utenom dokumentert sortering.
--   - provider_id settes på companies, agreements OG company_registrations.
--
-- Kontrakt: samme signatur og samme retur-shape (+ additivt provider_id-felt).
-- CREATE OR REPLACE beholder eksisterende grants (anon/authenticated/service_role).
...
begin
  ...
  -- Provider-scope (fail-closed): resolves server-side fra postnummer.
  -- Klienten kan aldri styre provider_id. Ingen match => ingen rader skrives.
