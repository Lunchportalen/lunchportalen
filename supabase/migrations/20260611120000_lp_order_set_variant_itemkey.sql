-- lp_order_set: variant item_key (CMS slug) no longer filtered by products.sku
-- Base: 20260610130000_lp_order_set_varmmat_msdi_alias.sql (prod hkpoky, includes varmmat→varmrett MSDI alias).
-- Delta: remove MSDI WHERE (v_item_raw = 'default' OR pr.sku = v_item_raw) in BOTH selects.
--   MSDI resolves on category slug (v_slug_msdi) + tier price only (one product per category in sync).
--   p_item_key / v_item_raw still stored in day_choices.item_key (null when default) for kitchen/CMS variant display.
