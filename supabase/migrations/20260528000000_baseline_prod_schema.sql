...
--
-- Name: provider_role; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: lp_assert_provider_admin_access(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_provider_admin_access(p_provider_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
...
--
-- Name: lp_assert_provider_admin_or_superadmin(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_provider_admin_or_superadmin(p_provider_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
...
begin
  ...
  if exists (
    ...
    where pm.user_id = auth.uid() and pm.provider_id = p_provider_id
...
--
-- Name: lp_assert_provider_kitchen_access(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_provider_kitchen_access(p_provider_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
--
-- Name: lp_assert_provider_member_read(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_provider_member_read(p_provider_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
begin
...
--
-- Name: COLUMN company_registrations.company_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.company_registrations.company_id IS 'NULL until provider/superadmin approval (Patch 13 intake). Legacy rows retain company_id.';


--
-- Name: lp_assert_registration_approve_access(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_registration_approve_access(p_registration_id uuid) RETURNS public.company_registrations
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_reg public.company_registrations%rowtype; BEGIN SELECT * INTO v_reg FROM public.company_registrations WHERE id = p_registration_id FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'REGISTRATION_NOT_FOUND' USING ERRCODE = 'P0002'; END IF; IF public.is_platform_admin() THEN RETURN v_reg; END IF; IF v_reg.provider_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.provider_memberships pm WHERE pm.user_id = auth.uid() AND pm.provider_id = v_reg.provider_id AND pm.role = 'provider_admin'::public.provider_role) THEN RETURN v_reg; END IF; IF v_reg.provider_id IS NULL AND public.is_platform_admin() THEN RETURN v_reg; END IF; RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END; $$;


--
-- Name: lp_assert_user_lifecycle_access(uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_assert_user_lifecycle_access(p_user_id uuid, OUT o_company_id uuid, OUT o_provider_id uuid) RETURNS record
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: lp_enqueue_agreement_invoice_outbox(uuid, uuid, uuid, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_enqueue_agreement_invoice_outbox(p_invoice_id uuid, p_provider_id uuid, p_agreement_id uuid, p_request_rid text DEFAULT NULL::text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_enqueue_saas_invoice_outbox(uuid, uuid, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_enqueue_saas_invoice_outbox(p_invoice_id uuid, p_provider_id uuid, p_request_rid text DEFAULT NULL::text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_generate_agreement_invoice_core(uuid, date, date, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_generate_agreement_invoice_core(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
begin
  ...
  select * into v_agreement from public.agreements a where a.id = p_agreement_id;
  if not found then raise exception 'AGREEMENT_NOT_FOUND' using errcode = 'P0002'; end if;

  select * into v_company from public.companies c where c.id = v_agreement.company_id;
  ...
  if upper(v_agreement.status::text) <> 'ACTIVE' then
    return jsonb_build_object('ok', true, 'skipped', true, 'reason', 'AGREEMENT_NOT_ACTIVE', 'agreement_id', p_agreement_id, 'status', v_agreement.status::text);
  end if;

  if v_company.suspended_at is not null or v_company.paused_at is not null then
    return jsonb_build_object('ok', true, 'skipped', true, 'reason', 'COMPANY_SUSPENDED_OR_PAUSED', 'agreement_id', p_agreement_id, 'company_id', v_company.id);
  end if;
  ...
  if exists (
    ...
    return jsonb_build_object('ok', true, 'skipped', true, 'reason', 'PROVIDER_SUSPENDED_OR_PAUSED', 'agreement_id', p_agreement_id, 'provider_id', v_agreement.provider_id);
  end if;

  select ai.id, ai.status into v_existing, v_existing_status
  from public.agreement_invoices ai
  where ai.agreement_id = p_agreement_id and ai.invoice_period_start = p_period_start;

  if v_existing is not null then
    v_idempotent := true;
    v_invoice_id := v_existing;
    if v_existing_status = 'DRAFT' then
      v_event_key := private.lp_enqueue_agreement_invoice_outbox(v_invoice_id, v_agreement.provider_id, v_agreement.id, p_request_rid);
    end if;
    return jsonb_build_object('ok', true, 'invoice_id', v_invoice_id, 'idempotent', v_idempotent, 'event_key', v_event_key);
  end if;
  ...
  if v_order_count = 0 then
    return jsonb_build_object('ok', true, 'skipped', true, 'reason', 'ZERO_ORDERS', 'agreement_id', p_agreement_id, 'period_start', p_period_start, 'period_end', p_period_end);
  end if;

  select p.slug into v_slug from public.providers p where p.id = v_agreement.provider_id;

  insert into public.agreement_invoices (
    agreement_id, provider_id, company_id, invoice_period_start, invoice_period_end,
    billing_cycle, amount_net, amount_tax, amount_total, tripletex_vat_code, status, due_date, metadata
  ) values (
    v_agreement.id, v_agreement.provider_id, v_agreement.company_id, p_period_start, p_period_end,
    coalesce(nullif(btrim(v_agreement.billing_cycle), ''), 'monthly'), 0, 0, 0, v_primary_tax_code, 'DRAFT',
    (p_period_end + interval '14 days')::date,
    jsonb_build_object('generated_by', 'lp_provider_generate_agreement_invoice_for_period', 'order_count', v_order_count)
  ) returning id into v_invoice_id;

  v_invoice_number := format('AGR-%s-%s-%s', upper(coalesce(nullif(btrim(v_slug), ''), 'PROV')), to_char(p_period_start, 'YYYYMMDD'), left(replace(v_invoice_id::text, '-', ''), 8));
  update public.agreement_invoices set invoice_number = v_invoice_number where id = v_invoice_id;

  for v_line in
    select upper(o.tier::text) as tier, o.unit_price_nok::numeric(12, 2) as unit_price,
           count(*)::int as quantity, sum(o.unit_price_nok)::numeric(12, 2) as line_net
    from public.orders o
    where o.agreement_id = p_agreement_id and o.date >= p_period_start and o.date <= p_period_end
      and upper(o.status::text) not in ('CANCELLED')
    group by upper(o.tier::text), o.unit_price_nok
    order by upper(o.tier::text), o.unit_price_nok
  loop
    v_tier := v_line.tier;
    v_product_key := case when v_tier in ('BASIS', 'LUXUS', 'ENTERPRISE') then v_tier else 'CUSTOM' end;

    select bp.tax_code_id, btc.rate into v_tax_code_id, v_vat_rate
    from public.billing_products bp
    join public.billing_tax_codes btc on btc.id = bp.tax_code_id
    where bp.tier = case when v_product_key in ('BASIS', 'LUXUS', 'ENTERPRISE') then v_product_key else 'BASIS' end;

    if v_tax_code_id is null then
      select btc.id, btc.rate into v_tax_code_id, v_vat_rate from public.billing_tax_codes btc where btc.id = 'MVA_15';
    end if;
    ...
    insert into public.agreement_invoice_lines (
      invoice_id, product_key, description, quantity, unit_price, line_amount, vat_rate, vat_amount, tax_code_id
    ) values (
      v_invoice_id, v_product_key, v_desc, v_line.quantity, v_line.unit_price, v_line_net,
      coalesce(v_vat_rate, 0.15), v_line_tax, coalesce(v_tax_code_id, 'MVA_15')
    );
  ...
  update public.agreement_invoices
     set amount_net = v_net, amount_tax = v_tax, amount_total = v_total,
         tripletex_vat_code = v_primary_tax_code, updated_at = now()
   where id = v_invoice_id;

  update public.agreements set last_invoiced_at = now(), updated_at = now() where id = v_agreement.id;

  insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata)
  values (
    auth.uid(), 'agreement_invoice_generated', 'agreement_invoice', v_invoice_id,
    format('Invoice %s for agreement %s', v_invoice_number, v_agreement.id),
    jsonb_build_object('agreement_id', v_agreement.id, 'provider_id', v_agreement.provider_id,
      'company_id', v_agreement.company_id, 'period_start', p_period_start, 'period_end', p_period_end,
      'amount_total', v_total, 'line_count', v_order_count)
  );

  v_event_key := private.lp_enqueue_agreement_invoice_outbox(v_invoice_id, v_agreement.provider_id, v_agreement.id, p_request_rid);

  return jsonb_build_object('ok', true, 'invoice_id', v_invoice_id, 'invoice_number', v_invoice_number,
    'amount_net', v_net, 'amount_tax', v_tax, 'amount_total', v_total, 'order_count', v_order_count,
    'idempotent', false, 'event_key', v_event_key);
end;
$$;


--
-- Name: lp_is_elevated_caller(); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_is_elevated_caller() RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
  select
...
--
-- Name: lp_lifecycle_audit(text, text, uuid, text, jsonb); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_lifecycle_audit(p_action text, p_entity_type text, p_entity_id uuid, p_reason text, p_metadata jsonb DEFAULT '{}'::jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
--
-- Name: lp_lifecycle_require_reason(text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_lifecycle_require_reason(p_reason text) RETURNS void
...
--
-- Name: lp_orders_cancel_active(uuid, uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_orders_cancel_active(p_company_id uuid DEFAULT NULL::uuid, p_provider_id uuid DEFAULT NULL::uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: lp_orders_pause_active(uuid, uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_orders_pause_active(p_company_id uuid DEFAULT NULL::uuid, p_provider_id uuid DEFAULT NULL::uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: lp_orders_resume_paused(uuid, uuid); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_orders_resume_paused(p_company_id uuid DEFAULT NULL::uuid, p_provider_id uuid DEFAULT NULL::uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: lp_provider_tripletex_credentials_vault_cleanup(); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_provider_tripletex_credentials_vault_cleanup() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'vault', 'pg_catalog'
    AS $$
begin
  delete from vault.secrets where id in (old.consumer_token_secret_id, old.employee_token_secret_id);
  return old;
end;
$$;
...
--
-- Name: lp_provider_webhook_secret_vault_cleanup(); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_provider_webhook_secret_vault_cleanup() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'vault', 'pg_catalog'
    AS $$ begin delete from vault.secrets where id = old.webhook_secret_id; return old; end; $$;


--
-- Name: lp_tripletex_allowed_transition(text, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_allowed_transition(p_from text, p_to text) RETURNS boolean
...
--
-- Name: lp_tripletex_onboarding_audit(text, uuid, text, uuid, jsonb); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_onboarding_audit(p_action text, p_provider_id uuid, p_env text, p_actor uuid, p_metadata jsonb DEFAULT '{}'::jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
begin
...
--
-- Name: provider_tripletex_credentials; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.provider_tripletex_credentials (
    ...
    vault_purge_at timestamp with time zone,
    health_check_at timestamp with time zone,
    onboarding_provisioning_complete_at timestamp with time zone,
    cached_tripletex_company_name text,
    CONSTRAINT provider_tripletex_credentials_connection_state_check CHECK ((connection_state = ANY (ARRAY['NOT_CONNECTED'::text, 'CONFIGURING'::text, 'CONNECTED'::text, 'DEGRADED'::text, 'DISCONNECTED'::text]))),
    CONSTRAINT provider_tripletex_credentials_env_check CHECK ((env = ANY (ARRAY['test'::text, 'prod'::text]))),
    CONSTRAINT provider_tripletex_credentials_sync_status_check CHECK ((sync_status = ANY (ARRAY['PENDING'::text, 'READY'::text, 'DISABLED'::text, 'FAILED'::text])))
);


--
-- Name: lp_tripletex_transition_connection_state(uuid, text, text, uuid, jsonb); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_transition_connection_state(p_provider_id uuid, p_env text, p_new_state text, p_actor uuid, p_metadata jsonb DEFAULT '{}'::jsonb) RETURNS public.provider_tripletex_credentials
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_tripletex_validate_verification_result(jsonb); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_validate_verification_result(p_result jsonb) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
declare
  v_all boolean;
...
--
-- Name: lp_tripletex_vault_secret_name(uuid, text, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_vault_secret_name(p_provider_id uuid, p_env text, p_kind text) RETURNS text
    LANGUAGE sql IMMUTABLE
    AS $$
...
--
-- Name: lp_tripletex_webhook_vault_name(uuid, text); Type: FUNCTION; Schema: private; Owner: -
--

CREATE FUNCTION private.lp_tripletex_webhook_vault_name(p_provider_id uuid, p_env text) RETURNS text
    LANGUAGE sql IMMUTABLE
    AS $$ select 'tpt_provider_' || p_provider_id::text || '_' || lower(btrim(p_env)) || '_webhook'; $$;
...
--
-- Name: can_access_provider(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.can_access_provider(p_provider_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT EXISTS (
...
--
-- Name: FUNCTION can_access_provider(p_provider_id uuid); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.can_access_provider(p_provider_id uuid) IS 'True when auth.uid() has provider_membership for p_provider_id, or is platform admin. Mirror of can_access_company().';
...
--
-- Name: lp_agreement_lifecycle_hook(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_agreement_lifecycle_hook() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
begin
  if new.status = 'ACTIVE'::public.agreement_status
     and (tg_op = 'INSERT' or old.status is distinct from 'ACTIVE'::public.agreement_status)
     and new.company_id is not null
     and new.provider_id is not null
  then
    v_request_rid := replace(gen_random_uuid()::text, '-', '');
    v_customer_key := format('tripletex.company_customer_create_provider:%s:%s', new.company_id::text, new.provider_id::text);
    insert into public.outbox (event_key, payload, status, attempts, last_error, locked_at, locked_by)
    values (v_customer_key, jsonb_build_object('company_id', new.company_id, 'provider_id', new.provider_id, 'env', v_env, 'request_rid', v_request_rid, 'source', 'agreement_lifecycle', 'agreement_id', new.id, 'trigger', 'status_active'), 'PENDING', 0, null, null, null)
    on conflict (event_key) do nothing;
    insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata)
    values (null, 'agreement_lifecycle_hook_fired', 'agreement', new.id, 'Agreement ACTIVE → enqueue customer sync', jsonb_build_object('hook', 'status_active', 'agreement_id', new.id, 'company_id', new.company_id, 'provider_id', new.provider_id, 'event_key', v_customer_key, 'request_rid', v_request_rid));
  end if;
  if tg_op = 'UPDATE' and new.tier is distinct from old.tier and new.provider_id is not null then
    v_tier := upper(btrim(new.tier::text));
    if v_tier in ('BASIS', 'LUXUS', 'ENTERPRISE') then
      v_request_rid := replace(gen_random_uuid()::text, '-', '');
      v_product_key := format('tripletex.provider_product_sync:%s:%s', new.provider_id::text, v_tier);
      insert into public.outbox (event_key, payload, status, attempts, last_error, locked_at, locked_by)
      values (v_product_key, jsonb_build_object('provider_id', new.provider_id, 'tier', v_tier, 'env', v_env, 'request_rid', v_request_rid, 'source', 'agreement_lifecycle', 'agreement_id', new.id, 'trigger', 'tier_change', 'previous_tier', upper(btrim(old.tier::text))), 'PENDING', 0, null, null, null)
      on conflict (event_key) do nothing;
      insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata)
      values (null, 'agreement_lifecycle_hook_fired', 'agreement', new.id, format('Agreement tier %s → %s → enqueue product sync', old.tier, new.tier), jsonb_build_object('hook', 'tier_change', 'agreement_id', new.id, 'provider_id', new.provider_id, 'tier', v_tier, 'previous_tier', old.tier, 'event_key', v_product_key, 'request_rid', v_request_rid));
    end if;
  end if;
  return new;
end;
...
--
-- Name: lp_apply_tripletex_paid_status(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_apply_tripletex_paid_status(p_provider_id uuid, p_tripletex_invoice_id text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare v_tt_id text := btrim(coalesce(p_tripletex_invoice_id, '')); v_row public.agreement_invoices%rowtype; v_prev text;
begin
  if auth.role() <> 'service_role' then raise exception 'PERMISSION_DENIED' using errcode = '42501'; end if;
  if v_tt_id = '' then raise exception 'TRIPLETEX_INVOICE_ID_REQUIRED' using errcode = 'P0001'; end if;
  select * into v_row from public.agreement_invoices ai where ai.provider_id = p_provider_id and ai.tripletex_invoice_id = v_tt_id limit 1;
  if not found then return jsonb_build_object('ok', true, 'updated', false, 'reason', 'NOT_FOUND', 'provider_id', p_provider_id, 'tripletex_invoice_id', v_tt_id); end if;
  v_prev := v_row.status;
  if v_prev = 'PAID' then return jsonb_build_object('ok', true, 'updated', false, 'previous_status', v_prev, 'invoice_id', v_row.id, 'reason', 'ALREADY_PAID'); end if;
  if v_prev <> 'SENT' then
    insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) values (null, 'agreement_invoice_paid_transition_rejected', 'agreement_invoice', v_row.id, format('Invalid transition %s → PAID', v_prev), jsonb_build_object('provider_id', p_provider_id, 'tripletex_invoice_id', v_tt_id, 'previous_status', v_prev, 'target_status', 'PAID'));
    return jsonb_build_object('ok', true, 'updated', false, 'previous_status', v_prev, 'invoice_id', v_row.id, 'reason', 'INVALID_TRANSITION');
  end if;
  insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) values (null, 'agreement_invoice_paid', 'agreement_invoice', v_row.id, format('Tripletex invoice %s marked PAID', v_tt_id), jsonb_build_object('provider_id', p_provider_id, 'tripletex_invoice_id', v_tt_id, 'previous_status', v_prev, 'agreement_id', v_row.agreement_id, 'company_id', v_row.company_id));
  update public.agreement_invoices set status = 'PAID', paid_at = now(), last_status_change = now(), updated_at = now() where id = v_row.id and status = 'SENT';
  return jsonb_build_object('ok', true, 'updated', true, 'previous_status', v_prev, 'invoice_id', v_row.id, 'tripletex_invoice_id', v_tt_id);
end; $$;
...
--
-- Name: lp_company_delete(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_delete(p_company_id uuid, p_reason text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_provider_id uuid; v_cascade_orders int := 0;
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'COMPANY_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  PERFORM private.lp_assert_provider_admin_access(v_provider_id);
  IF EXISTS (SELECT 1 FROM public.companies WHERE id = p_company_id AND deleted_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_deleted', true); END IF;
  ...
  PERFORM private.lp_lifecycle_audit('delete', 'company', p_company_id, p_reason, jsonb_build_object('provider_id', v_provider_id, 'cascade_orders_cancelled', v_cascade_orders, 'grace_days', 30));
...
--
-- Name: FUNCTION lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date) IS 'Firma-scope ordre-aggregering for company_admin / company_finance (membership) eller superadmin (profiles.role). Ingen produkt- eller valgdetaljer.';


--
-- Name: lp_company_pause(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_pause(p_company_id uuid, p_reason text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_provider_id uuid; v_cascade_orders int := 0;
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'COMPANY_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  PERFORM private.lp_assert_provider_admin_access(v_provider_id);
  IF EXISTS (SELECT 1 FROM public.companies WHERE id = p_company_id AND paused_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_paused', true); END IF;
  UPDATE public.companies SET paused_at = now(), paused_by = auth.uid(), paused_reason = p_reason WHERE id = p_company_id;
  v_cascade_orders := private.lp_orders_pause_active(p_company_id := p_company_id);
  PERFORM private.lp_lifecycle_audit('pause', 'company', p_company_id, p_reason, jsonb_build_object('provider_id', v_provider_id, 'cascade_orders_paused', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_orders_paused', v_cascade_orders);
END; $$;


--
-- Name: lp_company_provider_customer_create(uuid, uuid, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_provider_customer_create(p_company_id uuid, p_provider_id uuid, p_env text DEFAULT 'prod'::text, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_company_registration_approve_provider(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_registration_approve_provider(p_registration_id uuid, p_agreement_tier text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_reg public.company_registrations%rowtype; v_tier text := upper(btrim(coalesce(p_agreement_tier, ''))); v_actor uuid := auth.uid(); v_company_id uuid; v_location_id uuid; v_agreement_id uuid; v_invite_id uuid; v_token_hash text; v_now timestamptz := now(); v_weekday_tiers jsonb; v_delivery_days text[] := ARRAY['mon','tue','wed','thu','fri']; v_slot_start time := time '11:00'; v_slot_end time := time '13:00'; v_binding int := 12; v_notice int := 3; v_full_address text; v_agreement_json jsonb; v_existing_company uuid; BEGIN IF v_actor IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED' USING ERRCODE = '42501'; END IF; IF v_tier NOT IN ('BASIS', 'LUXUS') THEN RAISE EXCEPTION 'AGREEMENT_TIER_INVALID' USING ERRCODE = 'P0001'; END IF; v_reg := private.lp_assert_registration_approve_access(p_registration_id); IF upper(coalesce(v_reg.status, '')) <> 'PENDING' THEN RAISE EXCEPTION 'REGISTRATION_NOT_PENDING' USING ERRCODE = 'P0001'; END IF; v_weekday_tiers := jsonb_build_object('mon', v_tier, 'tue', v_tier, 'wed', v_tier, 'thu', v_tier, 'fri', v_tier); v_company_id := v_reg.company_id; IF v_company_id IS NULL THEN SELECT c.id INTO v_existing_company FROM public.companies c WHERE c.deleted_at IS NULL AND (btrim(coalesce(c.orgnr, '')) = btrim(coalesce(v_reg.orgnr, '')) OR btrim(coalesce(c.organization_number, '')) = btrim(coalesce(v_reg.orgnr, ''))) LIMIT 1 FOR UPDATE; IF v_existing_company IS NOT NULL THEN v_company_id := v_existing_company; ELSE INSERT INTO public.companies (name, orgnr, organization_number, status, provider_id) VALUES (coalesce(v_reg.company_name, 'Ny bedrift'), v_reg.orgnr, v_reg.orgnr, 'PENDING'::public.company_status, v_reg.provider_id) RETURNING id INTO v_company_id; END IF; v_full_address := trim(both ' ' from coalesce(v_reg.address_line, '') || ', ' || coalesce(v_reg.postal_code, '') || ' ' || coalesce(v_reg.city, '')); SELECT cl.id INTO v_location_id FROM public.company_locations cl WHERE cl.company_id = v_company_id AND cl.name = coalesce(v_reg.company_name, 'Hovedkontor') LIMIT 1; IF v_location_id IS NULL THEN INSERT INTO public.company_locations (company_id, name, address, status) VALUES (v_company_id, coalesce(v_reg.company_name, 'Hovedkontor'), nullif(v_full_address, ''), 'ACTIVE') RETURNING id INTO v_location_id; ELSE UPDATE public.company_locations SET address = nullif(v_full_address, ''), updated_at = v_now WHERE id = v_location_id; END IF; UPDATE public.companies SET default_location_id = coalesce(default_location_id, v_location_id), updated_at = v_now WHERE id = v_company_id; ELSE SELECT cl.id INTO v_location_id FROM public.company_locations cl WHERE cl.company_id = v_company_id ORDER BY cl.created_at ASC LIMIT 1 FOR UPDATE; IF v_location_id IS NULL THEN RAISE EXCEPTION 'COMPANY_LOCATION_REQUIRED' USING ERRCODE = 'P0001'; END IF; END IF; IF v_reg.provider_id IS NOT NULL THEN UPDATE public.companies SET provider_id = v_reg.provider_id, updated_at = v_now WHERE id = v_company_id; END IF; v_agreement_json := jsonb_build_object('version', 1, 'source', 'company_registration_approve_provider', 'registration_id', v_reg.id, 'company_id', v_company_id, 'created_at', v_now, 'plan', jsonb_build_object('days', jsonb_build_object('mon', jsonb_build_object('enabled', true, 'tier', v_tier), 'tue', jsonb_build_object('enabled', true, 'tier', v_tier), 'wed', jsonb_build_object('enabled', true, 'tier', v_tier), 'thu', jsonb_build_object('enabled', true, 'tier', v_tier), 'fri', jsonb_build_object('enabled', true, 'tier', v_tier)))); UPDATE public.companies SET status = 'ACTIVE'::public.company_status, agreement_json = v_agreement_json, updated_at = v_now WHERE id = v_company_id; INSERT INTO public.agreements (company_id, location_id, provider_id, tier, status, delivery_days, slot_start, slot_end, starts_at, start_date, binding_months, notice_months, submitted_by_email, submitted_by_name, reviewed_by, reviewed_at, approved_at, activated_at, comment_from_company) VALUES (v_company_id, v_location_id, v_reg.provider_id, v_tier::public.agreement_tier, 'ACTIVE'::public.agreement_status, to_jsonb(v_delivery_days), v_slot_start, v_slot_end, current_date, current_date, v_binding, v_notice, coalesce(v_reg.contact_email, v_reg.submitted_by_email), coalesce(v_reg.contact_name, v_reg.submitted_by_name), v_actor, v_now, v_now, v_now, 'Godkjent fra provider-registreringsko (Patch 13).') RETURNING id INTO v_agreement_id; UPDATE public.company_registrations SET status = 'APPROVED', company_id = v_company_id, plan_tier = v_tier, weekday_meal_tiers = v_weekday_tiers, reviewed_at = v_now, reviewed_by = v_actor, agreement_id = v_agreement_id, updated_at = v_now WHERE id = v_reg.id; IF to_regprocedure('public.lp_materialize_agreement_day_slots(uuid,uuid)') IS NOT NULL THEN PERFORM public.lp_materialize_agreement_day_slots(v_company_id, v_agreement_id); END IF; v_token_hash := encode(digest(gen_random_uuid()::text || ':' || clock_timestamp()::text || ':' || v_company_id::text, 'sha256'), 'hex'); INSERT INTO public.company_invites (company_id, email, contact_email, contact_name, role, token_hash, expires_at) VALUES (v_company_id, coalesce(v_reg.contact_email, v_reg.submitted_by_email), coalesce(v_reg.contact_email, v_reg.submitted_by_email), coalesce(v_reg.contact_name, v_reg.submitted_by_name), 'company_admin', v_token_hash, v_now + interval '7 days') RETURNING id INTO v_invite_id; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (v_actor, 'company_registration_approved', 'company_registration', v_reg.id, NULL, jsonb_build_object('company_id', v_company_id, 'agreement_id', v_agreement_id, 'tier', v_tier, 'provider_id', v_reg.provider_id)); INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (v_actor, 'registration_approval_email_stub', 'company_registration', v_reg.id, 'Invite email stub (Patch 13)', jsonb_build_object('invite_id', v_invite_id, 'contact_email', v_reg.contact_email)); RETURN jsonb_build_object('ok', true, 'company_id', v_company_id, 'agreement_id', v_agreement_id, 'invite_id', v_invite_id, 'status', 'APPROVED'); END; $$;


--
-- Name: lp_company_registration_create(jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_registration_create(p_payload jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $_$
DECLARE
  ...
  v_raw jsonb;
BEGIN
  IF v_company_name = '' THEN RAISE EXCEPTION 'COMPANY_NAME_REQUIRED' USING ERRCODE = 'P0001'; END IF;
  IF length(v_orgnr) <> 9 THEN RAISE EXCEPTION 'ORGNR_INVALID' USING ERRCODE = 'P0001'; END IF;
  IF v_contact_name = '' THEN RAISE EXCEPTION 'CONTACT_NAME_REQUIRED' USING ERRCODE = 'P0001'; END IF;
  IF v_contact_email = '' OR v_contact_email !~ '^[^@\s]+@[^@\s]+\.[^@\s]+$' THEN RAISE EXCEPTION 'CONTACT_EMAIL_INVALID' USING ERRCODE = 'P0001'; END IF;
  v_contact_phone := regexp_replace(btrim(coalesce(p_payload ->> 'contact_phone', '')), '\D', '', 'g');
  IF length(v_contact_phone) < 8 THEN RAISE EXCEPTION 'CONTACT_PHONE_INVALID' USING ERRCODE = 'P0001'; END IF;
  IF length(v_postal_code) <> 4 THEN RAISE EXCEPTION 'POSTAL_CODE_INVALID' USING ERRCODE = 'P0001'; END IF;
  IF v_city = '' THEN RAISE EXCEPTION 'CITY_REQUIRED' USING ERRCODE = 'P0001'; END IF;
  v_employees := (p_payload ->> 'employees_estimate')::int;
  IF v_employees IS NULL THEN v_employees := (p_payload ->> 'employee_count')::int; END IF;
  IF v_employees IS NULL OR v_employees < 20 THEN RAISE EXCEPTION 'EMPLOYEE_COUNT_MIN_20' USING ERRCODE = 'P0001'; END IF;
  IF v_address_line = '' THEN v_address_line := v_city || ' ' || v_postal_code; END IF;
  IF EXISTS (SELECT 1 FROM public.companies c WHERE c.deleted_at IS NULL AND (btrim(coalesce(c.orgnr, '')) = v_orgnr OR btrim(coalesce(c.organization_number, '')) = v_orgnr) AND c.status <> 'PENDING'::public.company_status) THEN RAISE EXCEPTION 'ORGNR_ALREADY_REGISTERED' USING ERRCODE = 'P0001'; END IF;
  v_provider_id := public.lp_match_provider_by_postal_code(v_postal_code);
  IF v_provider_id IS NOT NULL THEN SELECT p.name INTO v_provider_name FROM public.providers p WHERE p.id = v_provider_id; END IF;
  v_submitted := jsonb_build_object('source', 'provider_registration_intake', 'company_name', v_company_name, 'orgnr', v_orgnr, 'contact_name', v_contact_name, 'contact_email', v_contact_email, 'contact_phone', v_contact_phone, 'postal_code', v_postal_code, 'city', v_city, 'employee_count', v_employees, 'notes', v_notes, 'expand_my_area', v_provider_id IS NULL);
  v_raw := coalesce(p_payload, '{}'::jsonb);
  INSERT INTO public.company_registrations (company_id, status, orgnr, company_name, submitted_by_email, submitted_by_name, contact_name, contact_email, contact_phone, address_line, postal_code, city, employee_count, provider_id, requested_postal_code, requested_city, submitted_payload, raw_payload)
  VALUES (NULL, 'PENDING', v_orgnr, v_company_name, v_contact_email, v_contact_name, v_contact_name, v_contact_email, v_contact_phone, v_address_line, v_postal_code, v_city, v_employees, v_provider_id, v_postal_code, v_city, v_submitted, v_raw)
  RETURNING id INTO v_registration_id;
  INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (NULL, 'company_registration_intake', 'company_registration', v_registration_id, NULL, jsonb_build_object('provider_id', v_provider_id, 'postal_code', v_postal_code, 'expand_my_area', v_provider_id IS NULL));
  IF v_provider_id IS NOT NULL THEN INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (NULL, 'registration_provider_notify_stub', 'provider', v_provider_id, 'Provider admin notification stub (Patch 13)', jsonb_build_object('registration_id', v_registration_id, 'contact_email', v_contact_email)); END IF;
  RETURN jsonb_build_object('ok', true, 'registration_id', v_registration_id, 'matched_provider_id', v_provider_id, 'matched_provider_name', v_provider_name, 'expand_my_area', v_provider_id IS NULL, 'message', CASE WHEN v_provider_id IS NOT NULL THEN 'Registrering mottatt. Leverandor er varslet.' ELSE 'Registrering mottatt. Vi dekker ikke omradet enna, men har notert interessen.' END);
END;
$_$;


--
-- Name: lp_company_registration_reject_provider(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_registration_reject_provider(p_registration_id uuid, p_reason text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_reg public.company_registrations%rowtype; v_actor uuid := auth.uid(); v_reason text := nullif(btrim(coalesce(p_reason, '')), ''); v_now timestamptz := now(); BEGIN IF v_actor IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED' USING ERRCODE = '42501'; END IF; v_reg := private.lp_assert_registration_approve_access(p_registration_id); IF upper(coalesce(v_reg.status, '')) <> 'PENDING' THEN RAISE EXCEPTION 'REGISTRATION_NOT_PENDING' USING ERRCODE = 'P0001'; END IF; UPDATE public.company_registrations SET status = 'REJECTED', reviewed_at = v_now, reviewed_by = v_actor, rejection_reason = v_reason, updated_at = v_now WHERE id = v_reg.id; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (v_actor, 'company_registration_rejected', 'company_registration', v_reg.id, v_reason, jsonb_build_object('provider_id', v_reg.provider_id)); RETURN jsonb_build_object('ok', true, 'status', 'REJECTED'); END; $$;


--
-- Name: lp_company_resume(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_resume(p_company_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_provider_id uuid; v_cascade_orders int := 0;
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'COMPANY_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  PERFORM private.lp_assert_provider_admin_access(v_provider_id);
  UPDATE public.companies SET suspended_at = NULL, suspended_by = NULL, suspended_reason = NULL, paused_at = NULL, paused_by = NULL, paused_reason = NULL WHERE id = p_company_id;
  v_cascade_orders := private.lp_orders_resume_paused(p_company_id := p_company_id);
  PERFORM private.lp_lifecycle_audit('resume', 'company', p_company_id, NULL, jsonb_build_object('provider_id', v_provider_id, 'cascade_orders_resumed', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_orders_resumed', v_cascade_orders);
END; $$;


--
-- Name: lp_company_suspend(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_company_suspend(p_company_id uuid, p_reason text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_provider_id uuid; v_cascade_orders int := 0;
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'COMPANY_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  PERFORM private.lp_assert_provider_admin_access(v_provider_id);
  IF EXISTS (SELECT 1 FROM public.companies WHERE id = p_company_id AND suspended_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_suspended', true); END IF;
  UPDATE public.companies SET suspended_at = now(), suspended_by = auth.uid(), suspended_reason = p_reason WHERE id = p_company_id;
  v_cascade_orders := private.lp_orders_pause_active(p_company_id := p_company_id);
  PERFORM private.lp_lifecycle_audit('suspend', 'company', p_company_id, p_reason, jsonb_build_object('provider_id', v_provider_id, 'cascade_orders_paused', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_orders_paused', v_cascade_orders);
END; $$;


--
-- Name: lp_compute_agreements_due_today(date); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_compute_agreements_due_today(p_today date) RETURNS TABLE(agreement_id uuid, provider_id uuid, company_id uuid, billing_cycle text, period_start date, period_end date)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
begin
...
--
-- Name: lp_generate_agreement_invoices_for_period(date, date, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_generate_agreement_invoices_for_period(p_period_start date, p_period_end date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
begin
  ...
  if p_period_start is null or p_period_end is null then raise exception 'PERIOD_REQUIRED' using errcode = 'P0001'; end if;
  ...
  for v_agreement in
    ...
    where upper(a.status::text) = 'ACTIVE'
      and c.suspended_at is null and c.paused_at is null
      and p.suspended_at is null and p.paused_at is null and p.deleted_at is null
    order by a.provider_id, a.id
  loop
    begin
      v_result := private.lp_generate_agreement_invoice_core(v_agreement.id, p_period_start, p_period_end, p_request_rid);
      if coalesce((v_result->>'skipped')::boolean, false) then v_skipped := v_skipped + 1;
    ...
    exception when others then
      v_errors := v_errors || jsonb_build_array(jsonb_build_object('agreement_id', v_agreement.id, 'sqlstate', sqlstate, 'message', sqlerrm));
    end;
  end loop;

  return jsonb_build_object('ok', true, 'period_start', p_period_start, 'period_end', p_period_end,
    'generated', v_generated, 'skipped', v_skipped, 'error_count', jsonb_array_length(v_errors),
    'errors', v_errors, 'invoice_ids', v_invoice_ids);
end;
$$;


--
-- Name: lp_generate_saas_invoices_for_period(date, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_generate_saas_invoices_for_period(p_invoice_period date DEFAULT NULL::date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_match_provider_by_postal_code(text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_match_provider_by_postal_code(p_postal_code text) RETURNS uuid
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_pc text := regexp_replace(btrim(coalesce(p_postal_code, '')), '\D', '', 'g'); v_provider_id uuid; BEGIN IF length(v_pc) <> 4 THEN RETURN NULL; END IF; SELECT psa.provider_id INTO v_provider_id FROM public.provider_service_areas psa INNER JOIN public.providers p ON p.id = psa.provider_id WHERE psa.active = true AND p.status = 'ACTIVE'::public.provider_status AND p.deleted_at IS NULL AND regexp_replace(btrim(coalesce(psa.postal_code_from, '')), '\D', '', 'g') <= v_pc AND regexp_replace(btrim(coalesce(psa.postal_code_to, '')), '\D', '', 'g') >= v_pc ORDER BY psa.provider_id ASC LIMIT 1; RETURN v_provider_id; END; $$;


--
-- Name: lp_order_advance_status(uuid, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: FUNCTION lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text) IS 'Provider kitchen flow: ACTIVE/LOCKED→PREPARED→DISPATCHED→DELIVERED. Admin may reopen DELIVERED→DISPATCHED.';
...
--
-- Name: lp_outbox_claim(integer, text, text[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_outbox_claim(p_limit integer DEFAULT 25, p_worker text DEFAULT NULL::text, p_exclude_prefixes text[] DEFAULT NULL::text[]) RETURNS SETOF public.outbox
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_outbox_retry_event(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_outbox_retry_event(p_event_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_apply_connection_health_check(uuid, text, boolean, boolean, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_apply_connection_health_check(p_provider_id uuid, p_env text, p_ok boolean, p_auth_failed boolean DEFAULT false, p_company_name text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_complete_onboarding_provisioning(uuid, text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_complete_onboarding_provisioning(p_provider_id uuid, p_env text, p_summary jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_complete_tripletex_connection(uuid, text, bigint, text, text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_complete_tripletex_connection(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_consumer_token text, p_verification_result jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
...
--
-- Name: lp_provider_create(text, text, text, text, text, text, text, text, text, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_create(p_slug text, p_name text, p_contact_email text, p_org_number text DEFAULT NULL::text, p_contact_phone text DEFAULT NULL::text, p_billing_org_no text DEFAULT NULL::text, p_billing_email text DEFAULT NULL::text, p_billing_address text DEFAULT NULL::text, p_default_tier_pricing text DEFAULT NULL::text, p_billing_model text DEFAULT 'SAAS_FIXED'::text, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $_$
declare
  ...
  v_event_key text;
begin
  if not public.is_platform_admin() then raise exception 'PERMISSION_DENIED' using errcode = '42501'; end if;
  if v_slug = '' or v_slug !~ '^[a-z0-9]([a-z0-9-]*[a-z0-9])?$' then raise exception 'INVALID_SLUG' using errcode = '22023'; end if;
  if v_name = '' then raise exception 'NAME_REQUIRED' using errcode = '22023'; end if;
  if v_email = '' or position('@' in v_email) = 0 then raise exception 'INVALID_CONTACT_EMAIL' using errcode = '22023'; end if;
  if v_billing_model not in ('SAAS_FIXED', 'SAAS_PER_COMPANY', 'CUSTOM') then raise exception 'INVALID_BILLING_MODEL' using errcode = '22023'; end if;
  if exists (select 1 from public.providers where slug = v_slug) then raise exception 'SLUG_ALREADY_EXISTS' using errcode = '23505'; end if;
  if exists (select 1 from public.providers where name = v_name) then raise exception 'NAME_ALREADY_EXISTS' using errcode = '23505'; end if;
  if v_org is not null and exists (select 1 from public.providers where org_number = v_org) then raise exception 'ORG_NUMBER_ALREADY_EXISTS' using errcode = '23505'; end if;
  if to_regclass('public.outbox') is null then raise exception 'OUTBOX_MISSING' using errcode = 'P0001'; end if;
  insert into public.providers (id, name, slug, org_number, contact_email, contact_phone, billing_model, status) values (v_provider_id, v_name, v_slug, v_org, v_email, v_phone, v_billing_model, 'ACTIVE'::public.provider_status);
  perform private.lp_lifecycle_audit('provider_created', 'provider', v_provider_id, null, jsonb_build_object('slug', v_slug, 'name', v_name, 'contact_email', v_email, 'org_number', v_org, 'billing_model', v_billing_model, 'billing_address', nullif(btrim(coalesce(p_billing_address, '')), ''), 'default_tier_pricing', nullif(btrim(coalesce(p_default_tier_pricing, '')), '')));
  v_request_rid := coalesce(nullif(btrim(coalesce(p_request_rid, '')), ''), replace(gen_random_uuid()::text, '-', ''));
  v_event_key := format('tripletex.provider_customer_create_lp:%s', v_provider_id::text);
  insert into public.outbox (event_key, payload, status, attempts, last_error, locked_at, locked_by) values (v_event_key, jsonb_build_object('provider_id', v_provider_id, 'target', 'lp', 'request_rid', v_request_rid), 'PENDING', 0, null, null, null) on conflict (event_key) do nothing;
  return jsonb_build_object('ok', true, 'provider_id', v_provider_id, 'event_key', v_event_key, 'request_rid', v_request_rid);
end;
$_$;


--
-- Name: lp_provider_delete(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_delete(p_provider_id uuid, p_reason text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_cascade_companies int := 0; v_cascade_orders int := 0;
BEGIN
  PERFORM private.lp_lifecycle_require_reason(p_reason);
  IF NOT public.is_platform_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.providers WHERE id = p_provider_id) THEN RAISE EXCEPTION 'PROVIDER_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  IF EXISTS (SELECT 1 FROM public.providers WHERE id = p_provider_id AND deleted_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_deleted', true); END IF;
  UPDATE public.providers SET status = 'CLOSED'::public.provider_status, deleted_at = now() WHERE id = p_provider_id;
  UPDATE public.companies SET deleted_at = now() WHERE provider_id = p_provider_id AND deleted_at IS NULL;
  ...
  PERFORM private.lp_lifecycle_audit('delete', 'provider', p_provider_id, p_reason, jsonb_build_object('cascade_companies_deleted', v_cascade_companies, 'cascade_orders_cancelled', v_cascade_orders, 'grace_days', 30));
  RETURN jsonb_build_object('ok', true, 'cascade_companies_deleted', v_cascade_companies, 'cascade_orders_cancelled', v_cascade_orders);
END;
$$;


--
-- Name: lp_provider_disconnect_tripletex(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_disconnect_tripletex(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_finalize_tripletex_connection(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_finalize_tripletex_connection(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_generate_agreement_invoice_for_period(uuid, date, date, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_generate_agreement_invoice_for_period(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare v_agreement public.agreements%rowtype;
begin
  select * into v_agreement from public.agreements a where a.id = p_agreement_id;
  if not found then raise exception 'AGREEMENT_NOT_FOUND' using errcode = 'P0002'; end if;
  if v_agreement.provider_id is null then raise exception 'AGREEMENT_PROVIDER_MISSING' using errcode = 'P0001'; end if;
  perform private.lp_assert_provider_admin_or_superadmin(v_agreement.provider_id);
  return private.lp_generate_agreement_invoice_core(p_agreement_id, p_period_start, p_period_end, p_request_rid);
end;
$$;


--
-- Name: lp_provider_generate_invoice_for_period(uuid, date, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_generate_invoice_for_period(p_provider_id uuid, p_invoice_period date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_get_connection_health(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_get_connection_health(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_get_tripletex_credentials_status(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_get_tripletex_credentials_status(p_provider_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare v_row public.provider_tripletex_credentials%rowtype;
begin
  perform private.lp_assert_provider_admin_or_superadmin(p_provider_id);
  select * into v_row from public.provider_tripletex_credentials c where c.provider_id = p_provider_id;
  if not found then
    return jsonb_build_object('is_configured', false, 'env', null, 'last_used_at', null,
      'company_id_external', null, 'sync_status', null);
  end if;
  return jsonb_build_object('is_configured', true, 'env', v_row.env, 'last_used_at', v_row.last_used_at,
    'company_id_external', v_row.company_id_external, 'sync_status', v_row.sync_status, 'rotation_due', v_row.rotation_due);
end;
$$;


--
-- Name: lp_provider_load_tripletex_credentials(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_load_tripletex_credentials(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'vault', 'pg_catalog'
    AS $$
declare
  v_env text := lower(btrim(coalesce(p_env, '')));
  v_row public.provider_tripletex_credentials%rowtype;
  v_consumer text; v_employee text;
begin
  if v_env not in ('test', 'prod') then raise exception 'INVALID_ENV' using errcode = '22023'; end if;
  select * into v_row from public.provider_tripletex_credentials c where c.provider_id = p_provider_id;
  if not found then raise exception 'PROVIDER_CREDENTIALS_NOT_CONFIGURED' using errcode = 'P0002'; end if;
  if v_row.env <> v_env then
    raise exception 'PROVIDER_CREDENTIALS_ENV_MISMATCH: stored=%, requested=%', v_row.env, v_env using errcode = 'P0001';
  end if;
  if v_row.sync_status = 'DISABLED' then raise exception 'PROVIDER_CREDENTIALS_DISABLED' using errcode = 'P0001'; end if;
  select ds.decrypted_secret into v_consumer from vault.decrypted_secrets ds where ds.id = v_row.consumer_token_secret_id;
  select ds.decrypted_secret into v_employee from vault.decrypted_secrets ds where ds.id = v_row.employee_token_secret_id;
  if coalesce(v_consumer, '') = '' or coalesce(v_employee, '') = '' then
    raise exception 'PROVIDER_CREDENTIALS_NOT_CONFIGURED' using errcode = 'P0002';
  end if;
  update public.provider_tripletex_credentials set last_used_at = now(), updated_at = now() where provider_id = p_provider_id;
  insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata)
  values (null, 'tripletex_credentials_loaded', 'tripletex_credentials', p_provider_id,
    'Server-side credential load for Tripletex API',
    jsonb_build_object('env', v_env, 'request_source', 'lp_provider_load_tripletex_credentials'));
  return jsonb_build_object('provider_id', p_provider_id, 'env', v_env, 'company_id_external', v_row.company_id_external,
    'consumer_token', v_consumer, 'employee_token', v_employee);
end;
$$;


--
-- Name: lp_provider_load_webhook_secret(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_load_webhook_secret(p_provider_id uuid, p_env text DEFAULT 'prod'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'vault', 'pg_catalog'
    AS $$
declare v_env text := lower(btrim(coalesce(p_env, 'prod'))); v_row public.provider_tripletex_webhook_secrets%rowtype; v_secret text;
begin
  if auth.role() <> 'service_role' then raise exception 'PERMISSION_DENIED' using errcode = '42501'; end if;
  if v_env not in ('test', 'prod') then raise exception 'INVALID_ENV' using errcode = '22023'; end if;
  select * into v_row from public.provider_tripletex_webhook_secrets s where s.provider_id = p_provider_id and s.env = v_env;
  if not found then raise exception 'WEBHOOK_SECRET_NOT_CONFIGURED' using errcode = 'P0002'; end if;
  select ds.decrypted_secret into v_secret from vault.decrypted_secrets ds where ds.id = v_row.webhook_secret_id;
  if coalesce(v_secret, '') = '' then raise exception 'WEBHOOK_SECRET_NOT_CONFIGURED' using errcode = 'P0002'; end if;
  return jsonb_build_object('ok', true, 'provider_id', p_provider_id, 'env', v_env, 'webhook_secret', v_secret);
end; $$;


--
-- Name: lp_provider_pause(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_pause(p_provider_id uuid, p_reason text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_cascade_orders int := 0;
BEGIN
  IF NOT public.is_platform_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.providers WHERE id = p_provider_id) THEN RAISE EXCEPTION 'PROVIDER_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  IF EXISTS (SELECT 1 FROM public.providers WHERE id = p_provider_id AND paused_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_paused', true); END IF;
  UPDATE public.providers SET status = 'PAUSED'::public.provider_status, paused_at = now(), paused_by = auth.uid(), paused_reason = p_reason WHERE id = p_provider_id;
  v_cascade_orders := private.lp_orders_pause_active(p_provider_id := p_provider_id);
  PERFORM private.lp_lifecycle_audit('pause', 'provider', p_provider_id, p_reason, jsonb_build_object('cascade_orders_paused', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_orders_paused', v_cascade_orders);
END;
$$;
...
--
-- Name: lp_provider_purge_disconnected_vault(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_purge_disconnected_vault(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_reconnect_tripletex(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_reconnect_tripletex(p_provider_id uuid, p_env text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_resume(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_resume(p_provider_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_cascade_companies int := 0; v_cascade_orders int := 0;
BEGIN
  IF NOT public.is_platform_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.providers WHERE id = p_provider_id) THEN RAISE EXCEPTION 'PROVIDER_NOT_FOUND' USING ERRCODE = '02000'; END IF;
  UPDATE public.providers SET status = 'ACTIVE'::public.provider_status, suspended_at = NULL, suspended_by = NULL, suspended_reason = NULL, paused_at = NULL, paused_by = NULL, paused_reason = NULL WHERE id = p_provider_id;
  UPDATE public.companies SET suspended_at = NULL, suspended_by = NULL, suspended_reason = NULL, paused_at = NULL, paused_by = NULL, paused_reason = NULL WHERE provider_id = p_provider_id AND deleted_at IS NULL;
  GET DIAGNOSTICS v_cascade_companies = ROW_COUNT;
  v_cascade_orders := private.lp_orders_resume_paused(p_provider_id := p_provider_id);
  PERFORM private.lp_lifecycle_audit('resume', 'provider', p_provider_id, NULL, jsonb_build_object('cascade_companies_resumed', v_cascade_companies, 'cascade_orders_resumed', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_companies_resumed', v_cascade_companies, 'cascade_orders_resumed', v_cascade_orders);
END;
$$;


--
-- Name: lp_provider_rotate_webhook_secret(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_rotate_webhook_secret(p_provider_id uuid, p_env text DEFAULT 'prod'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'vault', 'extensions', 'pg_catalog'
    AS $$
declare v_env text := lower(btrim(coalesce(p_env, 'prod'))); v_secret text; v_secret_id uuid; v_name text; v_existing public.provider_tripletex_webhook_secrets%rowtype; v_actor uuid := auth.uid();
begin
  if v_env not in ('test', 'prod') then raise exception 'INVALID_ENV' using errcode = '22023'; end if;
  perform private.lp_assert_provider_admin_or_superadmin(p_provider_id);
  if not exists (select 1 from public.providers p where p.id = p_provider_id and p.deleted_at is null) then raise exception 'PROVIDER_NOT_FOUND' using errcode = 'P0002'; end if;
  v_secret := encode(extensions.gen_random_bytes(32), 'hex'); v_name := private.lp_tripletex_webhook_vault_name(p_provider_id, v_env);
  select * into v_existing from public.provider_tripletex_webhook_secrets s where s.provider_id = p_provider_id and s.env = v_env;
  if found then delete from vault.secrets where id = v_existing.webhook_secret_id; end if;
  v_secret_id := vault.create_secret(v_secret, v_name, 'Tripletex webhook secret for provider ' || p_provider_id::text);
  insert into public.provider_tripletex_webhook_secrets (provider_id, env, webhook_secret_id, last_rotated_at, updated_at) values (p_provider_id, v_env, v_secret_id, now(), now()) on conflict (provider_id, env) do update set webhook_secret_id = excluded.webhook_secret_id, last_rotated_at = excluded.last_rotated_at, updated_at = excluded.updated_at;
  insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) values (v_actor, 'tripletex_webhook_secret_rotated', 'tripletex_webhook_secret', p_provider_id, 'Webhook secret rotated (shown once in RPC response)', jsonb_build_object('env', v_env, 'provider_id', p_provider_id));
  return jsonb_build_object('ok', true, 'provider_id', p_provider_id, 'env', v_env, 'webhook_secret', v_secret, 'rotated_at', now());
end; $$;


--
-- Name: lp_provider_set_subscription(uuid, text, numeric, text, text, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_set_subscription(p_provider_id uuid, p_plan text, p_monthly_amount numeric, p_billing_email text, p_billing_org_number text DEFAULT NULL::text, p_billing_address text DEFAULT NULL::text, p_notes text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $_$ DECLARE v_plan text := upper(btrim(coalesce(p_plan, ''))); v_email text := lower(btrim(coalesce(p_billing_email, ''))); v_old uuid; v_new uuid; BEGIN IF NOT public.is_platform_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF; IF v_plan NOT IN ('SAAS_FIXED','SAAS_PER_COMPANY','CUSTOM') THEN RAISE EXCEPTION 'INVALID_PLAN' USING ERRCODE = '22023'; END IF; IF p_monthly_amount IS NULL OR p_monthly_amount < 0 THEN RAISE EXCEPTION 'INVALID_AMOUNT' USING ERRCODE = '22023'; END IF; IF v_email = '' OR v_email !~ '^[^@\s]+@[^@\s]+\.[^@\s]+$' THEN RAISE EXCEPTION 'INVALID_BILLING_EMAIL' USING ERRCODE = '22023'; END IF; UPDATE public.provider_subscriptions SET active_to = now(), status = CASE WHEN status = 'ACTIVE' THEN 'CANCELLED' ELSE status END WHERE provider_id = p_provider_id AND active_to IS NULL RETURNING id INTO v_old; INSERT INTO public.provider_subscriptions (provider_id, plan, monthly_amount, tax_code_id, billing_email, billing_org_number, billing_address, status, notes, created_by) VALUES (p_provider_id, v_plan, p_monthly_amount, 'MVA_25', v_email, nullif(btrim(coalesce(p_billing_org_number,'')),'') , nullif(btrim(coalesce(p_billing_address,'')),'') , 'ACTIVE', nullif(btrim(coalesce(p_notes,'')),'') , auth.uid()) RETURNING id INTO v_new; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (auth.uid(), 'subscription_set', 'provider', p_provider_id, format('SaaS-fee %s', p_monthly_amount), jsonb_build_object('new_subscription_id', v_new)); RETURN jsonb_build_object('ok', true, 'subscription_id', v_new); END; $_$;


--
-- Name: lp_provider_set_tripletex_credentials(uuid, text, text, text, bigint); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_set_tripletex_credentials(p_provider_id uuid, p_env text, p_consumer_token text, p_employee_token text, p_company_id_external bigint DEFAULT NULL::bigint) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'vault', 'pg_catalog'
    AS $$
declare
  ...
  v_actor uuid := auth.uid();
  v_row public.provider_tripletex_credentials%rowtype;
  v_consumer_name text; v_employee_name text;
  v_consumer_secret_id uuid; v_employee_secret_id uuid;
begin
  perform private.lp_assert_provider_admin_or_superadmin(p_provider_id);
  if v_env not in ('test', 'prod') then raise exception 'INVALID_ENV' using errcode = '22023'; end if;
  if v_consumer = '' or v_employee = '' then raise exception 'INVALID_CREDENTIALS' using errcode = '22023'; end if;
  ...
  if found then
    perform vault.update_secret(v_row.consumer_token_secret_id, v_consumer, v_consumer_name);
    perform vault.update_secret(v_row.employee_token_secret_id, v_employee, v_employee_name);
    update public.provider_tripletex_credentials set env = v_env, company_id_external = p_company_id_external,
      sync_status = 'READY', updated_at = now() where provider_id = p_provider_id returning * into v_row;
  else
    v_consumer_secret_id := vault.create_secret(v_consumer, v_consumer_name, 'Tripletex consumer token (TPT-B-1)');
    v_employee_secret_id := vault.create_secret(v_employee, v_employee_name, 'Tripletex employee token (TPT-B-1)');
    insert into public.provider_tripletex_credentials (provider_id, env, consumer_token_secret_id, employee_token_secret_id, company_id_external, sync_status)
    values (p_provider_id, v_env, v_consumer_secret_id, v_employee_secret_id, p_company_id_external, 'READY') returning * into v_row;
  end if;
  insert into public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata)
  values (v_actor, 'tripletex_credentials_set', 'tripletex_credentials', p_provider_id, null,
    jsonb_build_object('env', v_env, 'company_id_external', p_company_id_external, 'sync_status', v_row.sync_status));
  return jsonb_build_object('ok', true, 'provider_id', p_provider_id, 'env', v_row.env, 'is_configured', true,
    'company_id_external', v_row.company_id_external, 'sync_status', v_row.sync_status);
end;
$$;


--
-- Name: lp_provider_suspend(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_suspend(p_provider_id uuid, p_reason text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  UPDATE public.providers SET status = 'SUSPENDED'::public.provider_status, suspended_at = now(), suspended_by = auth.uid(), suspended_reason = p_reason WHERE id = p_provider_id;
  UPDATE public.companies SET suspended_at = now(), suspended_by = auth.uid(), suspended_reason = p_reason WHERE provider_id = p_provider_id AND deleted_at IS NULL AND suspended_at IS NULL;
  GET DIAGNOSTICS v_cascade_companies = ROW_COUNT;
  v_cascade_orders := private.lp_orders_pause_active(p_provider_id := p_provider_id);
  PERFORM private.lp_lifecycle_audit('suspend', 'provider', p_provider_id, p_reason, jsonb_build_object('cascade_companies', v_cascade_companies, 'cascade_orders_paused', v_cascade_orders));
  RETURN jsonb_build_object('ok', true, 'cascade_companies', v_cascade_companies, 'cascade_orders_paused', v_cascade_orders);
END;
$$;


--
-- Name: lp_provider_test_tripletex_token(uuid, text, bigint, text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_test_tripletex_token(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text DEFAULT NULL::text, p_verification_result jsonb DEFAULT NULL::jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_provider_update_billing_contact(uuid, text, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_provider_update_billing_contact(p_provider_id uuid, p_billing_email text, p_billing_org_number text DEFAULT NULL::text, p_billing_address text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $_$ DECLARE v_email text := lower(btrim(coalesce(p_billing_email, ''))); v_sub_id uuid; BEGIN IF v_email = '' OR v_email !~ '^[^@\s]+@[^@\s]+\.[^@\s]+$' THEN RAISE EXCEPTION 'INVALID_BILLING_EMAIL' USING ERRCODE = '22023'; END IF; IF NOT public.is_platform_admin() AND NOT EXISTS (SELECT 1 FROM public.provider_memberships pm WHERE pm.user_id = auth.uid() AND pm.provider_id = p_provider_id AND pm.role = 'provider_admin'::public.provider_role) THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF; UPDATE public.provider_subscriptions SET billing_email = v_email, billing_org_number = nullif(btrim(coalesce(p_billing_org_number,'')),'') , billing_address = nullif(btrim(coalesce(p_billing_address,'')),'')  WHERE provider_id = p_provider_id AND active_to IS NULL AND status = 'ACTIVE' RETURNING id INTO v_sub_id; IF v_sub_id IS NULL THEN RAISE EXCEPTION 'ACTIVE_SUBSCRIPTION_NOT_FOUND' USING ERRCODE = 'P0002'; END IF; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (auth.uid(), 'subscription_billing_contact_updated', 'provider_subscription', v_sub_id, NULL, jsonb_build_object('provider_id', p_provider_id)); RETURN jsonb_build_object('ok', true, 'subscription_id', v_sub_id); END; $_$;
...
--
-- Name: lp_run_daily_agreement_billing(date, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_run_daily_agreement_billing(p_today date DEFAULT (timezone('Europe/Oslo'::text, now()))::date, p_request_rid text DEFAULT NULL::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$
declare
...
--
-- Name: lp_service_area_save(uuid, uuid, text, text, text, integer, integer, text[], boolean); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_service_area_save(p_id uuid, p_provider_id uuid, p_city text, p_postal_code_from text, p_postal_code_to text, p_min_employees integer, p_max_employees integer, p_available_days text[], p_active boolean) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_from text := regexp_replace(btrim(coalesce(p_postal_code_from, '')), '\D', '', 'g'); v_to text := regexp_replace(btrim(coalesce(p_postal_code_to, '')), '\D', '', 'g'); v_city text := btrim(coalesce(p_city, '')); v_active boolean := coalesce(p_active, true); v_overlap_city text; v_overlap_from text; v_overlap_to text; v_audit_action text; v_saved_id uuid; v_days text[]; BEGIN IF NOT public.is_platform_admin() AND NOT EXISTS (SELECT 1 FROM public.provider_memberships pm WHERE pm.user_id = auth.uid() AND pm.provider_id = p_provider_id AND pm.role = 'provider_admin'::public.provider_role) THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF; IF v_city = '' THEN RAISE EXCEPTION 'CITY_REQUIRED' USING ERRCODE = 'P0001'; END IF; IF length(v_from) <> 4 OR length(v_to) <> 4 THEN RAISE EXCEPTION 'POSTAL_CODE_FORMAT_INVALID' USING ERRCODE = '22023'; END IF; IF v_from > v_to THEN RAISE EXCEPTION 'POSTAL_RANGE_INVALID' USING ERRCODE = '22023'; END IF; IF p_min_employees IS NOT NULL AND p_max_employees IS NOT NULL AND p_min_employees > p_max_employees THEN RAISE EXCEPTION 'EMPLOYEE_RANGE_INVALID' USING ERRCODE = 'P0001'; END IF; v_days := coalesce(p_available_days, ARRAY['mon','tue','wed','thu','fri']::text[]); IF v_active THEN SELECT psa.city, psa.postal_code_from, psa.postal_code_to INTO v_overlap_city, v_overlap_from, v_overlap_to FROM public.provider_service_areas psa WHERE psa.provider_id = p_provider_id AND (p_id IS NULL OR psa.id <> p_id) AND psa.active = true AND NOT (psa.postal_code_to < v_from OR psa.postal_code_from > v_to) LIMIT 1; IF FOUND THEN RAISE EXCEPTION '%', format('POSTAL_RANGE_OVERLAPS_EXISTING:%s:%s-%s', coalesce(v_overlap_city, 'omrade'), coalesce(v_overlap_from, '?'), coalesce(v_overlap_to, '?')) USING ERRCODE = '23505'; END IF; END IF; IF p_id IS NULL THEN INSERT INTO public.provider_service_areas (provider_id, country, city, postal_code_from, postal_code_to, min_employees, max_employees, available_days, active) VALUES (p_provider_id, 'NO', v_city, v_from, v_to, p_min_employees, p_max_employees, v_days, v_active) RETURNING id INTO v_saved_id; v_audit_action := 'service_area_create'; ELSE IF NOT EXISTS (SELECT 1 FROM public.provider_service_areas psa WHERE psa.id = p_id AND psa.provider_id = p_provider_id) THEN RAISE EXCEPTION 'SERVICE_AREA_NOT_FOUND' USING ERRCODE = 'P0002'; END IF; UPDATE public.provider_service_areas SET city = v_city, postal_code_from = v_from, postal_code_to = v_to, min_employees = p_min_employees, max_employees = p_max_employees, available_days = v_days, active = v_active WHERE id = p_id AND provider_id = p_provider_id; v_saved_id := p_id; v_audit_action := 'service_area_update'; END IF; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (auth.uid(), v_audit_action, 'service_area', v_saved_id, format('Service area %s for %s-%s', v_audit_action, v_from, v_to), jsonb_build_object('provider_id', p_provider_id, 'city', v_city, 'active', v_active, 'postal_code_from', v_from, 'postal_code_to', v_to)); RETURN jsonb_build_object('ok', true, 'id', v_saved_id); END; $$;


--
-- Name: lp_service_area_toggle_active(uuid, boolean); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_service_area_toggle_active(p_id uuid, p_active boolean) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_catalog'
    AS $$ DECLARE v_row public.provider_service_areas%rowtype; v_active boolean := coalesce(p_active, false); v_overlap_city text; v_overlap_from text; v_overlap_to text; BEGIN SELECT * INTO v_row FROM public.provider_service_areas WHERE id = p_id; IF NOT FOUND THEN RAISE EXCEPTION 'SERVICE_AREA_NOT_FOUND' USING ERRCODE = 'P0002'; END IF; IF NOT public.is_platform_admin() AND NOT EXISTS (SELECT 1 FROM public.provider_memberships pm WHERE pm.user_id = auth.uid() AND pm.provider_id = v_row.provider_id AND pm.role = 'provider_admin'::public.provider_role) THEN RAISE EXCEPTION 'PERMISSION_DENIED' USING ERRCODE = '42501'; END IF; IF v_active THEN SELECT psa.city, psa.postal_code_from, psa.postal_code_to INTO v_overlap_city, v_overlap_from, v_overlap_to FROM public.provider_service_areas psa WHERE psa.provider_id = v_row.provider_id AND psa.id <> p_id AND psa.active = true AND NOT (psa.postal_code_to < v_row.postal_code_from OR psa.postal_code_from > v_row.postal_code_to) LIMIT 1; IF FOUND THEN RAISE EXCEPTION '%', format('POSTAL_RANGE_OVERLAPS_EXISTING:%s:%s-%s', coalesce(v_overlap_city, 'omrade'), coalesce(v_overlap_from, '?'), coalesce(v_overlap_to, '?')) USING ERRCODE = '23505'; END IF; END IF; UPDATE public.provider_service_areas SET active = v_active WHERE id = p_id; INSERT INTO public.lifecycle_audit_log (actor_id, action, entity_type, entity_id, reason, metadata) VALUES (auth.uid(), 'service_area_toggle', 'service_area', p_id, format('Service area active=%s', v_active), jsonb_build_object('provider_id', v_row.provider_id, 'city', v_row.city, 'active', v_active, 'postal_code_from', v_row.postal_code_from, 'postal_code_to', v_row.postal_code_to)); RETURN jsonb_build_object('ok', true, 'id', p_id, 'active', v_active); END; $$;
...
--
-- Name: lp_user_delete(uuid, text, boolean); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_user_delete(p_user_id uuid, p_reason text, p_gdpr boolean DEFAULT false) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_company_id uuid; v_provider_id uuid; v_meta jsonb;
BEGIN
  PERFORM private.lp_lifecycle_require_reason(p_reason);
  SELECT o_company_id, o_provider_id INTO v_company_id, v_provider_id FROM private.lp_assert_user_lifecycle_access(p_user_id);
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_user_id AND deleted_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_deleted', true); END IF;
  UPDATE public.profiles SET deleted_at = now(), full_name = CASE WHEN p_gdpr THEN NULL ELSE full_name END, phone = CASE WHEN p_gdpr THEN NULL ELSE phone END WHERE id = p_user_id;
  IF p_gdpr THEN UPDATE auth.users SET email = 'deleted-' || p_user_id::text || '@deleted.local', banned_until = '9999-12-31 00:00:00+00'::timestamptz WHERE id = p_user_id; END IF;
  v_meta := jsonb_build_object('company_id', v_company_id, 'provider_id', v_provider_id, 'gdpr_invoked', coalesce(p_gdpr, false));
...
--
-- Name: lp_user_pause(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_user_pause(p_user_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_company_id uuid; v_provider_id uuid;
BEGIN
  SELECT o_company_id, o_provider_id INTO v_company_id, v_provider_id FROM private.lp_assert_user_lifecycle_access(p_user_id);
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_user_id AND paused_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_paused', true); END IF;
  UPDATE public.profiles SET paused_at = now() WHERE id = p_user_id;
  PERFORM private.lp_lifecycle_audit('pause', 'user', p_user_id, NULL, jsonb_build_object('company_id', v_company_id, 'provider_id', v_provider_id));
  RETURN jsonb_build_object('ok', true);
END; $$;


--
-- Name: lp_user_resume(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_user_resume(p_user_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_company_id uuid; v_provider_id uuid;
BEGIN
  SELECT o_company_id, o_provider_id INTO v_company_id, v_provider_id FROM private.lp_assert_user_lifecycle_access(p_user_id);
  UPDATE public.profiles SET suspended_at = NULL, suspended_by = NULL, suspended_reason = NULL, paused_at = NULL WHERE id = p_user_id;
  PERFORM private.lp_lifecycle_audit('resume', 'user', p_user_id, NULL, jsonb_build_object('company_id', v_company_id, 'provider_id', v_provider_id));
  RETURN jsonb_build_object('ok', true);
END; $$;


--
-- Name: lp_user_suspend(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.lp_user_suspend(p_user_id uuid, p_reason text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE v_company_id uuid; v_provider_id uuid;
BEGIN
  PERFORM private.lp_lifecycle_require_reason(p_reason);
  SELECT o_company_id, o_provider_id INTO v_company_id, v_provider_id FROM private.lp_assert_user_lifecycle_access(p_user_id);
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_user_id AND suspended_at IS NOT NULL) THEN RETURN jsonb_build_object('ok', true, 'already_suspended', true); END IF;
  UPDATE public.profiles SET suspended_at = now(), suspended_by = auth.uid(), suspended_reason = p_reason WHERE id = p_user_id;
  PERFORM private.lp_lifecycle_audit('suspend', 'user', p_user_id, p_reason, jsonb_build_object('company_id', v_company_id, 'provider_id', v_provider_id));
  RETURN jsonb_build_object('ok', true);
END; $$;
...
--
-- Name: FUNCTION project_profile_scope_from_memberships(p_user_id uuid); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.project_profile_scope_from_memberships(p_user_id uuid) IS 'Returns compatibility profile.company_id/location_id projected from active memberships. Memberships remain authoritative.';
...
--
-- Name: FUNCTION recompute_profile_legacy_scope(p_user_id uuid); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.recompute_profile_legacy_scope(p_user_id uuid) IS 'Recomputes legacy profile scope as a projection from memberships. Used for compatibility only.';
...
CREATE TABLE public.agreements (
    ...
    CONSTRAINT agreements_billing_cycle_check CHECK ((billing_cycle = ANY (ARRAY['monthly'::text, 'biweekly'::text])))
);
...
--
-- Name: agreement_invoice_lines; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.agreement_invoice_lines (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    invoice_id uuid NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT agreement_invoice_lines_line_amount_check CHECK ((line_amount >= (0)::numeric)),
    CONSTRAINT agreement_invoice_lines_product_key_check CHECK ((product_key = ANY (ARRAY['BASIS'::text, 'LUXUS'::text, 'ENTERPRISE'::text, 'CUSTOM'::text]))),
    CONSTRAINT agreement_invoice_lines_quantity_check CHECK ((quantity > 0)),
    CONSTRAINT agreement_invoice_lines_unit_price_check CHECK ((unit_price >= (0)::numeric)),
    CONSTRAINT agreement_invoice_lines_vat_amount_check CHECK ((vat_amount >= (0)::numeric)),
    CONSTRAINT agreement_invoice_lines_vat_rate_check CHECK (((vat_rate >= (0)::numeric) AND (vat_rate <= (1)::numeric)))
);


--
-- Name: agreement_invoices; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.agreement_invoices (
    ...
    amount_tax numeric(12,2) DEFAULT 0 NOT NULL,
    amount_total numeric(12,2) DEFAULT 0 NOT NULL,
    tripletex_vat_code text DEFAULT 'MVA_15'::text NOT NULL,
    ...
    CONSTRAINT agreement_invoices_billing_cycle_check CHECK ((billing_cycle = ANY (ARRAY['monthly'::text, 'biweekly'::text]))),
    CONSTRAINT agreement_invoices_status_check CHECK ((status = ANY (ARRAY['DRAFT'::text, 'PENDING_SYNC'::text, 'SENT'::text, 'PAID'::text, 'OVERDUE'::text, 'SYNC_FAILED'::text, 'VOID'::text])))
);
...
CREATE TABLE public.billing_products (
    ...
    tripletex_product_id text,
    revenue_account text,
    tax_code_id text NOT NULL,
    unit text DEFAULT 'stk'::text NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT billing_products_tier_check CHECK ((tier = ANY (ARRAY['BASIS'::text, 'LUXUS'::text, 'ENTERPRISE'::text])))
);


--
-- Name: billing_tax_codes; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.billing_tax_codes (
    id text NOT NULL,
    rate numeric(6,4) NOT NULL,
    tripletex_vat_code text,
    description text NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT billing_tax_codes_rate_check CHECK (((rate >= (0)::numeric) AND (rate <= (1)::numeric)))
);
...
CREATE TABLE public.companies (
    ...
    billing_city text,
    ehf_enabled boolean DEFAULT false NOT NULL,
    ehf_endpoint text,
    billing_country text DEFAULT 'NO'::text NOT NULL,
...
--
-- Name: TABLE day_choices; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.day_choices IS 'Canonical employee meal choice per date. Kitchen excludes rows where latest choice status is CANCELLED.';
...
--
-- Name: VIEW employee_order_items; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.employee_order_items IS 'Non-price projection of order_lines for employee-safe reads.';
...
--
-- Name: VIEW employee_orders; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.employee_orders IS 'Non-price projection of orders for employee-safe reads (complement to app-side pickOrderColumns).';
...
--
-- Name: invoice_periods; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.invoice_periods (
    ...
    count_basis integer DEFAULT 0 NOT NULL,
    count_luxus integer DEFAULT 0 NOT NULL,
    ...
    CONSTRAINT invoice_periods_period_ck CHECK ((period ~ '^\d{4}-(0[1-9]|1[0-2])$'::text)),
    CONSTRAINT invoice_periods_status_ck CHECK ((status = ANY (ARRAY['PENDING'::text, 'READY'::text, 'SENT'::text, 'FAILED'::text, 'FAILED_PERMANENT'::text]))),
    CONSTRAINT invoice_periods_total_ck CHECK ((total >= (0)::numeric)),
    CONSTRAINT invoice_periods_unique_ref_match_ck CHECK ((unique_ref = (((company_id)::text || ':'::text) || period))),
    CONSTRAINT invoice_periods_unit_price_basis_ck CHECK ((unit_price_basis > (0)::numeric)),
    CONSTRAINT invoice_periods_unit_price_luxus_ck CHECK ((unit_price_luxus > (0)::numeric))
);
...
--
-- Name: TABLE kitchen_batches; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.kitchen_batches IS 'Canonical kitchen batch status shared by kitchen and driver for date/window/location.';
...
--
-- Name: VIEW kitchen_batch; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.kitchen_batch IS 'Compatibility alias for legacy singular kitchen_batch code paths. Writes target public.kitchen_batches.';
...
COMMENT ON COLUMN public.profiles.company_id IS 'Legacy compatibility scope. Membership tables are authoritative for access control.';
...
COMMENT ON COLUMN public.profiles.location_id IS 'Legacy compatibility scope. Membership tables are authoritative for access control.';
...
--
-- Name: lifecycle_audit_log; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.lifecycle_audit_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    actor_id uuid,
    action text NOT NULL,
    entity_type text NOT NULL,
...
--
-- Name: TABLE lifecycle_audit_log; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.provider_invoices (
    ...
    metadata jsonb DEFAULT '{}'::jsonb NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT provider_invoices_status_check CHECK ((status = ANY (ARRAY['DRAFT'::text, 'SENT'::text, 'PAID'::text, 'OVERDUE'::text, 'VOID'::text])))
);
...
--
-- Name: TABLE provider_memberships; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.provider_service_areas (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    provider_id uuid NOT NULL,
    country text DEFAULT 'NO'::text NOT NULL,
    city text NOT NULL,
    postal_code_from text NOT NULL,
    postal_code_to text NOT NULL,
    min_employees integer DEFAULT 20,
    max_employees integer,
    available_days text[] DEFAULT ARRAY['mon'::text, 'tue'::text, 'wed'::text, 'thu'::text, 'fri'::text],
    active boolean DEFAULT true NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL
);
...
--
-- Name: TABLE provider_service_areas; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.provider_subscriptions (
    ...
    currency text DEFAULT 'NOK'::text NOT NULL,
    tax_code_id text DEFAULT 'MVA_25'::text NOT NULL,
    ...
    created_by uuid,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT provider_subscriptions_monthly_amount_check CHECK ((monthly_amount >= (0)::numeric)),
    CONSTRAINT provider_subscriptions_plan_check CHECK ((plan = ANY (ARRAY['SAAS_FIXED'::text, 'SAAS_PER_COMPANY'::text, 'CUSTOM'::text]))),
    CONSTRAINT provider_subscriptions_status_check CHECK ((status = ANY (ARRAY['ACTIVE'::text, 'PAUSED'::text, 'CANCELLED'::text])))
);


--
-- Name: provider_tripletex_products; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.provider_tripletex_products (
    ...
    env text DEFAULT 'prod'::text NOT NULL,
    synced_at timestamp with time zone DEFAULT now() NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT provider_tripletex_products_env_check CHECK ((env = ANY (ARRAY['test'::text, 'prod'::text]))),
    CONSTRAINT provider_tripletex_products_tier_check CHECK ((tier = ANY (ARRAY['BASIS'::text, 'LUXUS'::text, 'ENTERPRISE'::text])))
);


--
-- Name: TABLE provider_tripletex_products; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.provider_tripletex_products IS 'Per-provider Tripletex product + VAT mapping for meal tiers (Flow B). TPT-B-2.';


--
-- Name: provider_tripletex_webhook_secrets; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.provider_tripletex_webhook_secrets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    provider_id uuid NOT NULL,
    env text NOT NULL,
    ...
    CONSTRAINT provider_tripletex_webhook_secrets_env_check CHECK ((env = ANY (ARRAY['test'::text, 'prod'::text])))
);


--
-- Name: TABLE provider_tripletex_webhook_secrets; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.provider_tripletex_webhook_secrets IS 'Per-provider Tripletex webhook auth secret (Vault). TPT-B-6.';


--
-- Name: provider_tripletex_webhook_subscriptions; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.provider_tripletex_webhook_subscriptions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    provider_id uuid NOT NULL,
    env text NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT provider_tripletex_webhook_subscriptions_env_check CHECK ((env = ANY (ARRAY['test'::text, 'prod'::text])))
);


--
-- Name: TABLE provider_tripletex_webhook_subscriptions; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.provider_tripletex_webhook_subscriptions IS 'Mirrors Tripletex /event/subscription rows per provider for dashboard and disconnect cleanup (TPT-B-7 polish-9).';
...
CREATE TABLE public.providers (
    ...
    primary_color text,
    description text,
    billing_model text DEFAULT 'SAAS_FIXED'::text NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    suspended_at timestamp with time zone,
    suspended_by uuid,
...
CREATE TABLE public.tripletex_customers (
    ...
    ehf_enabled boolean DEFAULT false NOT NULL,
    provider_id uuid,
    CONSTRAINT tripletex_customers_scope_check CHECK (((company_id IS NOT NULL) OR (provider_id IS NOT NULL)))
);


--
-- Name: CONSTRAINT tripletex_customers_scope_check ON tripletex_customers; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON CONSTRAINT tripletex_customers_scope_check ON public.tripletex_customers IS 'Flow A Lp: company_id XOR provider_id. Flow B: both set (company customer in provider Tripletex).';


--
-- Name: tripletex_exports; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.tripletex_exports (
    unique_ref text NOT NULL,
...
--
-- Name: tripletex_webhook_events; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.tripletex_webhook_events (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    provider_id uuid NOT NULL,
    env text NOT NULL,
    ...
    status text DEFAULT 'PENDING'::text NOT NULL,
    error_detail text,
    CONSTRAINT tripletex_webhook_events_env_check CHECK ((env = ANY (ARRAY['test'::text, 'prod'::text]))),
    CONSTRAINT tripletex_webhook_events_status_check CHECK ((status = ANY (ARRAY['PENDING'::text, 'PROCESSED'::text, 'IGNORED'::text, 'FAILED'::text])))
);


--
-- Name: TABLE tripletex_webhook_events; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.tripletex_webhook_events IS 'Inbound Tripletex webhooks per provider (Flow B). TPT-B-6.';
...
CREATE TABLE public.webhook_events (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    source text DEFAULT 'tripletex'::text NOT NULL,
    ...
    status text DEFAULT 'PENDING'::text NOT NULL,
    error_detail text,
    CONSTRAINT webhook_events_status_check CHECK ((status = ANY (ARRAY['PENDING'::text, 'PROCESSED'::text, 'FAILED'::text, 'IGNORED'::text])))
);
...
COMMENT ON TABLE public.webhook_events IS 'Inbound integration webhooks (Tripletex Flow A). event_id is idempotency key.';
...
--
-- Name: agreement_invoice_lines agreement_invoice_lines_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoice_lines
    ADD CONSTRAINT agreement_invoice_lines_pkey PRIMARY KEY (id);


--
-- Name: agreement_invoices agreement_invoices_agreement_id_invoice_period_start_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoices
    ADD CONSTRAINT agreement_invoices_agreement_id_invoice_period_start_key UNIQUE (agreement_id, invoice_period_start);


--
-- Name: agreement_invoices agreement_invoices_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoices
    ADD CONSTRAINT agreement_invoices_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.billing_products
    ADD CONSTRAINT billing_products_pkey PRIMARY KEY (tier);


--
-- Name: billing_tax_codes billing_tax_codes_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.billing_tax_codes
    ADD CONSTRAINT billing_tax_codes_pkey PRIMARY KEY (id);
...
--
-- Name: invoice_periods invoice_periods_company_period_uniq; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.invoice_periods
    ADD CONSTRAINT invoice_periods_company_period_uniq UNIQUE (company_id, period);


--
-- Name: invoice_periods invoice_periods_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: invoice_periods invoice_periods_unique_ref_uniq; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.invoice_periods
    ADD CONSTRAINT invoice_periods_unique_ref_uniq UNIQUE (unique_ref);
...
--
-- Name: lifecycle_audit_log lifecycle_audit_log_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.lifecycle_audit_log
    ADD CONSTRAINT lifecycle_audit_log_pkey PRIMARY KEY (id);
...
--
-- Name: provider_invoices provider_invoices_invoice_number_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: provider_invoices provider_invoices_provider_id_invoice_period_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_invoices
    ADD CONSTRAINT provider_invoices_provider_id_invoice_period_key UNIQUE (provider_id, invoice_period);
...
--
-- Name: provider_memberships provider_memberships_user_id_provider_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_memberships
    ADD CONSTRAINT provider_memberships_user_id_provider_id_key UNIQUE (user_id, provider_id);
...
--
-- Name: provider_subscriptions provider_subscriptions_provider_id_active_to_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_subscriptions
    ADD CONSTRAINT provider_subscriptions_provider_id_active_to_key UNIQUE (provider_id, active_to);


--
-- Name: provider_tripletex_credentials provider_tripletex_credentials_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_credentials
    ADD CONSTRAINT provider_tripletex_credentials_pkey PRIMARY KEY (id);


--
-- Name: provider_tripletex_credentials provider_tripletex_credentials_provider_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_credentials
    ADD CONSTRAINT provider_tripletex_credentials_provider_id_key UNIQUE (provider_id);


--
-- Name: provider_tripletex_products provider_tripletex_products_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_products
    ADD CONSTRAINT provider_tripletex_products_pkey PRIMARY KEY (id);


--
-- Name: provider_tripletex_products provider_tripletex_products_provider_id_tier_env_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_products
    ADD CONSTRAINT provider_tripletex_products_provider_id_tier_env_key UNIQUE (provider_id, tier, env);


--
-- Name: provider_tripletex_webhook_secrets provider_tripletex_webhook_secrets_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_secrets
    ADD CONSTRAINT provider_tripletex_webhook_secrets_pkey PRIMARY KEY (id);


--
-- Name: provider_tripletex_webhook_secrets provider_tripletex_webhook_secrets_provider_id_env_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_secrets
    ADD CONSTRAINT provider_tripletex_webhook_secrets_provider_id_env_key UNIQUE (provider_id, env);


--
-- Name: provider_tripletex_webhook_subscriptions provider_tripletex_webhook_subscriptions_env_event_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_subscriptions
    ADD CONSTRAINT provider_tripletex_webhook_subscriptions_env_event_unique UNIQUE (provider_id, env, event_type);


--
-- Name: provider_tripletex_webhook_subscriptions provider_tripletex_webhook_subscriptions_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_subscriptions
    ADD CONSTRAINT provider_tripletex_webhook_subscriptions_pkey PRIMARY KEY (id);
...
--
-- Name: providers providers_org_number_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.providers
    ADD CONSTRAINT providers_org_number_key UNIQUE (org_number);
...
--
-- Name: tripletex_exports tripletex_exports_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_exports
    ADD CONSTRAINT tripletex_exports_pkey PRIMARY KEY (unique_ref);
...
--
-- Name: tripletex_webhook_events tripletex_webhook_events_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_webhook_events
    ADD CONSTRAINT tripletex_webhook_events_pkey PRIMARY KEY (id);


--
-- Name: tripletex_webhook_events tripletex_webhook_events_provider_id_env_tripletex_event_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_webhook_events
    ADD CONSTRAINT tripletex_webhook_events_provider_id_env_tripletex_event_id_key UNIQUE (provider_id, env, tripletex_event_id);
...
--
-- Name: agreements_billing_scheduler_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX agreements_billing_scheduler_idx ON public.agreements USING btree (status, billing_cycle, billing_anchor_date) WHERE (status = 'ACTIVE'::public.agreement_status);
...
--
-- Name: idx_agreement_invoice_lines_invoice; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreement_invoice_lines_invoice ON public.agreement_invoice_lines USING btree (invoice_id);


--
-- Name: idx_agreement_invoices_company; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreement_invoices_company ON public.agreement_invoices USING btree (company_id);


--
-- Name: idx_agreement_invoices_period; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreement_invoices_period ON public.agreement_invoices USING btree (invoice_period_start, invoice_period_end);


--
-- Name: idx_agreement_invoices_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreement_invoices_provider ON public.agreement_invoices USING btree (provider_id);


--
-- Name: idx_agreement_invoices_provider_tripletex; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreement_invoices_provider_tripletex ON public.agreement_invoices USING btree (provider_id, tripletex_invoice_id) WHERE (tripletex_invoice_id IS NOT NULL);


--
-- Name: idx_agreements_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_agreements_provider ON public.agreements USING btree (provider_id);
...
--
-- Name: idx_companies_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_companies_provider ON public.companies USING btree (provider_id);
...
--
-- Name: idx_company_registrations_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_company_registrations_provider ON public.company_registrations USING btree (provider_id);
...
--
-- Name: idx_lifecycle_audit_log_actor; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lifecycle_audit_log_actor ON public.lifecycle_audit_log USING btree (actor_id);


--
-- Name: idx_lifecycle_audit_log_created; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lifecycle_audit_log_created ON public.lifecycle_audit_log USING btree (created_at DESC);


--
-- Name: idx_lifecycle_audit_log_entity; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lifecycle_audit_log_entity ON public.lifecycle_audit_log USING btree (entity_type, entity_id);
...
--
-- Name: idx_menu_service_days_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_menu_service_days_provider ON public.menu_service_days USING btree (provider_id);
...
--
-- Name: idx_orders_provider; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_provider_subscriptions_active; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_provider_subscriptions_active ON public.provider_subscriptions USING btree (provider_id) WHERE (active_to IS NULL);


--
-- Name: idx_provider_tripletex_credentials_env; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_credentials_env ON public.provider_tripletex_credentials USING btree (env);


--
-- Name: idx_provider_tripletex_credentials_health; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_credentials_health ON public.provider_tripletex_credentials USING btree (connection_state, health_check_at) WHERE (connection_state = ANY (ARRAY['CONNECTED'::text, 'DEGRADED'::text]));


--
-- Name: idx_provider_tripletex_credentials_purge; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_credentials_purge ON public.provider_tripletex_credentials USING btree (vault_purge_at) WHERE (connection_state = 'DISCONNECTED'::text);


--
-- Name: idx_provider_tripletex_credentials_rotation_due; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_credentials_rotation_due ON public.provider_tripletex_credentials USING btree (rotation_due) WHERE (rotation_due IS NOT NULL);


--
-- Name: idx_provider_tripletex_products_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_products_provider ON public.provider_tripletex_products USING btree (provider_id);


--
-- Name: idx_provider_tripletex_webhook_secrets_provider; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_webhook_secrets_provider ON public.provider_tripletex_webhook_secrets USING btree (provider_id);


--
-- Name: idx_provider_tripletex_webhook_subscriptions_provider_env; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_provider_tripletex_webhook_subscriptions_provider_env ON public.provider_tripletex_webhook_subscriptions USING btree (provider_id, env) WHERE (active = true);


--
-- Name: idx_providers_slug; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_providers_slug ON public.providers USING btree (slug) WHERE (deleted_at IS NULL);
...
CREATE INDEX idx_providers_status ON public.providers USING btree (status) WHERE (deleted_at IS NULL);


--
-- Name: idx_service_areas_active; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_service_areas_active ON public.provider_service_areas USING btree (provider_id) WHERE (active = true);


--
-- Name: idx_service_areas_postal; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_service_areas_postal ON public.provider_service_areas USING btree (postal_code_from, postal_code_to);
...
--
-- Name: idx_tripletex_webhook_events_provider_received; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_tripletex_webhook_events_provider_received ON public.tripletex_webhook_events USING btree (provider_id, received_at DESC);
...
--
-- Name: invoice_periods_period_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX invoice_periods_period_idx ON public.invoice_periods USING btree (period);


--
-- Name: invoice_periods_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX invoice_periods_status_idx ON public.invoice_periods USING btree (status);
...
--
-- Name: tripletex_customers_company_lp_only; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX tripletex_customers_company_lp_only ON public.tripletex_customers USING btree (company_id) WHERE ((provider_id IS NULL) AND (company_id IS NOT NULL));


--
-- Name: tripletex_customers_provider_company; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX tripletex_customers_provider_company ON public.tripletex_customers USING btree (provider_id, company_id) WHERE ((company_id IS NOT NULL) AND (provider_id IS NOT NULL));


--
-- Name: tripletex_customers_provider_lp_only; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX tripletex_customers_provider_lp_only ON public.tripletex_customers USING btree (provider_id) WHERE ((company_id IS NULL) AND (provider_id IS NOT NULL));


--
-- Name: tripletex_customers_tripletex_customer_id_uniq; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX tripletex_customers_tripletex_customer_id_uniq ON public.tripletex_customers USING btree (tripletex_customer_id);
...
--
-- Name: agreements trg_agreement_lifecycle_hook; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_agreement_lifecycle_hook AFTER INSERT OR UPDATE OF status, tier ON public.agreements FOR EACH ROW EXECUTE FUNCTION public.lp_agreement_lifecycle_hook();
...
--
-- Name: provider_tripletex_credentials trg_provider_tripletex_credentials_vault_cleanup; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_provider_tripletex_credentials_vault_cleanup BEFORE DELETE ON public.provider_tripletex_credentials FOR EACH ROW EXECUTE FUNCTION private.lp_provider_tripletex_credentials_vault_cleanup();


--
-- Name: provider_tripletex_webhook_secrets trg_provider_tripletex_webhook_secrets_vault_cleanup; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_provider_tripletex_webhook_secrets_vault_cleanup BEFORE DELETE ON public.provider_tripletex_webhook_secrets FOR EACH ROW EXECUTE FUNCTION private.lp_provider_webhook_secret_vault_cleanup();
...
--
-- Name: agreement_invoice_lines agreement_invoice_lines_invoice_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoice_lines
    ADD CONSTRAINT agreement_invoice_lines_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES public.agreement_invoices(id) ON DELETE CASCADE;


--
-- Name: agreement_invoice_lines agreement_invoice_lines_tax_code_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoice_lines
    ADD CONSTRAINT agreement_invoice_lines_tax_code_id_fkey FOREIGN KEY (tax_code_id) REFERENCES public.billing_tax_codes(id) ON UPDATE CASCADE ON DELETE RESTRICT;


--
-- Name: agreement_invoices agreement_invoices_agreement_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoices
    ADD CONSTRAINT agreement_invoices_agreement_id_fkey FOREIGN KEY (agreement_id) REFERENCES public.agreements(id) ON DELETE RESTRICT;


--
-- Name: agreement_invoices agreement_invoices_company_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoices
    ADD CONSTRAINT agreement_invoices_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.companies(id) ON DELETE RESTRICT;


--
-- Name: agreement_invoices agreement_invoices_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreement_invoices
    ADD CONSTRAINT agreement_invoices_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE RESTRICT;
...
--
-- Name: agreements agreements_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agreements
    ADD CONSTRAINT agreements_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id);
...
--
-- Name: billing_products billing_products_tax_code_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.billing_products
    ADD CONSTRAINT billing_products_tax_code_id_fkey FOREIGN KEY (tax_code_id) REFERENCES public.billing_tax_codes(id) ON UPDATE CASCADE ON DELETE RESTRICT;
...
--
-- Name: companies companies_paused_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.companies
    ADD CONSTRAINT companies_paused_by_fkey FOREIGN KEY (paused_by) REFERENCES public.profiles(id);


--
-- Name: companies companies_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.companies
    ADD CONSTRAINT companies_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id);


--
-- Name: companies companies_suspended_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.companies
    ADD CONSTRAINT companies_suspended_by_fkey FOREIGN KEY (suspended_by) REFERENCES public.profiles(id);
...
--
-- Name: company_registrations company_registrations_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.company_registrations
    ADD CONSTRAINT company_registrations_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id);
...
--
-- Name: invoice_periods invoice_periods_company_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.invoice_periods
    ADD CONSTRAINT invoice_periods_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.companies(id) ON DELETE CASCADE;
...
--
-- Name: lifecycle_audit_log lifecycle_audit_log_actor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.lifecycle_audit_log
    ADD CONSTRAINT lifecycle_audit_log_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES public.profiles(id);
...
--
-- Name: menu_service_days menu_service_days_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.menu_service_days
    ADD CONSTRAINT menu_service_days_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id);
...
ALTER TABLE ONLY public.orders
    ADD CONSTRAINT orders_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id);
...
ALTER TABLE ONLY public.profiles
    ADD CONSTRAINT profiles_suspended_by_fkey FOREIGN KEY (suspended_by) REFERENCES public.profiles(id);


--
-- Name: provider_invoices provider_invoices_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_invoices
    ADD CONSTRAINT provider_invoices_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_invoices provider_invoices_subscription_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_invoices
    ADD CONSTRAINT provider_invoices_subscription_id_fkey FOREIGN KEY (subscription_id) REFERENCES public.provider_subscriptions(id) ON DELETE RESTRICT;


--
-- Name: provider_invoices provider_invoices_tax_code_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_invoices
    ADD CONSTRAINT provider_invoices_tax_code_id_fkey FOREIGN KEY (tax_code_id) REFERENCES public.billing_tax_codes(id) ON UPDATE CASCADE ON DELETE RESTRICT;


--
-- Name: provider_memberships provider_memberships_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_memberships
    ADD CONSTRAINT provider_memberships_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_memberships provider_memberships_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_memberships
    ADD CONSTRAINT provider_memberships_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
--
-- Name: provider_subscriptions provider_subscriptions_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_subscriptions
    ADD CONSTRAINT provider_subscriptions_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id);
...
ALTER TABLE ONLY public.provider_subscriptions
    ADD CONSTRAINT provider_subscriptions_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_subscriptions provider_subscriptions_tax_code_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_subscriptions
    ADD CONSTRAINT provider_subscriptions_tax_code_id_fkey FOREIGN KEY (tax_code_id) REFERENCES public.billing_tax_codes(id) ON UPDATE CASCADE ON DELETE RESTRICT;


--
-- Name: provider_tripletex_credentials provider_tripletex_credentials_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_credentials
    ADD CONSTRAINT provider_tripletex_credentials_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_tripletex_products provider_tripletex_products_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_products
    ADD CONSTRAINT provider_tripletex_products_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_tripletex_webhook_secrets provider_tripletex_webhook_secrets_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_secrets
    ADD CONSTRAINT provider_tripletex_webhook_secrets_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: provider_tripletex_webhook_subscriptions provider_tripletex_webhook_subscriptions_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.provider_tripletex_webhook_subscriptions
    ADD CONSTRAINT provider_tripletex_webhook_subscriptions_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;


--
-- Name: providers providers_paused_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.providers
    ADD CONSTRAINT providers_paused_by_fkey FOREIGN KEY (paused_by) REFERENCES public.profiles(id);


--
-- Name: providers providers_suspended_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.providers
    ADD CONSTRAINT providers_suspended_by_fkey FOREIGN KEY (suspended_by) REFERENCES public.profiles(id);
...
--
-- Name: tripletex_customers tripletex_customers_company_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_customers
    ADD CONSTRAINT tripletex_customers_company_fk FOREIGN KEY (company_id) REFERENCES public.companies(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: tripletex_customers tripletex_customers_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_customers
    ADD CONSTRAINT tripletex_customers_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;
...
--
-- Name: tripletex_webhook_events tripletex_webhook_events_provider_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tripletex_webhook_events
    ADD CONSTRAINT tripletex_webhook_events_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.providers(id) ON DELETE CASCADE;
...
--
-- Name: agreement_invoice_lines; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.agreement_invoice_lines ENABLE ROW LEVEL SECURITY;

--
-- Name: agreement_invoice_lines agreement_invoice_lines_company_admin_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoice_lines_company_admin_select ON public.agreement_invoice_lines FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM (public.agreement_invoices ai
     JOIN public.profiles p ON ((p.id = auth.uid())))
  WHERE ((ai.id = agreement_invoice_lines.invoice_id) AND (p.role = 'company_admin'::public.user_role) AND (p.company_id = ai.company_id)))));


--
-- Name: agreement_invoice_lines agreement_invoice_lines_provider_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoice_lines_provider_select ON public.agreement_invoice_lines FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.agreement_invoices ai
  WHERE ((ai.id = agreement_invoice_lines.invoice_id) AND public.can_access_provider(ai.provider_id)))));


--
-- Name: agreement_invoice_lines agreement_invoice_lines_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoice_lines_superadmin_all ON public.agreement_invoice_lines TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());


--
-- Name: agreement_invoices; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.agreement_invoices ENABLE ROW LEVEL SECURITY;

--
-- Name: agreement_invoices agreement_invoices_company_admin_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoices_company_admin_select ON public.agreement_invoices FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.profiles p
  WHERE ((p.id = auth.uid()) AND (p.role = 'company_admin'::public.user_role) AND (p.company_id = agreement_invoices.company_id)))));


--
-- Name: agreement_invoices agreement_invoices_provider_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoices_provider_select ON public.agreement_invoices FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: agreement_invoices agreement_invoices_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreement_invoices_superadmin_all ON public.agreement_invoices TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
--
-- Name: agreements agreements_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreements_select_provider_scope ON public.agreements FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));
...
--
-- Name: agreements agreements_update_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY agreements_update_provider_scope ON public.agreements FOR UPDATE TO authenticated USING (public.can_access_provider(provider_id)) WITH CHECK (public.can_access_provider(provider_id));
...
--
-- Name: billing_products billing_products_authenticated_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY billing_products_authenticated_select ON public.billing_products FOR SELECT TO authenticated USING (true);


--
-- Name: billing_tax_codes; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.billing_tax_codes ENABLE ROW LEVEL SECURITY;

--
-- Name: billing_tax_codes billing_tax_codes_authenticated_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY billing_tax_codes_authenticated_select ON public.billing_tax_codes FOR SELECT TO authenticated USING (true);
...
--
-- Name: companies companies_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY companies_select_provider_scope ON public.companies FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));
...
--
-- Name: companies companies_update_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY companies_update_provider_scope ON public.companies FOR UPDATE TO authenticated USING (public.can_access_provider(provider_id)) WITH CHECK (public.can_access_provider(provider_id));
...
--
-- Name: company_deletions; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.company_deletions ENABLE ROW LEVEL SECURITY;

--
-- Name: company_deletions company_deletions_superadmin_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY company_deletions_superadmin_select ON public.company_deletions FOR SELECT TO authenticated USING (public.is_platform_admin());
...
--
-- Name: company_registrations company_registrations_anon_insert; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY company_registrations_anon_insert ON public.company_registrations FOR INSERT TO authenticated, anon WITH CHECK (((status = 'PENDING'::text) AND (company_id IS NULL) AND (length(btrim(COALESCE(company_name, ''::text))) > 0) AND (length(btrim(COALESCE(contact_email, ''::text))) > 0) AND (requested_postal_code IS NOT NULL) AND (length(regexp_replace(btrim(COALESCE(requested_postal_code, ''::text)), '\D'::text, ''::text, 'g'::text)) = 4)));


--
-- Name: company_registrations company_registrations_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY company_registrations_select_provider_scope ON public.company_registrations FOR SELECT TO authenticated USING (((provider_id IS NOT NULL) AND public.can_access_provider(provider_id)));
...
--
-- Name: company_registrations company_registrations_update_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY company_registrations_update_provider_scope ON public.company_registrations FOR UPDATE TO authenticated USING (((provider_id IS NOT NULL) AND public.can_access_provider(provider_id))) WITH CHECK (((provider_id IS NOT NULL) AND public.can_access_provider(provider_id)));
...
--
-- Name: invoice_periods; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.invoice_periods ENABLE ROW LEVEL SECURITY;

--
-- Name: invoice_periods invoice_periods_tenant_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY invoice_periods_tenant_select ON public.invoice_periods FOR SELECT TO authenticated USING ((public.is_platform_admin() OR public.can_access_company(company_id)));
...
--
-- Name: provider_invoices invoices_provider_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY invoices_provider_select ON public.provider_invoices FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));
...
--
-- Name: provider_invoices invoices_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY invoices_superadmin_all ON public.provider_invoices TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
--
-- Name: lifecycle_audit_log; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.lifecycle_audit_log ENABLE ROW LEVEL SECURITY;

--
-- Name: lifecycle_audit_log lifecycle_audit_log_insert_via_rpc; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY lifecycle_audit_log_insert_via_rpc ON public.lifecycle_audit_log FOR INSERT TO authenticated WITH CHECK (((actor_id = auth.uid()) OR public.is_platform_admin()));


--
-- Name: lifecycle_audit_log lifecycle_audit_log_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY lifecycle_audit_log_select_provider_scope ON public.lifecycle_audit_log FOR SELECT USING ((((entity_type = 'provider'::text) AND public.can_access_provider(entity_id)) OR ((entity_type = 'company'::text) AND (EXISTS ( SELECT 1
   FROM public.companies c
  WHERE ((c.id = lifecycle_audit_log.entity_id) AND public.can_access_provider(c.provider_id))))) OR ((entity_type = 'user'::text) AND (EXISTS ( SELECT 1
   FROM (public.company_memberships cm
     JOIN public.companies c ON ((c.id = cm.company_id)))
  WHERE ((cm.user_id = lifecycle_audit_log.entity_id) AND public.can_access_provider(c.provider_id)))))));


--
-- Name: lifecycle_audit_log lifecycle_audit_log_superadmin_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY lifecycle_audit_log_superadmin_select ON public.lifecycle_audit_log FOR SELECT USING (public.is_platform_admin());
...
--
-- Name: menu_service_days menu_service_days_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY menu_service_days_select_provider_scope ON public.menu_service_days FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: menu_service_days menu_service_days_update_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY menu_service_days_update_provider_scope ON public.menu_service_days FOR UPDATE TO authenticated USING (public.can_access_provider(provider_id)) WITH CHECK (public.can_access_provider(provider_id));
...
--
-- Name: orders orders_select_provider_scope; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY orders_select_provider_scope ON public.orders FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));
...
--
-- Name: provider_invoices; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_invoices ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_memberships; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_memberships ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_memberships provider_memberships_delete_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_memberships_delete_admin ON public.provider_memberships FOR DELETE TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = provider_memberships.provider_id) AND (pm.role = 'provider_admin'::public.provider_role)))));


--
-- Name: provider_memberships provider_memberships_insert_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_memberships_insert_admin ON public.provider_memberships FOR INSERT TO authenticated WITH CHECK ((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = provider_memberships.provider_id) AND (pm.role = 'provider_admin'::public.provider_role)))));


--
-- Name: provider_memberships provider_memberships_select_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_memberships_select_admin ON public.provider_memberships FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: provider_memberships provider_memberships_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_memberships_superadmin_all ON public.provider_memberships USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
--
-- Name: provider_service_areas provider_service_areas_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_service_areas_superadmin_all ON public.provider_service_areas USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
--
-- Name: provider_tripletex_credentials; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_tripletex_credentials ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_tripletex_credentials provider_tripletex_credentials_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_credentials_superadmin_all ON public.provider_tripletex_credentials TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());


--
-- Name: provider_tripletex_products; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_tripletex_products ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_tripletex_products provider_tripletex_products_provider_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_products_provider_select ON public.provider_tripletex_products FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: provider_tripletex_products provider_tripletex_products_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_products_superadmin_all ON public.provider_tripletex_products TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());


--
-- Name: provider_tripletex_webhook_secrets; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_tripletex_webhook_secrets ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_tripletex_webhook_secrets provider_tripletex_webhook_secrets_provider_admin_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_webhook_secrets_provider_admin_select ON public.provider_tripletex_webhook_secrets FOR SELECT USING (public.can_access_provider(provider_id));


--
-- Name: provider_tripletex_webhook_secrets provider_tripletex_webhook_secrets_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_webhook_secrets_superadmin_all ON public.provider_tripletex_webhook_secrets USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());


--
-- Name: provider_tripletex_webhook_subscriptions; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.provider_tripletex_webhook_subscriptions ENABLE ROW LEVEL SECURITY;

--
-- Name: provider_tripletex_webhook_subscriptions provider_tripletex_webhook_subscriptions_provider_read; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_webhook_subscriptions_provider_read ON public.provider_tripletex_webhook_subscriptions FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: provider_tripletex_webhook_subscriptions provider_tripletex_webhook_subscriptions_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY provider_tripletex_webhook_subscriptions_superadmin_all ON public.provider_tripletex_webhook_subscriptions TO authenticated USING (public.is_superadmin()) WITH CHECK (public.is_superadmin());
...
--
-- Name: providers providers_select_member; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY providers_select_member ON public.providers FOR SELECT TO authenticated USING (public.can_access_provider(id));


--
-- Name: providers providers_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY providers_superadmin_all ON public.providers USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());


--
-- Name: providers providers_update_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY providers_update_admin ON public.providers FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = providers.id) AND (pm.role = 'provider_admin'::public.provider_role))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = providers.id) AND (pm.role = 'provider_admin'::public.provider_role)))));
...
--
-- Name: provider_service_areas service_areas_insert_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY service_areas_insert_admin ON public.provider_service_areas FOR INSERT TO authenticated WITH CHECK (((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = provider_service_areas.provider_id) AND (pm.role = 'provider_admin'::public.provider_role)))) OR public.is_platform_admin()));


--
-- Name: provider_service_areas service_areas_select_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY service_areas_select_admin ON public.provider_service_areas FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: provider_service_areas service_areas_update_admin; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY service_areas_update_admin ON public.provider_service_areas FOR UPDATE TO authenticated USING (((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = provider_service_areas.provider_id) AND (pm.role = 'provider_admin'::public.provider_role)))) OR public.is_platform_admin())) WITH CHECK (((EXISTS ( SELECT 1
   FROM public.provider_memberships pm
  WHERE ((pm.user_id = auth.uid()) AND (pm.provider_id = provider_service_areas.provider_id) AND (pm.role = 'provider_admin'::public.provider_role)))) OR public.is_platform_admin()));
...
--
-- Name: provider_subscriptions subscriptions_provider_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY subscriptions_provider_select ON public.provider_subscriptions FOR SELECT TO authenticated USING (public.can_access_provider(provider_id));


--
-- Name: provider_subscriptions subscriptions_superadmin_all; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY subscriptions_superadmin_all ON public.provider_subscriptions TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());
...
--
-- Name: tripletex_exports; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.tripletex_exports ENABLE ROW LEVEL SECURITY;

--
-- Name: tripletex_exports tripletex_exports_tenant_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY tripletex_exports_tenant_select ON public.tripletex_exports FOR SELECT TO authenticated USING ((public.is_platform_admin() OR (EXISTS ( SELECT 1
   FROM public.invoice_periods ip
  WHERE ((ip.unique_ref = tripletex_exports.unique_ref) AND public.can_access_company(ip.company_id)))) OR (EXISTS ( SELECT 1
   FROM public.agreement_invoices ai
  WHERE ((tripletex_exports.unique_ref = ('lp_agreement:'::text || (ai.id)::text)) AND public.can_access_company(ai.company_id)))) OR ((unique_ref ~~ 'lp_bw:%'::text) AND public.can_access_company((split_part(unique_ref, ':'::text, 2))::uuid))));
...
--
-- Name: tripletex_webhook_events; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION can_access_company(_company_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_access_company(_company_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_access_company(_company_id uuid) TO authenticated;


--
-- Name: FUNCTION can_access_delivery_run(_delivery_run_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_access_delivery_run(_delivery_run_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_access_delivery_run(_delivery_run_id uuid) TO authenticated;


--
-- Name: FUNCTION can_access_location(_location_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_access_location(_location_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_access_location(_location_id uuid) TO authenticated;


--
-- Name: FUNCTION can_access_menu_day(_menu_service_day_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_access_menu_day(_menu_service_day_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_access_menu_day(_menu_service_day_id uuid) TO authenticated;


--
-- Name: FUNCTION can_edit_order(_order_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_edit_order(_order_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_edit_order(_order_id uuid) TO authenticated;


--
-- Name: FUNCTION can_finance_company(_company_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_finance_company(_company_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_finance_company(_company_id uuid) TO authenticated;


--
-- Name: FUNCTION can_manage_company(_company_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_manage_company(_company_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_manage_company(_company_id uuid) TO authenticated;


--
-- Name: FUNCTION can_manage_location(_location_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_manage_location(_location_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_manage_location(_location_id uuid) TO authenticated;


--
-- Name: FUNCTION can_manage_menu_day(_menu_service_day_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_manage_menu_day(_menu_service_day_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_manage_menu_day(_menu_service_day_id uuid) TO authenticated;


--
-- Name: FUNCTION can_operate_delivery_run(_delivery_run_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_operate_delivery_run(_delivery_run_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_operate_delivery_run(_delivery_run_id uuid) TO authenticated;


--
-- Name: FUNCTION can_view_order(_order_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_view_order(_order_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_view_order(_order_id uuid) TO authenticated;


--
-- Name: FUNCTION can_view_profile(_profile_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.can_view_profile(_profile_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION private.can_view_profile(_profile_id uuid) TO authenticated;


--
-- Name: FUNCTION ensure_audit_log_partitions(months_ahead integer); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.ensure_audit_log_partitions(months_ahead integer) FROM PUBLIC;


--
-- Name: FUNCTION has_platform_role(_roles public.platform_role[]); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.has_platform_role(_roles public.platform_role[]) FROM PUBLIC;
GRANT ALL ON FUNCTION private.has_platform_role(_roles public.platform_role[]) TO authenticated;
...
--
-- Name: TABLE company_registrations; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.company_registrations TO anon;
GRANT ALL ON TABLE public.company_registrations TO authenticated;
GRANT ALL ON TABLE public.company_registrations TO service_role;


--
-- Name: TABLE provider_tripletex_credentials; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.provider_tripletex_credentials TO service_role;
GRANT SELECT ON TABLE public.provider_tripletex_credentials TO authenticated;


--
-- Name: FUNCTION role_is_company_finance(_role text); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.role_is_company_finance(_role text) FROM PUBLIC;
GRANT ALL ON FUNCTION private.role_is_company_finance(_role text) TO authenticated;


--
-- Name: FUNCTION role_is_company_manager(_role text); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.role_is_company_manager(_role text) FROM PUBLIC;
GRANT ALL ON FUNCTION private.role_is_company_manager(_role text) TO authenticated;


--
-- Name: FUNCTION role_is_location_manager(_role text); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.role_is_location_manager(_role text) FROM PUBLIC;
GRANT ALL ON FUNCTION private.role_is_location_manager(_role text) TO authenticated;


--
-- Name: FUNCTION shares_company_with(_other_user_id uuid); Type: ACL; Schema: private; Owner: -
--

REVOKE ALL ON FUNCTION private.shares_company_with(_other_user_id uuid) FROM PUBLIC;


--
-- Name: FUNCTION agreement_delivery_days_array(p_agreement_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.agreement_delivery_days_array(p_agreement_id uuid) TO anon;
GRANT ALL ON FUNCTION public.agreement_delivery_days_array(p_agreement_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.agreement_delivery_days_array(p_agreement_id uuid) TO service_role;


--
-- Name: FUNCTION apply_standing_orders(_service_date date, _location_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.apply_standing_orders(_service_date date, _location_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.apply_standing_orders(_service_date date, _location_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.apply_standing_orders(_service_date date, _location_id uuid) TO service_role;


--
-- Name: FUNCTION assert_menu_day_mutable(_menu_service_day_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.assert_menu_day_mutable(_menu_service_day_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.assert_menu_day_mutable(_menu_service_day_id uuid) TO service_role;


--
-- Name: FUNCTION assert_order_mutable(_order_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.assert_order_mutable(_order_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.assert_order_mutable(_order_id uuid) TO service_role;


--
-- Name: FUNCTION audit_direct_profile_scope_write(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.audit_direct_profile_scope_write() TO anon;
GRANT ALL ON FUNCTION public.audit_direct_profile_scope_write() TO authenticated;
GRANT ALL ON FUNCTION public.audit_direct_profile_scope_write() TO service_role;


--
-- Name: FUNCTION can_access_company(company_uuid uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_access_company(company_uuid uuid) TO anon;
GRANT ALL ON FUNCTION public.can_access_company(company_uuid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_access_company(company_uuid uuid) TO service_role;


--
-- Name: FUNCTION can_access_location(location_uuid uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_access_location(location_uuid uuid) TO anon;
GRANT ALL ON FUNCTION public.can_access_location(location_uuid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_access_location(location_uuid uuid) TO service_role;


--
-- Name: FUNCTION can_access_provider(p_provider_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_access_provider(p_provider_id uuid) TO anon;
GRANT ALL ON FUNCTION public.can_access_provider(p_provider_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_access_provider(p_provider_id uuid) TO service_role;


--
-- Name: FUNCTION can_admin_company(company_uuid uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_admin_company(company_uuid uuid) TO anon;
GRANT ALL ON FUNCTION public.can_admin_company(company_uuid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_admin_company(company_uuid uuid) TO service_role;


--
-- Name: FUNCTION can_admin_location(location_uuid uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_admin_location(location_uuid uuid) TO anon;
GRANT ALL ON FUNCTION public.can_admin_location(location_uuid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_admin_location(location_uuid uuid) TO service_role;


--
-- Name: FUNCTION can_kitchen_location(location_uuid uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.can_kitchen_location(location_uuid uuid) TO anon;
GRANT ALL ON FUNCTION public.can_kitchen_location(location_uuid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.can_kitchen_location(location_uuid uuid) TO service_role;


--
-- Name: TABLE repair_jobs; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION claim_repair_jobs(p_limit integer); Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION cleanup_ai_action_memory(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.cleanup_ai_action_memory() TO anon;
GRANT ALL ON FUNCTION public.cleanup_ai_action_memory() TO authenticated;
GRANT ALL ON FUNCTION public.cleanup_ai_action_memory() TO service_role;


--
-- Name: FUNCTION compute_cutoff_at(_location_id uuid, _service_date date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.compute_cutoff_at(_location_id uuid, _service_date date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.compute_cutoff_at(_location_id uuid, _service_date date) TO service_role;


--
-- Name: FUNCTION current_profile_active(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.current_profile_active() FROM PUBLIC;
GRANT ALL ON FUNCTION public.current_profile_active() TO anon;
GRANT ALL ON FUNCTION public.current_profile_active() TO authenticated;
GRANT ALL ON FUNCTION public.current_profile_active() TO service_role;


--
-- Name: FUNCTION current_profile_company_id(); Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION current_profile_location_id(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.current_profile_location_id() FROM PUBLIC;
GRANT ALL ON FUNCTION public.current_profile_location_id() TO anon;
GRANT ALL ON FUNCTION public.current_profile_location_id() TO authenticated;
GRANT ALL ON FUNCTION public.current_profile_location_id() TO service_role;
...
--
-- Name: FUNCTION ensure_invoice_head(p_run_id uuid, p_company_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.ensure_invoice_head(p_run_id uuid, p_company_id uuid) TO anon;
GRANT ALL ON FUNCTION public.ensure_invoice_head(p_run_id uuid, p_company_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.ensure_invoice_head(p_run_id uuid, p_company_id uuid) TO service_role;


--
-- Name: FUNCTION extract_agreement_delivery_days(p_delivery_days jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.extract_agreement_delivery_days(p_delivery_days jsonb) TO anon;
GRANT ALL ON FUNCTION public.extract_agreement_delivery_days(p_delivery_days jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.extract_agreement_delivery_days(p_delivery_days jsonb) TO service_role;


--
-- Name: FUNCTION finalize_due_service_days(_as_of timestamp with time zone, _location_id uuid, _service_date date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.finalize_due_service_days(_as_of timestamp with time zone, _location_id uuid, _service_date date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.finalize_due_service_days(_as_of timestamp with time zone, _location_id uuid, _service_date date) TO authenticated;
GRANT ALL ON FUNCTION public.finalize_due_service_days(_as_of timestamp with time zone, _location_id uuid, _service_date date) TO service_role;


--
-- Name: FUNCTION generate_invoice_run(_company_id uuid, _period_start date, _period_end date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.generate_invoice_run(_company_id uuid, _period_start date, _period_end date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.generate_invoice_run(_company_id uuid, _period_start date, _period_end date) TO authenticated;
GRANT ALL ON FUNCTION public.generate_invoice_run(_company_id uuid, _period_start date, _period_end date) TO service_role;


--
-- Name: FUNCTION get_effective_product_price_ex_vat(_company_id uuid, _product_id uuid, _service_date date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.get_effective_product_price_ex_vat(_company_id uuid, _product_id uuid, _service_date date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.get_effective_product_price_ex_vat(_company_id uuid, _product_id uuid, _service_date date) TO service_role;
...
--
-- Name: FUNCTION is_driver(); Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION is_ops(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.is_ops() TO anon;
GRANT ALL ON FUNCTION public.is_ops() TO authenticated;
GRANT ALL ON FUNCTION public.is_ops() TO service_role;
...
--
-- Name: FUNCTION is_platform_admin_legacy(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.is_platform_admin_legacy() TO anon;
GRANT ALL ON FUNCTION public.is_platform_admin_legacy() TO authenticated;
GRANT ALL ON FUNCTION public.is_platform_admin_legacy() TO service_role;


--
-- Name: FUNCTION is_platform_admin_or_service(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.is_platform_admin_or_service() TO anon;
GRANT ALL ON FUNCTION public.is_platform_admin_or_service() TO authenticated;
GRANT ALL ON FUNCTION public.is_platform_admin_or_service() TO service_role;
...
--
-- Name: FUNCTION jwt_email_lower(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.jwt_email_lower() TO anon;
GRANT ALL ON FUNCTION public.jwt_email_lower() TO authenticated;
GRANT ALL ON FUNCTION public.jwt_email_lower() TO service_role;


--
-- Name: FUNCTION log_ai_config_changes(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.log_ai_config_changes() TO anon;
GRANT ALL ON FUNCTION public.log_ai_config_changes() TO authenticated;
GRANT ALL ON FUNCTION public.log_ai_config_changes() TO service_role;


--
-- Name: FUNCTION lp_advisory_lock(p_key text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_advisory_lock(p_key text) TO anon;
GRANT ALL ON FUNCTION public.lp_advisory_lock(p_key text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_advisory_lock(p_key text) TO service_role;


--
-- Name: FUNCTION lp_agreement_activate(p_agreement_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_agreement_activate(p_agreement_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_agreement_activate(p_agreement_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_agreement_activate(p_agreement_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_agreement_activate(p_agreement_id uuid) TO service_role;


--
-- Name: FUNCTION lp_agreement_approve_active(p_agreement_id uuid, p_actor_user_id uuid); Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION lp_agreement_lifecycle_hook(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_agreement_lifecycle_hook() TO anon;
GRANT ALL ON FUNCTION public.lp_agreement_lifecycle_hook() TO authenticated;
GRANT ALL ON FUNCTION public.lp_agreement_lifecycle_hook() TO service_role;


--
-- Name: FUNCTION lp_agreement_reject_pending(p_agreement_id uuid, p_actor_user_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_agreement_reject_pending(p_agreement_id uuid, p_actor_user_id uuid, p_reason text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_agreement_reject_pending(p_agreement_id uuid, p_actor_user_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_apply_tripletex_paid_status(p_provider_id uuid, p_tripletex_invoice_id text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_apply_tripletex_paid_status(p_provider_id uuid, p_tripletex_invoice_id text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_apply_tripletex_paid_status(p_provider_id uuid, p_tripletex_invoice_id text) TO service_role;


--
-- Name: FUNCTION lp_assert_company_active(p_company_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_assert_company_active(p_company_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_assert_company_active(p_company_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_assert_company_active(p_company_id uuid) TO service_role;


--
-- Name: FUNCTION lp_company_delete(p_company_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_delete(p_company_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_delete(p_company_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_delete(p_company_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_order_summary(p_company_id uuid, p_period_start date, p_period_end date) TO service_role;


--
-- Name: FUNCTION lp_company_pause(p_company_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_pause(p_company_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_pause(p_company_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_pause(p_company_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_company_provider_customer_create(p_company_id uuid, p_provider_id uuid, p_env text, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_provider_customer_create(p_company_id uuid, p_provider_id uuid, p_env text, p_request_rid text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_provider_customer_create(p_company_id uuid, p_provider_id uuid, p_env text, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_provider_customer_create(p_company_id uuid, p_provider_id uuid, p_env text, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_company_register(p_company_name text, p_orgnr text, p_employee_count integer, p_contact_name text, p_contact_email text, p_contact_phone text, p_address_line text, p_postal_code text, p_postal_city text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_register(p_company_name text, p_orgnr text, p_employee_count integer, p_contact_name text, p_contact_email text, p_contact_phone text, p_address_line text, p_postal_code text, p_postal_city text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_register(p_company_name text, p_orgnr text, p_employee_count integer, p_contact_name text, p_contact_email text, p_contact_phone text, p_address_line text, p_postal_code text, p_postal_city text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_register(p_company_name text, p_orgnr text, p_employee_count integer, p_contact_name text, p_contact_email text, p_contact_phone text, p_address_line text, p_postal_code text, p_postal_city text) TO service_role;


--
-- Name: FUNCTION lp_company_registration_approve_provider(p_registration_id uuid, p_agreement_tier text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_registration_approve_provider(p_registration_id uuid, p_agreement_tier text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_registration_approve_provider(p_registration_id uuid, p_agreement_tier text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_registration_approve_provider(p_registration_id uuid, p_agreement_tier text) TO service_role;


--
-- Name: FUNCTION lp_company_registration_create(p_payload jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_registration_create(p_payload jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_company_registration_create(p_payload jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_registration_create(p_payload jsonb) TO service_role;


--
-- Name: FUNCTION lp_company_registration_reject_provider(p_registration_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_registration_reject_provider(p_registration_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_registration_reject_provider(p_registration_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_registration_reject_provider(p_registration_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_company_resume(p_company_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_resume(p_company_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_company_resume(p_company_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_resume(p_company_id uuid) TO service_role;


--
-- Name: FUNCTION lp_company_suspend(p_company_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_company_suspend(p_company_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_company_suspend(p_company_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_company_suspend(p_company_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_compute_agreements_due_today(p_today date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_compute_agreements_due_today(p_today date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_compute_agreements_due_today(p_today date) TO service_role;


--
-- Name: FUNCTION lp_delivery_set_status(p_delivery_id uuid, p_status public.delivery_status, p_proof jsonb); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_delivery_set_status(p_delivery_id uuid, p_status public.delivery_status, p_proof jsonb) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_delivery_set_status(p_delivery_id uuid, p_status public.delivery_status, p_proof jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_delivery_set_status(p_delivery_id uuid, p_status public.delivery_status, p_proof jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_delivery_set_status(p_delivery_id uuid, p_status public.delivery_status, p_proof jsonb) TO service_role;


--
-- Name: FUNCTION lp_esg_rollup_month(p_month_start date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_esg_rollup_month(p_month_start date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_esg_rollup_month(p_month_start date) TO anon;
GRANT ALL ON FUNCTION public.lp_esg_rollup_month(p_month_start date) TO authenticated;
GRANT ALL ON FUNCTION public.lp_esg_rollup_month(p_month_start date) TO service_role;


--
-- Name: FUNCTION lp_generate_agreement_invoices_for_period(p_period_start date, p_period_end date, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_generate_agreement_invoices_for_period(p_period_start date, p_period_end date, p_request_rid text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_generate_agreement_invoices_for_period(p_period_start date, p_period_end date, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_generate_agreement_invoices_for_period(p_period_start date, p_period_end date, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_generate_saas_invoices_for_period(p_invoice_period date, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_generate_saas_invoices_for_period(p_invoice_period date, p_request_rid text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_generate_saas_invoices_for_period(p_invoice_period date, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_generate_saas_invoices_for_period(p_invoice_period date, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_idem_begin(p_scope text, p_key text, p_request_hash text, p_ttl_seconds integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_idem_begin(p_scope text, p_key text, p_request_hash text, p_ttl_seconds integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_idem_begin(p_scope text, p_key text, p_request_hash text, p_ttl_seconds integer) TO anon;
GRANT ALL ON FUNCTION public.lp_idem_begin(p_scope text, p_key text, p_request_hash text, p_ttl_seconds integer) TO authenticated;
GRANT ALL ON FUNCTION public.lp_idem_begin(p_scope text, p_key text, p_request_hash text, p_ttl_seconds integer) TO service_role;


--
-- Name: FUNCTION lp_idem_complete(p_scope text, p_key text, p_request_hash text, p_response_json jsonb, p_response_code integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_idem_complete(p_scope text, p_key text, p_request_hash text, p_response_json jsonb, p_response_code integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_idem_complete(p_scope text, p_key text, p_request_hash text, p_response_json jsonb, p_response_code integer) TO anon;
GRANT ALL ON FUNCTION public.lp_idem_complete(p_scope text, p_key text, p_request_hash text, p_response_json jsonb, p_response_code integer) TO authenticated;
GRANT ALL ON FUNCTION public.lp_idem_complete(p_scope text, p_key text, p_request_hash text, p_response_json jsonb, p_response_code integer) TO service_role;


--
-- Name: FUNCTION lp_idem_fail(p_scope text, p_key text, p_request_hash text, p_error text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_idem_fail(p_scope text, p_key text, p_request_hash text, p_error text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_idem_fail(p_scope text, p_key text, p_request_hash text, p_error text) TO anon;
GRANT ALL ON FUNCTION public.lp_idem_fail(p_scope text, p_key text, p_request_hash text, p_error text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_idem_fail(p_scope text, p_key text, p_request_hash text, p_error text) TO service_role;


--
-- Name: FUNCTION lp_invoice_build_month(p_month_start date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_invoice_build_month(p_month_start date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_invoice_build_month(p_month_start date) TO anon;
GRANT ALL ON FUNCTION public.lp_invoice_build_month(p_month_start date) TO authenticated;
GRANT ALL ON FUNCTION public.lp_invoice_build_month(p_month_start date) TO service_role;


--
-- Name: FUNCTION lp_match_provider_by_postal_code(p_postal_code text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_match_provider_by_postal_code(p_postal_code text) TO anon;
GRANT ALL ON FUNCTION public.lp_match_provider_by_postal_code(p_postal_code text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_match_provider_by_postal_code(p_postal_code text) TO service_role;


--
-- Name: FUNCTION lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text) TO anon;
GRANT ALL ON FUNCTION public.lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_order_advance_status(p_order_id uuid, p_target_status text, p_note text) TO service_role;


--
-- Name: FUNCTION lp_order_set(p_date date, p_action text, p_note text, p_slot text, p_choice_key text, p_item_key text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_order_set(p_date date, p_action text, p_note text, p_slot text, p_choice_key text, p_item_key text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_order_set(p_date date, p_action text, p_note text, p_slot text, p_choice_key text, p_item_key text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_order_set(p_date date, p_action text, p_note text, p_slot text, p_choice_key text, p_item_key text) TO service_role;
...
--
-- Name: FUNCTION lp_outbox_claim(p_limit integer, p_worker text, p_exclude_prefixes text[]); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_outbox_claim(p_limit integer, p_worker text, p_exclude_prefixes text[]) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_outbox_claim(p_limit integer, p_worker text, p_exclude_prefixes text[]) TO service_role;


--
-- Name: FUNCTION lp_outbox_mark_failed(p_id uuid, p_error text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_outbox_mark_failed(p_id uuid, p_error text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_outbox_mark_failed(p_id uuid, p_error text) TO service_role;


--
-- Name: FUNCTION lp_outbox_mark_sent(p_id uuid, p_message_id text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_outbox_mark_sent(p_id uuid, p_message_id text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_outbox_mark_sent(p_id uuid, p_message_id text) TO service_role;


--
-- Name: FUNCTION lp_outbox_reset_stale(p_stale_minutes integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_outbox_reset_stale(p_stale_minutes integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_outbox_reset_stale(p_stale_minutes integer) TO service_role;


--
-- Name: FUNCTION lp_outbox_retry_event(p_event_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_outbox_retry_event(p_event_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_outbox_retry_event(p_event_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_outbox_retry_event(p_event_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_outbox_retry_event(p_event_id uuid) TO service_role;


--
-- Name: FUNCTION lp_production_freeze_day(p_date date); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_production_freeze_day(p_date date) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_production_freeze_day(p_date date) TO anon;
GRANT ALL ON FUNCTION public.lp_production_freeze_day(p_date date) TO authenticated;
GRANT ALL ON FUNCTION public.lp_production_freeze_day(p_date date) TO service_role;


--
-- Name: FUNCTION lp_provider_apply_connection_health_check(p_provider_id uuid, p_env text, p_ok boolean, p_auth_failed boolean, p_company_name text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_apply_connection_health_check(p_provider_id uuid, p_env text, p_ok boolean, p_auth_failed boolean, p_company_name text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_apply_connection_health_check(p_provider_id uuid, p_env text, p_ok boolean, p_auth_failed boolean, p_company_name text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_apply_connection_health_check(p_provider_id uuid, p_env text, p_ok boolean, p_auth_failed boolean, p_company_name text) TO service_role;


--
-- Name: FUNCTION lp_provider_complete_onboarding_provisioning(p_provider_id uuid, p_env text, p_summary jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_complete_onboarding_provisioning(p_provider_id uuid, p_env text, p_summary jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_complete_onboarding_provisioning(p_provider_id uuid, p_env text, p_summary jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_complete_onboarding_provisioning(p_provider_id uuid, p_env text, p_summary jsonb) TO service_role;


--
-- Name: FUNCTION lp_provider_complete_tripletex_connection(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_consumer_token text, p_verification_result jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_complete_tripletex_connection(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_consumer_token text, p_verification_result jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_complete_tripletex_connection(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_consumer_token text, p_verification_result jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_complete_tripletex_connection(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_consumer_token text, p_verification_result jsonb) TO service_role;


--
-- Name: FUNCTION lp_provider_create(p_slug text, p_name text, p_contact_email text, p_org_number text, p_contact_phone text, p_billing_org_no text, p_billing_email text, p_billing_address text, p_default_tier_pricing text, p_billing_model text, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_create(p_slug text, p_name text, p_contact_email text, p_org_number text, p_contact_phone text, p_billing_org_no text, p_billing_email text, p_billing_address text, p_default_tier_pricing text, p_billing_model text, p_request_rid text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_create(p_slug text, p_name text, p_contact_email text, p_org_number text, p_contact_phone text, p_billing_org_no text, p_billing_email text, p_billing_address text, p_default_tier_pricing text, p_billing_model text, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_create(p_slug text, p_name text, p_contact_email text, p_org_number text, p_contact_phone text, p_billing_org_no text, p_billing_email text, p_billing_address text, p_default_tier_pricing text, p_billing_model text, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_provider_delete(p_provider_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_delete(p_provider_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_delete(p_provider_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_delete(p_provider_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_provider_disconnect_tripletex(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_disconnect_tripletex(p_provider_id uuid, p_env text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_disconnect_tripletex(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_disconnect_tripletex(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_finalize_tripletex_connection(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_finalize_tripletex_connection(p_provider_id uuid, p_env text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_finalize_tripletex_connection(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_finalize_tripletex_connection(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_generate_agreement_invoice_for_period(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_provider_generate_agreement_invoice_for_period(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_provider_generate_agreement_invoice_for_period(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_generate_agreement_invoice_for_period(p_agreement_id uuid, p_period_start date, p_period_end date, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_provider_generate_invoice_for_period(p_provider_id uuid, p_invoice_period date, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_provider_generate_invoice_for_period(p_provider_id uuid, p_invoice_period date, p_request_rid text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_provider_generate_invoice_for_period(p_provider_id uuid, p_invoice_period date, p_request_rid text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_generate_invoice_for_period(p_provider_id uuid, p_invoice_period date, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_provider_get_connection_health(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_get_connection_health(p_provider_id uuid, p_env text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_get_connection_health(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_get_connection_health(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_get_tripletex_credentials_status(p_provider_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_get_tripletex_credentials_status(p_provider_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_get_tripletex_credentials_status(p_provider_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_get_tripletex_credentials_status(p_provider_id uuid) TO service_role;


--
-- Name: FUNCTION lp_provider_load_tripletex_credentials(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_provider_load_tripletex_credentials(p_provider_id uuid, p_env text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_provider_load_tripletex_credentials(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_load_webhook_secret(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_provider_load_webhook_secret(p_provider_id uuid, p_env text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_provider_load_webhook_secret(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_pause(p_provider_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_pause(p_provider_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_pause(p_provider_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_pause(p_provider_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_provider_purge_disconnected_vault(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_purge_disconnected_vault(p_provider_id uuid, p_env text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_purge_disconnected_vault(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_purge_disconnected_vault(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_reconnect_tripletex(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_reconnect_tripletex(p_provider_id uuid, p_env text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_reconnect_tripletex(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_reconnect_tripletex(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_resume(p_provider_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_resume(p_provider_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_resume(p_provider_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_resume(p_provider_id uuid) TO service_role;


--
-- Name: FUNCTION lp_provider_rotate_webhook_secret(p_provider_id uuid, p_env text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_provider_rotate_webhook_secret(p_provider_id uuid, p_env text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_provider_rotate_webhook_secret(p_provider_id uuid, p_env text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_rotate_webhook_secret(p_provider_id uuid, p_env text) TO service_role;


--
-- Name: FUNCTION lp_provider_set_subscription(p_provider_id uuid, p_plan text, p_monthly_amount numeric, p_billing_email text, p_billing_org_number text, p_billing_address text, p_notes text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_set_subscription(p_provider_id uuid, p_plan text, p_monthly_amount numeric, p_billing_email text, p_billing_org_number text, p_billing_address text, p_notes text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_set_subscription(p_provider_id uuid, p_plan text, p_monthly_amount numeric, p_billing_email text, p_billing_org_number text, p_billing_address text, p_notes text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_set_subscription(p_provider_id uuid, p_plan text, p_monthly_amount numeric, p_billing_email text, p_billing_org_number text, p_billing_address text, p_notes text) TO service_role;


--
-- Name: FUNCTION lp_provider_set_tripletex_credentials(p_provider_id uuid, p_env text, p_consumer_token text, p_employee_token text, p_company_id_external bigint); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_set_tripletex_credentials(p_provider_id uuid, p_env text, p_consumer_token text, p_employee_token text, p_company_id_external bigint) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_set_tripletex_credentials(p_provider_id uuid, p_env text, p_consumer_token text, p_employee_token text, p_company_id_external bigint) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_set_tripletex_credentials(p_provider_id uuid, p_env text, p_consumer_token text, p_employee_token text, p_company_id_external bigint) TO service_role;


--
-- Name: FUNCTION lp_provider_suspend(p_provider_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_suspend(p_provider_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_suspend(p_provider_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_suspend(p_provider_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION lp_provider_test_tripletex_token(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_verification_result jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_test_tripletex_token(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_verification_result jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_test_tripletex_token(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_verification_result jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_test_tripletex_token(p_provider_id uuid, p_env text, p_tripletex_company_id bigint, p_employee_token text, p_verification_result jsonb) TO service_role;


--
-- Name: FUNCTION lp_provider_update_billing_contact(p_provider_id uuid, p_billing_email text, p_billing_org_number text, p_billing_address text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_provider_update_billing_contact(p_provider_id uuid, p_billing_email text, p_billing_org_number text, p_billing_address text) TO anon;
GRANT ALL ON FUNCTION public.lp_provider_update_billing_contact(p_provider_id uuid, p_billing_email text, p_billing_org_number text, p_billing_address text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_provider_update_billing_contact(p_provider_id uuid, p_billing_email text, p_billing_org_number text, p_billing_address text) TO service_role;


--
-- Name: FUNCTION lp_req_hash(p_payload jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_req_hash(p_payload jsonb) TO anon;
GRANT ALL ON FUNCTION public.lp_req_hash(p_payload jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.lp_req_hash(p_payload jsonb) TO service_role;


--
-- Name: FUNCTION lp_run_daily_agreement_billing(p_today date, p_request_rid text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.lp_run_daily_agreement_billing(p_today date, p_request_rid text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.lp_run_daily_agreement_billing(p_today date, p_request_rid text) TO service_role;


--
-- Name: FUNCTION lp_service_area_save(p_id uuid, p_provider_id uuid, p_city text, p_postal_code_from text, p_postal_code_to text, p_min_employees integer, p_max_employees integer, p_available_days text[], p_active boolean); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_service_area_save(p_id uuid, p_provider_id uuid, p_city text, p_postal_code_from text, p_postal_code_to text, p_min_employees integer, p_max_employees integer, p_available_days text[], p_active boolean) TO anon;
GRANT ALL ON FUNCTION public.lp_service_area_save(p_id uuid, p_provider_id uuid, p_city text, p_postal_code_from text, p_postal_code_to text, p_min_employees integer, p_max_employees integer, p_available_days text[], p_active boolean) TO authenticated;
GRANT ALL ON FUNCTION public.lp_service_area_save(p_id uuid, p_provider_id uuid, p_city text, p_postal_code_from text, p_postal_code_to text, p_min_employees integer, p_max_employees integer, p_available_days text[], p_active boolean) TO service_role;


--
-- Name: FUNCTION lp_service_area_toggle_active(p_id uuid, p_active boolean); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_service_area_toggle_active(p_id uuid, p_active boolean) TO anon;
GRANT ALL ON FUNCTION public.lp_service_area_toggle_active(p_id uuid, p_active boolean) TO authenticated;
GRANT ALL ON FUNCTION public.lp_service_area_toggle_active(p_id uuid, p_active boolean) TO service_role;


--
-- Name: FUNCTION lp_touch_invites_updated_at(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_touch_invites_updated_at() TO anon;
GRANT ALL ON FUNCTION public.lp_touch_invites_updated_at() TO authenticated;
GRANT ALL ON FUNCTION public.lp_touch_invites_updated_at() TO service_role;


--
-- Name: FUNCTION lp_user_delete(p_user_id uuid, p_reason text, p_gdpr boolean); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_user_delete(p_user_id uuid, p_reason text, p_gdpr boolean) TO anon;
GRANT ALL ON FUNCTION public.lp_user_delete(p_user_id uuid, p_reason text, p_gdpr boolean) TO authenticated;
GRANT ALL ON FUNCTION public.lp_user_delete(p_user_id uuid, p_reason text, p_gdpr boolean) TO service_role;


--
-- Name: FUNCTION lp_user_pause(p_user_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_user_pause(p_user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_user_pause(p_user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_user_pause(p_user_id uuid) TO service_role;


--
-- Name: FUNCTION lp_user_resume(p_user_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_user_resume(p_user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.lp_user_resume(p_user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.lp_user_resume(p_user_id uuid) TO service_role;


--
-- Name: FUNCTION lp_user_suspend(p_user_id uuid, p_reason text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.lp_user_suspend(p_user_id uuid, p_reason text) TO anon;
GRANT ALL ON FUNCTION public.lp_user_suspend(p_user_id uuid, p_reason text) TO authenticated;
GRANT ALL ON FUNCTION public.lp_user_suspend(p_user_id uuid, p_reason text) TO service_role;


--
-- Name: FUNCTION normalize_delivery_weekday(input_value text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.normalize_delivery_weekday(input_value text) TO anon;
GRANT ALL ON FUNCTION public.normalize_delivery_weekday(input_value text) TO authenticated;
GRANT ALL ON FUNCTION public.normalize_delivery_weekday(input_value text) TO service_role;


--
-- Name: FUNCTION oslo_now_ts(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.oslo_now_ts() TO anon;
GRANT ALL ON FUNCTION public.oslo_now_ts() TO authenticated;
GRANT ALL ON FUNCTION public.oslo_now_ts() TO service_role;


--
-- Name: FUNCTION oslo_time(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.oslo_time() TO anon;
GRANT ALL ON FUNCTION public.oslo_time() TO authenticated;
GRANT ALL ON FUNCTION public.oslo_time() TO service_role;


--
-- Name: FUNCTION oslo_today(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.oslo_today() TO anon;
GRANT ALL ON FUNCTION public.oslo_today() TO authenticated;
GRANT ALL ON FUNCTION public.oslo_today() TO service_role;


--
-- Name: FUNCTION outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer) TO service_role;


--
-- Name: FUNCTION outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer, p_requeue_stale_seconds integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer, p_requeue_stale_seconds integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer, p_requeue_stale_seconds integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer, p_requeue_stale_seconds integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_claim_next(p_limit integer, p_worker text, p_lock_seconds integer, p_requeue_stale_seconds integer) TO service_role;


--
-- Name: FUNCTION outbox_mark_failed(p_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO service_role;


--
-- Name: FUNCTION outbox_mark_failed(p_id uuid, p_lease_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_lease_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_lease_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_lease_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_mark_failed(p_id uuid, p_lease_id uuid, p_error text, p_backoff_seconds integer, p_max_attempts integer) TO service_role;


--
-- Name: FUNCTION outbox_mark_permanent_failed(p_max_attempts integer); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.outbox_mark_permanent_failed(p_max_attempts integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_mark_permanent_failed(p_max_attempts integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_mark_permanent_failed(p_max_attempts integer) TO service_role;


--
-- Name: FUNCTION outbox_mark_sent(p_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_mark_sent(p_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid) TO anon;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid) TO service_role;


--
-- Name: FUNCTION outbox_mark_sent(p_id uuid, p_lease_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_mark_sent(p_id uuid, p_lease_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid, p_lease_id uuid) TO anon;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid, p_lease_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_mark_sent(p_id uuid, p_lease_id uuid) TO service_role;


--
-- Name: FUNCTION outbox_requeue_stale(p_stale_seconds integer); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.outbox_requeue_stale(p_stale_seconds integer) FROM PUBLIC;
GRANT ALL ON FUNCTION public.outbox_requeue_stale(p_stale_seconds integer) TO anon;
GRANT ALL ON FUNCTION public.outbox_requeue_stale(p_stale_seconds integer) TO authenticated;
GRANT ALL ON FUNCTION public.outbox_requeue_stale(p_stale_seconds integer) TO service_role;


--
-- Name: FUNCTION project_profile_scope_from_memberships(p_user_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.project_profile_scope_from_memberships(p_user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.project_profile_scope_from_memberships(p_user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.project_profile_scope_from_memberships(p_user_id uuid) TO service_role;


--
-- Name: FUNCTION recalculate_invoice_run_totals(_invoice_run_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.recalculate_invoice_run_totals(_invoice_run_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.recalculate_invoice_run_totals(_invoice_run_id uuid) TO service_role;


--
-- Name: FUNCTION recalculate_invoice_totals_nok(p_invoice_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.recalculate_invoice_totals_nok(p_invoice_id uuid) TO anon;
GRANT ALL ON FUNCTION public.recalculate_invoice_totals_nok(p_invoice_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.recalculate_invoice_totals_nok(p_invoice_id uuid) TO service_role;


--
-- Name: FUNCTION recalculate_order_totals(_order_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.recalculate_order_totals(_order_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.recalculate_order_totals(_order_id uuid) TO service_role;


--
-- Name: FUNCTION recompute_profile_legacy_scope(p_user_id uuid); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.recompute_profile_legacy_scope(p_user_id uuid) TO anon;
GRANT ALL ON FUNCTION public.recompute_profile_legacy_scope(p_user_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.recompute_profile_legacy_scope(p_user_id uuid) TO service_role;


--
-- Name: FUNCTION refresh_delivery_run_items(_delivery_run_id uuid); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.refresh_delivery_run_items(_delivery_run_id uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION public.refresh_delivery_run_items(_delivery_run_id uuid) TO service_role;


--
-- Name: FUNCTION safe_trim(v text); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.safe_trim(v text) TO anon;
GRANT ALL ON FUNCTION public.safe_trim(v text) TO authenticated;
GRANT ALL ON FUNCTION public.safe_trim(v text) TO service_role;
...
--
-- Name: FUNCTION sync_agreement_delivery_days_from_legacy_jsonb(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.sync_agreement_delivery_days_from_legacy_jsonb() TO anon;
GRANT ALL ON FUNCTION public.sync_agreement_delivery_days_from_legacy_jsonb() TO authenticated;
GRANT ALL ON FUNCTION public.sync_agreement_delivery_days_from_legacy_jsonb() TO service_role;


--
-- Name: FUNCTION sync_memberships_from_legacy_profile(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.sync_memberships_from_legacy_profile() TO anon;
GRANT ALL ON FUNCTION public.sync_memberships_from_legacy_profile() TO authenticated;
GRANT ALL ON FUNCTION public.sync_memberships_from_legacy_profile() TO service_role;


--
-- Name: FUNCTION tg_audit_row(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_audit_row() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_audit_row() TO service_role;


--
-- Name: FUNCTION tg_create_profile_from_auth_user(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_create_profile_from_auth_user() TO anon;
GRANT ALL ON FUNCTION public.tg_create_profile_from_auth_user() TO authenticated;
GRANT ALL ON FUNCTION public.tg_create_profile_from_auth_user() TO service_role;


--
-- Name: FUNCTION tg_guard_order_mutation(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_guard_order_mutation() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_guard_order_mutation() TO service_role;


--
-- Name: FUNCTION tg_invoice_lines_hydrate_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_invoice_lines_hydrate_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_invoice_lines_hydrate_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_invoice_lines_hydrate_v2() TO service_role;


--
-- Name: FUNCTION tg_invoice_lines_recalculate_invoice_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_invoice_lines_recalculate_invoice_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_invoice_lines_recalculate_invoice_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_invoice_lines_recalculate_invoice_v2() TO service_role;


--
-- Name: FUNCTION tg_menu_service_day_defaults(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_menu_service_day_defaults() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_menu_service_day_defaults() TO service_role;


--
-- Name: FUNCTION tg_menu_service_day_item_snapshot(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_menu_service_day_item_snapshot() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_menu_service_day_item_snapshot() TO service_role;


--
-- Name: FUNCTION tg_normalize_invoice_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_normalize_invoice_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_normalize_invoice_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_normalize_invoice_v2() TO service_role;


--
-- Name: FUNCTION tg_normalize_production_day_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_normalize_production_day_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_normalize_production_day_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_normalize_production_day_v2() TO service_role;


--
-- Name: FUNCTION tg_order_defaults(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_order_defaults() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_order_defaults() TO service_role;


--
-- Name: FUNCTION tg_order_identity_immutable(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_order_identity_immutable() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_order_identity_immutable() TO service_role;


--
-- Name: FUNCTION tg_order_item_snapshot(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_order_item_snapshot() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_order_item_snapshot() TO service_role;


--
-- Name: FUNCTION tg_order_status_history(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_order_status_history() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_order_status_history() TO service_role;


--
-- Name: FUNCTION tg_orders_block_closed_dates(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_block_closed_dates() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_block_closed_dates() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_block_closed_dates() TO service_role;


--
-- Name: FUNCTION tg_orders_block_if_frozen(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_block_if_frozen() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_block_if_frozen() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_block_if_frozen() TO service_role;


--
-- Name: FUNCTION tg_orders_cutoff_0800(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_cutoff_0800() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_cutoff_0800() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_cutoff_0800() TO service_role;


--
-- Name: FUNCTION tg_orders_enforce_integrity(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_enforce_integrity() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_enforce_integrity() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_enforce_integrity() TO service_role;


--
-- Name: FUNCTION tg_orders_hydrate_core_fields(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_hydrate_core_fields() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_hydrate_core_fields() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_hydrate_core_fields() TO service_role;


--
-- Name: FUNCTION tg_orders_require_active_agreement(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_require_active_agreement() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_require_active_agreement() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_require_active_agreement() TO service_role;


--
-- Name: FUNCTION tg_orders_require_active_company(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_orders_require_active_company() TO anon;
GRANT ALL ON FUNCTION public.tg_orders_require_active_company() TO authenticated;
GRANT ALL ON FUNCTION public.tg_orders_require_active_company() TO service_role;


--
-- Name: FUNCTION tg_profiles_enforce_company_location(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_profiles_enforce_company_location() TO anon;
GRANT ALL ON FUNCTION public.tg_profiles_enforce_company_location() TO authenticated;
GRANT ALL ON FUNCTION public.tg_profiles_enforce_company_location() TO service_role;


--
-- Name: FUNCTION tg_recalculate_order_totals_from_items(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_recalculate_order_totals_from_items() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_recalculate_order_totals_from_items() TO service_role;
...
--
-- Name: FUNCTION tg_tripletex_invoices_hydrate_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_tripletex_invoices_hydrate_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_tripletex_invoices_hydrate_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_tripletex_invoices_hydrate_v2() TO service_role;


--
-- Name: FUNCTION tg_validate_company_product_price_scope(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_validate_company_product_price_scope() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_validate_company_product_price_scope() TO service_role;


--
-- Name: FUNCTION tg_validate_delivery_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_validate_delivery_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_validate_delivery_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_validate_delivery_v2() TO service_role;


--
-- Name: FUNCTION tg_validate_driver_run_assignment_v2(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.tg_validate_driver_run_assignment_v2() TO anon;
GRANT ALL ON FUNCTION public.tg_validate_driver_run_assignment_v2() TO authenticated;
GRANT ALL ON FUNCTION public.tg_validate_driver_run_assignment_v2() TO service_role;


--
-- Name: FUNCTION tg_validate_membership_scope(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_validate_membership_scope() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_validate_membership_scope() TO service_role;


--
-- Name: FUNCTION tg_validate_standing_order_scope(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.tg_validate_standing_order_scope() FROM PUBLIC;
GRANT ALL ON FUNCTION public.tg_validate_standing_order_scope() TO service_role;


--
-- Name: FUNCTION trg_company_memberships_recompute_profile_legacy_scope(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.trg_company_memberships_recompute_profile_legacy_scope() TO anon;
GRANT ALL ON FUNCTION public.trg_company_memberships_recompute_profile_legacy_scope() TO authenticated;
GRANT ALL ON FUNCTION public.trg_company_memberships_recompute_profile_legacy_scope() TO service_role;


--
-- Name: FUNCTION trg_location_memberships_recompute_profile_legacy_scope(); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.trg_location_memberships_recompute_profile_legacy_scope() TO anon;
GRANT ALL ON FUNCTION public.trg_location_memberships_recompute_profile_legacy_scope() TO authenticated;
GRANT ALL ON FUNCTION public.trg_location_memberships_recompute_profile_legacy_scope() TO service_role;


--
-- Name: TABLE _migration_legacy_stub_invoice_lines_archive; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public._migration_legacy_stub_invoice_lines_archive TO anon;
GRANT ALL ON TABLE public._migration_legacy_stub_invoice_lines_archive TO authenticated;
GRANT ALL ON TABLE public._migration_legacy_stub_invoice_lines_archive TO service_role;


--
-- Name: TABLE _migration_legacy_stub_order_items_archive; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public._migration_legacy_stub_order_items_archive TO anon;
GRANT ALL ON TABLE public._migration_legacy_stub_order_items_archive TO authenticated;
GRANT ALL ON TABLE public._migration_legacy_stub_order_items_archive TO service_role;


--
-- Name: TABLE _migration_legacy_stub_orders_archive; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public._migration_legacy_stub_orders_archive TO anon;
GRANT ALL ON TABLE public._migration_legacy_stub_orders_archive TO authenticated;
GRANT ALL ON TABLE public._migration_legacy_stub_orders_archive TO service_role;


--
-- Name: TABLE _migration_legacy_stub_orders_manifest; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public._migration_legacy_stub_orders_manifest TO anon;
GRANT ALL ON TABLE public._migration_legacy_stub_orders_manifest TO authenticated;
GRANT ALL ON TABLE public._migration_legacy_stub_orders_manifest TO service_role;


--
-- Name: TABLE _migration_orders_location_id_backup; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public._migration_orders_location_id_backup TO anon;
GRANT ALL ON TABLE public._migration_orders_location_id_backup TO authenticated;
GRANT ALL ON TABLE public._migration_orders_location_id_backup TO service_role;
...
--
-- Name: TABLE agreement_active_overlap_conflicts_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_active_overlap_conflicts_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_active_overlap_conflicts_v TO authenticated;
GRANT ALL ON TABLE public.agreement_active_overlap_conflicts_v TO service_role;


--
-- Name: TABLE agreement_cleanup_audit; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.agreement_cleanup_audit TO anon;
GRANT ALL ON TABLE public.agreement_cleanup_audit TO authenticated;
GRANT ALL ON TABLE public.agreement_cleanup_audit TO service_role;


--
-- Name: SEQUENCE agreement_cleanup_audit_id_seq; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON SEQUENCE public.agreement_cleanup_audit_id_seq TO anon;
GRANT ALL ON SEQUENCE public.agreement_cleanup_audit_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.agreement_cleanup_audit_id_seq TO service_role;


--
-- Name: TABLE agreement_delivery_days; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.agreement_delivery_days TO anon;
GRANT ALL ON TABLE public.agreement_delivery_days TO authenticated;
GRANT ALL ON TABLE public.agreement_delivery_days TO service_role;


--
-- Name: TABLE agreement_invoice_lines; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.agreement_invoice_lines TO authenticated;
GRANT ALL ON TABLE public.agreement_invoice_lines TO service_role;


--
-- Name: TABLE agreement_invoices; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.agreement_invoices TO authenticated;
GRANT ALL ON TABLE public.agreement_invoices TO service_role;


--
-- Name: TABLE agreement_overlap_conflicts_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_overlap_conflicts_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_overlap_conflicts_v TO authenticated;
GRANT ALL ON TABLE public.agreement_overlap_conflicts_v TO service_role;


--
-- Name: TABLE agreement_pending_exact_shadow_duplicates_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_pending_exact_shadow_duplicates_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_pending_exact_shadow_duplicates_v TO authenticated;
GRANT ALL ON TABLE public.agreement_pending_exact_shadow_duplicates_v TO service_role;


--
-- Name: TABLE agreement_pending_nonexact_shadow_conflicts_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_pending_nonexact_shadow_conflicts_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_pending_nonexact_shadow_conflicts_v TO authenticated;
GRANT ALL ON TABLE public.agreement_pending_nonexact_shadow_conflicts_v TO service_role;


--
-- Name: TABLE agreement_pending_pending_exact_duplicates_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_pending_pending_exact_duplicates_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_pending_pending_exact_duplicates_v TO authenticated;
GRANT ALL ON TABLE public.agreement_pending_pending_exact_duplicates_v TO service_role;


--
-- Name: TABLE agreement_pending_shadow_conflicts_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.agreement_pending_shadow_conflicts_v TO anon;
GRANT MAINTAIN ON TABLE public.agreement_pending_shadow_conflicts_v TO authenticated;
GRANT ALL ON TABLE public.agreement_pending_shadow_conflicts_v TO service_role;


--
-- Name: TABLE agreement_requests; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.agreement_requests TO anon;
GRANT ALL ON TABLE public.agreement_requests TO authenticated;
GRANT ALL ON TABLE public.agreement_requests TO service_role;


--
-- Name: TABLE ai_action_memory; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.ai_action_memory TO anon;
GRANT ALL ON TABLE public.ai_action_memory TO authenticated;
GRANT ALL ON TABLE public.ai_action_memory TO service_role;


--
-- Name: TABLE ai_activity_log; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE ai_config_audit; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.ai_config_audit TO anon;
GRANT ALL ON TABLE public.ai_config_audit TO authenticated;
GRANT ALL ON TABLE public.ai_config_audit TO service_role;


--
-- Name: TABLE ai_health_checks; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.ai_health_checks TO anon;
GRANT ALL ON TABLE public.ai_health_checks TO authenticated;
GRANT ALL ON TABLE public.ai_health_checks TO service_role;
...
--
-- Name: TABLE ai_suggestions; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE allergens; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE audit_log_legacy; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_legacy TO anon;
GRANT ALL ON TABLE public.audit_log_legacy TO authenticated;
GRANT ALL ON TABLE public.audit_log_legacy TO service_role;


--
-- Name: TABLE audit_log_y2026m05; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m05 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m05 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m05 TO service_role;


--
-- Name: TABLE audit_log_y2026m06; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m06 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m06 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m06 TO service_role;


--
-- Name: TABLE audit_log_y2026m07; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m07 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m07 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m07 TO service_role;


--
-- Name: TABLE audit_log_y2026m08; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m08 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m08 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m08 TO service_role;


--
-- Name: TABLE audit_log_y2026m09; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m09 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m09 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m09 TO service_role;


--
-- Name: TABLE audit_log_y2026m10; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m10 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m10 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m10 TO service_role;


--
-- Name: TABLE audit_log_y2026m11; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m11 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m11 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m11 TO service_role;


--
-- Name: TABLE audit_log_y2026m12; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2026m12 TO anon;
GRANT ALL ON TABLE public.audit_log_y2026m12 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2026m12 TO service_role;


--
-- Name: TABLE audit_log_y2027m01; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m01 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m01 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m01 TO service_role;


--
-- Name: TABLE audit_log_y2027m02; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m02 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m02 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m02 TO service_role;


--
-- Name: TABLE audit_log_y2027m03; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m03 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m03 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m03 TO service_role;


--
-- Name: TABLE audit_log_y2027m04; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m04 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m04 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m04 TO service_role;


--
-- Name: TABLE audit_log_y2027m05; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m05 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m05 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m05 TO service_role;


--
-- Name: TABLE audit_log_y2027m06; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m06 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m06 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m06 TO service_role;


--
-- Name: TABLE audit_log_y2027m07; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m07 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m07 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m07 TO service_role;


--
-- Name: TABLE audit_log_y2027m08; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m08 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m08 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m08 TO service_role;


--
-- Name: TABLE audit_log_y2027m09; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m09 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m09 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m09 TO service_role;


--
-- Name: TABLE audit_log_y2027m10; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m10 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m10 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m10 TO service_role;


--
-- Name: TABLE audit_log_y2027m11; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m11 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m11 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m11 TO service_role;


--
-- Name: TABLE audit_log_y2027m12; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2027m12 TO anon;
GRANT ALL ON TABLE public.audit_log_y2027m12 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2027m12 TO service_role;


--
-- Name: TABLE audit_log_y2028m01; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m01 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m01 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m01 TO service_role;


--
-- Name: TABLE audit_log_y2028m02; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m02 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m02 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m02 TO service_role;


--
-- Name: TABLE audit_log_y2028m03; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m03 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m03 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m03 TO service_role;


--
-- Name: TABLE audit_log_y2028m04; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m04 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m04 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m04 TO service_role;


--
-- Name: TABLE audit_log_y2028m05; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m05 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m05 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m05 TO service_role;


--
-- Name: TABLE audit_log_y2028m06; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m06 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m06 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m06 TO service_role;


--
-- Name: TABLE audit_log_y2028m07; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m07 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m07 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m07 TO service_role;


--
-- Name: TABLE audit_log_y2028m08; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m08 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m08 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m08 TO service_role;


--
-- Name: TABLE audit_log_y2028m09; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m09 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m09 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m09 TO service_role;


--
-- Name: TABLE audit_log_y2028m10; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m10 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m10 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m10 TO service_role;


--
-- Name: TABLE audit_log_y2028m11; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m11 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m11 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m11 TO service_role;


--
-- Name: TABLE audit_log_y2028m12; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2028m12 TO anon;
GRANT ALL ON TABLE public.audit_log_y2028m12 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2028m12 TO service_role;


--
-- Name: TABLE audit_log_y2029m01; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2029m01 TO anon;
GRANT ALL ON TABLE public.audit_log_y2029m01 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2029m01 TO service_role;


--
-- Name: TABLE audit_log_y2029m02; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2029m02 TO anon;
GRANT ALL ON TABLE public.audit_log_y2029m02 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2029m02 TO service_role;


--
-- Name: TABLE audit_log_y2029m03; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2029m03 TO anon;
GRANT ALL ON TABLE public.audit_log_y2029m03 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2029m03 TO service_role;


--
-- Name: TABLE audit_log_y2029m04; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y2029m04 TO anon;
GRANT ALL ON TABLE public.audit_log_y2029m04 TO authenticated;
GRANT ALL ON TABLE public.audit_log_y2029m04 TO service_role;


--
-- Name: TABLE audit_log_y_default; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.audit_log_y_default TO anon;
GRANT ALL ON TABLE public.audit_log_y_default TO authenticated;
GRANT ALL ON TABLE public.audit_log_y_default TO service_role;


--
-- Name: TABLE billing_adjustments; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE billing_tax_codes; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.billing_tax_codes TO anon;
GRANT ALL ON TABLE public.billing_tax_codes TO authenticated;
GRANT ALL ON TABLE public.billing_tax_codes TO service_role;


--
-- Name: TABLE closed_dates; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.closed_dates TO anon;
GRANT ALL ON TABLE public.closed_dates TO authenticated;
...
--
-- Name: TABLE company_contracts; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.company_contracts TO anon;
GRANT ALL ON TABLE public.company_contracts TO authenticated;
GRANT ALL ON TABLE public.company_contracts TO service_role;


--
-- Name: TABLE company_current_agreement; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.company_current_agreement TO anon;
GRANT SELECT,MAINTAIN ON TABLE public.company_current_agreement TO authenticated;
GRANT ALL ON TABLE public.company_current_agreement TO service_role;


--
-- Name: TABLE company_current_agreement_rules; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.company_current_agreement_rules TO anon;
GRANT ALL ON TABLE public.company_current_agreement_rules TO authenticated;
GRANT ALL ON TABLE public.company_current_agreement_rules TO service_role;


--
-- Name: TABLE company_deletions; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE company_locations; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE company_product_prices; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.company_product_prices TO anon;
GRANT ALL ON TABLE public.company_product_prices TO authenticated;
GRANT ALL ON TABLE public.company_product_prices TO service_role;


--
-- Name: TABLE content_health; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.content_health TO anon;
GRANT MAINTAIN ON TABLE public.content_health TO authenticated;
GRANT ALL ON TABLE public.content_health TO service_role;


--
-- Name: TABLE content_page_variants; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.content_page_variants TO anon;
GRANT ALL ON TABLE public.content_page_variants TO authenticated;
GRANT ALL ON TABLE public.content_page_variants TO service_role;
...
--
-- Name: TABLE day_choices; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.day_choices TO service_role;
GRANT SELECT,INSERT,UPDATE ON TABLE public.day_choices TO authenticated;


--
-- Name: TABLE deliveries; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE delivery_run_items; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.delivery_run_items TO anon;
GRANT ALL ON TABLE public.delivery_run_items TO authenticated;
GRANT ALL ON TABLE public.delivery_run_items TO service_role;


--
-- Name: TABLE delivery_runs; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE dietary_tags; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE driver_runs; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.driver_runs TO anon;
GRANT ALL ON TABLE public.driver_runs TO authenticated;
GRANT ALL ON TABLE public.driver_runs TO service_role;


--
-- Name: TABLE employee_invites; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE employee_order_items; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.employee_order_items TO anon;
GRANT ALL ON TABLE public.employee_order_items TO authenticated;
GRANT ALL ON TABLE public.employee_order_items TO service_role;
...
--
-- Name: TABLE employee_orders; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.employee_orders TO anon;
GRANT ALL ON TABLE public.employee_orders TO authenticated;
GRANT ALL ON TABLE public.employee_orders TO service_role;


--
-- Name: TABLE enterprise_groups; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE idempotency; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE invoice_periods; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.invoice_periods TO anon;
GRANT ALL ON TABLE public.invoice_periods TO authenticated;
GRANT ALL ON TABLE public.invoice_periods TO service_role;


--
-- Name: TABLE invoice_runs; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE kitchen_batches; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.kitchen_batches TO service_role;
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE public.kitchen_batches TO authenticated;


--
-- Name: TABLE kitchen_batch; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.kitchen_batch TO service_role;


--
-- Name: TABLE location_memberships; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.location_memberships TO anon;
GRANT ALL ON TABLE public.location_memberships TO authenticated;
GRANT ALL ON TABLE public.location_memberships TO service_role;
...
--
-- Name: TABLE kitchen_scope_gap_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.kitchen_scope_gap_v TO anon;
GRANT MAINTAIN ON TABLE public.kitchen_scope_gap_v TO authenticated;
GRANT ALL ON TABLE public.kitchen_scope_gap_v TO service_role;


--
-- Name: TABLE lead_pipeline; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE lifecycle_audit_log; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.lifecycle_audit_log TO anon;
GRANT ALL ON TABLE public.lifecycle_audit_log TO authenticated;
GRANT ALL ON TABLE public.lifecycle_audit_log TO service_role;


--
-- Name: TABLE location_closed_dates; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.location_closed_dates TO anon;
GRANT ALL ON TABLE public.location_closed_dates TO authenticated;
GRANT ALL ON TABLE public.location_closed_dates TO service_role;


--
-- Name: TABLE location_policies; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.location_policies TO anon;
GRANT ALL ON TABLE public.location_policies TO authenticated;
GRANT ALL ON TABLE public.location_policies TO service_role;


--
-- Name: TABLE marketing_pages; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.marketing_pages TO anon;
GRANT ALL ON TABLE public.marketing_pages TO authenticated;
GRANT ALL ON TABLE public.marketing_pages TO service_role;


--
-- Name: TABLE media_items; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE membership_backfill_audit_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.membership_backfill_audit_v TO anon;
GRANT MAINTAIN ON TABLE public.membership_backfill_audit_v TO authenticated;
GRANT ALL ON TABLE public.membership_backfill_audit_v TO service_role;


--
-- Name: TABLE membership_scope_drift_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.membership_scope_drift_v TO anon;
GRANT MAINTAIN ON TABLE public.membership_scope_drift_v TO authenticated;
GRANT ALL ON TABLE public.membership_scope_drift_v TO service_role;


--
-- Name: TABLE menu_service_day_items; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.menu_service_day_items TO anon;
GRANT ALL ON TABLE public.menu_service_day_items TO authenticated;
GRANT ALL ON TABLE public.menu_service_day_items TO service_role;


--
-- Name: TABLE menu_service_days; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.menu_service_days TO anon;
GRANT ALL ON TABLE public.menu_service_days TO authenticated;
GRANT ALL ON TABLE public.menu_service_days TO service_role;


--
-- Name: TABLE menu_visibility_days; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.menu_visibility_days TO anon;
GRANT ALL ON TABLE public.menu_visibility_days TO authenticated;
GRANT ALL ON TABLE public.menu_visibility_days TO service_role;


--
-- Name: TABLE ops_events; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: SEQUENCE order_status_history_id_seq; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE orphan_kitchen_test_profiles_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.orphan_kitchen_test_profiles_v TO anon;
GRANT MAINTAIN ON TABLE public.orphan_kitchen_test_profiles_v TO authenticated;
GRANT ALL ON TABLE public.orphan_kitchen_test_profiles_v TO service_role;
...
--
-- Name: TABLE product_allergens; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE product_dietary_tags; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.product_dietary_tags TO anon;
GRANT ALL ON TABLE public.product_dietary_tags TO authenticated;
GRANT ALL ON TABLE public.product_dietary_tags TO service_role;
...
--
-- Name: TABLE production_manifests; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.production_manifests TO anon;
GRANT ALL ON TABLE public.production_manifests TO authenticated;
GRANT ALL ON TABLE public.production_manifests TO service_role;
...
--
-- Name: TABLE profile_cleanup_audit; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.profile_cleanup_audit TO anon;
GRANT ALL ON TABLE public.profile_cleanup_audit TO authenticated;
GRANT ALL ON TABLE public.profile_cleanup_audit TO service_role;


--
-- Name: SEQUENCE profile_cleanup_audit_id_seq; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON SEQUENCE public.profile_cleanup_audit_id_seq TO anon;
GRANT ALL ON SEQUENCE public.profile_cleanup_audit_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.profile_cleanup_audit_id_seq TO service_role;


--
-- Name: TABLE profile_scope_legacy_write_audit; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.profile_scope_legacy_write_audit TO anon;
GRANT ALL ON TABLE public.profile_scope_legacy_write_audit TO authenticated;
GRANT ALL ON TABLE public.profile_scope_legacy_write_audit TO service_role;


--
-- Name: SEQUENCE profile_scope_legacy_write_audit_id_seq; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON SEQUENCE public.profile_scope_legacy_write_audit_id_seq TO anon;
GRANT ALL ON SEQUENCE public.profile_scope_legacy_write_audit_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.profile_scope_legacy_write_audit_id_seq TO service_role;


--
-- Name: TABLE profile_scope_projection_status_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.profile_scope_projection_status_v TO anon;
GRANT MAINTAIN ON TABLE public.profile_scope_projection_status_v TO authenticated;
GRANT ALL ON TABLE public.profile_scope_projection_status_v TO service_role;


--
-- Name: TABLE profile_scope_projection_drift_v; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.profile_scope_projection_drift_v TO anon;
GRANT MAINTAIN ON TABLE public.profile_scope_projection_drift_v TO authenticated;
GRANT ALL ON TABLE public.profile_scope_projection_drift_v TO service_role;


--
-- Name: TABLE provider_invoices; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE provider_memberships; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.provider_memberships TO anon;
GRANT ALL ON TABLE public.provider_memberships TO authenticated;
GRANT ALL ON TABLE public.provider_memberships TO service_role;
...
--
-- Name: TABLE provider_subscriptions; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE provider_tripletex_products; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.provider_tripletex_products TO authenticated;
GRANT ALL ON TABLE public.provider_tripletex_products TO service_role;


--
-- Name: TABLE provider_tripletex_webhook_secrets; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.provider_tripletex_webhook_secrets TO authenticated;
GRANT ALL ON TABLE public.provider_tripletex_webhook_secrets TO service_role;


--
-- Name: TABLE provider_tripletex_webhook_subscriptions; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.provider_tripletex_webhook_subscriptions TO authenticated;
GRANT ALL ON TABLE public.provider_tripletex_webhook_subscriptions TO service_role;
...
--
-- Name: TABLE standing_orders; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.standing_orders TO anon;
GRANT ALL ON TABLE public.standing_orders TO authenticated;
GRANT ALL ON TABLE public.standing_orders TO service_role;


--
-- Name: SEQUENCE system_health_snapshots_id_seq; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE system_health_snapshots; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE system_incidents; Type: ACL; Schema: public; Owner: -
--
...
--
-- Name: TABLE tripletex_customers; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.tripletex_customers TO anon;
GRANT ALL ON TABLE public.tripletex_customers TO authenticated;
GRANT ALL ON TABLE public.tripletex_customers TO service_role;


--
-- Name: TABLE tripletex_exports; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.tripletex_exports TO anon;
GRANT ALL ON TABLE public.tripletex_exports TO authenticated;
GRANT ALL ON TABLE public.tripletex_exports TO service_role;


--
-- Name: TABLE tripletex_invoices; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.tripletex_invoices TO anon;
GRANT ALL ON TABLE public.tripletex_invoices TO authenticated;
GRANT ALL ON TABLE public.tripletex_invoices TO service_role;


--
-- Name: TABLE tripletex_webhook_events; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.tripletex_webhook_events TO service_role;


--
-- Name: TABLE v_kitchen_day_report; Type: ACL; Schema: public; Owner: -
--

GRANT MAINTAIN ON TABLE public.v_kitchen_day_report TO anon;
GRANT MAINTAIN ON TABLE public.v_kitchen_day_report TO authenticated;
GRANT ALL ON TABLE public.v_kitchen_day_report TO service_role;
...
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON SEQUENCES TO postgres;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON SEQUENCES TO anon;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON SEQUENCES TO authenticated;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON SEQUENCES TO service_role;
...
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON FUNCTIONS TO postgres;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON FUNCTIONS TO anon;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON FUNCTIONS TO authenticated;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON FUNCTIONS TO service_role;
...
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON TABLES TO postgres;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON TABLES TO anon;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON TABLES TO authenticated;
-- REPLAY_SKIP (pooler cannot set supabase_admin default ACLs): ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin IN SCHEMA public GRANT ALL ON TABLES TO service_role;
...
DECLARE jid bigint;
BEGIN
  FOR jid IN SELECT jobid FROM cron.job WHERE command LIKE '%ensure_audit_log_partitions%' LOOP
    PERFORM cron.unschedule(jid);
  END LOOP;
END $$;
SELECT cron.schedule('ensure_audit_log_partitions_monthly', '0 0 1 * *', $$SELECT private.ensure_audit_log_partitions(12)$$);
