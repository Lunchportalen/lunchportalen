-- lp_order_set: lifecycle robustness under set/cancel churn (read + write).
-- Base: 20260611120000_lp_order_set_variant_itemkey.sql (varmmat alias + variant MSDI).
-- Delta:
--   CANCEL idempotent: when no ACTIVE order, still delete orphan day_choices for scope+date.
--   ORDER unchanged: INSERT new ACTIVE after cancel (CANCELLED rows are immutable per assert_order_mutable).
