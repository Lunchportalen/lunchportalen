-- Phase 25 Foundation: AI activity log reconcile safety net.
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'ai_activity_log'
      and indexname = 'ai_activity_log_created_at_idx'
  ) then
    execute 'create index ai_activity_log_created_at_idx on public.ai_activity_log (created_at desc)';
  end if;
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'ai_activity_log'
      and indexname = 'ai_activity_log_page_variant_idx'
  ) then
    execute 'create index ai_activity_log_page_variant_idx on public.ai_activity_log (page_id, variant_id)';
  end if;
end
$$;
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'ai_activity_log'
      and policyname = 'ai_activity_log_all_superadmin'
  ) then
    execute $p$
