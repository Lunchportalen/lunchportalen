-- FASE 13-IMPL-3H — Firma-admin ordre-aggregering (SECURITY DEFINER RPC)
-- STOPP: ikke apply til prod uten eksplisitt review.
--
-- Leser ACTIVE-ordrer i periode, summert per selskap med per-ansatt aggregat.
-- Ingen produktvalg / varmrett-detaljer eksponeres.

-- Index for company + service_date filtrering (partial kun ACTIVE)
create index if not exists idx_orders_company_service_date_active
  on public.orders (company_id, service_date)
  where status = 'ACTIVE'::public.order_status;

-- FK order_id er svært brukt i join fra order_items → orders
...
begin
  ...
  -- Firmaadmin / økonomi (membership) ELLER plattform-admin
  select exists (
    select 1
  ...
  begin
    v_can_platform := coalesce(
      (
        select private.has_platform_role(
          array['platform_admin'::public.platform_role]
        )
      ),
      false
    );
...
comment on function public.lp_company_order_summary(uuid, date, date) is
  'Firma-scope ordre-aggregering for company_admin / company_finance (membership) eller platform_admin. Ingen produkt- eller valgdetaljer.';

revoke all on function public.lp_company_order_summary(uuid, date, date) from public;
revoke all on function public.lp_company_order_summary(uuid, date, date) from anon;
grant execute on function public.lp_company_order_summary(uuid, date, date) to authenticated;
