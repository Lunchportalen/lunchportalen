...
create table public.content_releases ( id uuid primary key default gen_random_uuid(), name text not null, environment text not null check (environment in ('prod','staging')), status text not null check (status in ('draft','scheduled','executed','cancelled')), publish_at timestamptz null, created_by text null, created_at timestamptz not null default now(), updated_at timestamptz not null default now() );
create index idx_releases_env_status_publish_at on public.content_releases (environment, status, publish_at);
create index idx_releases_created_at on public.content_releases (created_at desc);
...
create table public.content_release_items ( id uuid primary key default gen_random_uuid(), release_id uuid not null references public.content_releases(id) on delete cascade, variant_id uuid not null references public.content_page_variants(id) on delete cascade, page_id uuid not null references public.content_pages(id) on delete cascade, locale text not null, environment text not null, created_at timestamptz not null default now(), unique(release_id, variant_id) );
create index idx_release_items_release on public.content_release_items (release_id);
create index idx_release_items_variant on public.content_release_items (variant_id);
