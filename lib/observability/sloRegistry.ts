...
export const SLO_REGISTRY: Record<SloId, SloDefinition> = {
  ...
  order_write: {
    ...
    description: "Order placement (POST /api/orders) succeeds or fails deterministically without 5xx.",
    sliKey: "order_write",
    targetPercent: 99.9,
    windowMinutes: 60,
    criticalThresholdPercent: 99,
    warnThresholdPercent: 99.5,
    measurementNote: "Inferred from absence of open ORDER system_incidents (no persistent order-write failures). Future: ops_events order.write.success/failure.",
    operatorHint: "Undersøk åpne ORDER system_incidents, ordre-responser (POST /api/orders) og ev. DB-logg for ordre som feiler eller avvises feilaktig.",
  },
