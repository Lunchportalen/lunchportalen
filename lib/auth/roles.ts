...
/**
 * Role resolution: email allowlist → profiles.role → employee.
 * SECURITY (D4): `user_metadata` is NEVER an authorization source — a missing
 * profile role degrades to `employee` (fail-closed), it never upgrades.
 */
