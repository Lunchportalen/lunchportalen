...
import { addDaysISO, nowISO } from "@/lib/date/oslo";
...
export async function runMenuWeekRollout(opts: {
  /** Wall-clock instant used for Oslo “today” and Sanity timestamps. */
  instant?: Date;
  ...
  const osloToday = utcInstantToOsloDateISO(instant);
  const targetWeekMonday = startOfWeekMondayNPlus3(osloToday);
