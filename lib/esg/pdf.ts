...
import { formatDateTimeNO, formatMonthYearLongNO } from "@/lib/date/format";
...
function fmtMonthLabel(isoMonth01: string) {
  return formatMonthYearLongNO(isoMonth01);
}
...
export async function buildEsgPdf(input: BuildEsgPdfInput): Promise<Uint8Array> {
  ...
  const metaLeft = [
    ...
    `Generert: ${formatDateTimeNO(input.generatedAtISO)}`,
