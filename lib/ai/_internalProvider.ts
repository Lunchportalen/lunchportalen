...
/**
 * Internal OpenAI / capability implementation. Default export only — no named exports.
 * Allowed importer: lib/ai/runner.ts only (enforced by ESLint + scripts/check-ai-internal-provider.mjs).
 */
...
type AISuggestInput = {
  ...
  /** When set, overrides env/config resolved model (runner profitability / fallback). */
  model?: string;
};
 ...
 */
function getAiProviderConfig(): AiProviderConfigView {
 ...
 */
function normalizeProviderResult(raw: unknown): AISuggestOutput {
...
async function callOpenAIChat(
  ...
  const model =
    typeof req.model === "string" && req.model.trim() ? req.model.trim() : resolveOpenAIModel(config.model);
...
async function callOpenAIWithMessages(
  ...
  const model =
    typeof modelOverride === "string" && modelOverride.trim()
      ? modelOverride.trim()
      : resolveOpenAIModel(config.model);
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), PROVIDER_REQUEST_TIMEOUT_MS);
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
...
type SuggestEditorTextInput = { text: string; action: "improve" | "shorten"; locale: "nb" | "en"; model?: string };
type SuggestEditorTextOutput =
  | { ok: true; suggestion: string }
  | { ok: false; error: string };

async function suggestEditorText(req: SuggestEditorTextInput): Promise<SuggestEditorTextOutput> {
  const config = getAiProviderConfig();
...
type CtaSuggestionShape = { title: string; body?: string; buttonLabel?: string; buttonHref?: string };
type SuggestCtaImproveOutput =
  | { ok: true; suggestion: CtaSuggestionShape }
  | { ok: false; error: string };

async function suggestCtaImprove(req: SuggestCtaImproveInput): Promise<SuggestCtaImproveOutput> {
  const config = getAiProviderConfig();
  if (!config.enabled) return { ok: false, error: AI_PROVIDER_ERROR.AI_DISABLED };
  const provider = (config.provider || "").toLowerCase();
  if (provider !== "openai") return { ok: false, error: AI_PROVIDER_ERROR.PROVIDER_ERROR };
 ...
 */
async function suggestJSON(req: AISuggestInput): Promise<AISuggestOutput> {
  const config = getAiProviderConfig();
  if (!config.enabled) {
...
/** Low-level chat completion for the unified AI runner only. */
type ProviderChatMessage = { role: "system" | "user"; content: string };

type ProviderOpenAiChatRawResult = {
  text: string;
  model: string;
  usage: { promptTokens: number; completionTokens: number };
};
...
/**
 * OpenAI chat completions with optional JSON response format. Used only from lib/ai/runner.ts.
 * Throws on disabled AI, network error, non-OK response, or empty content.
 */
async function providerOpenAiChatRaw(options: {
  messages: ProviderChatMessage[];
  responseFormat?: "json_object" | "text";
  temperature?: number;
  max_tokens?: number;
  model?: string;
}): Promise<ProviderOpenAiChatRawResult> {
  const config = getAiProviderConfig();
  if (!config.enabled) throw new Error("AI_DISABLED");
  if ((config.provider || "").toLowerCase() !== "openai") throw new Error("PROVIDER_ERROR");
  const apiKey = getApiKey();
...
/**
 * OpenAI images generations. Used only from lib/ai/runner.ts. Throws on failure.
 */
async function providerOpenAiImagesGenerations(body: Record<string, unknown>): Promise<unknown> {
  const config = getAiProviderConfig();
  if (!config.enabled) throw new Error("AI_DISABLED");
  if ((config.provider || "").toLowerCase() !== "openai") throw new Error("PROVIDER_ERROR");
  const apiKey = getApiKey();
  ...
  try {
    ...
    if (!res.ok) throw new Error(`OpenAI images ${res.status}: ${rawText.slice(0, 200)}`);
...
const _internalAiProvider = {
  ...
  suggestCtaImprove,
  suggestJSON,
  providerOpenAiChatRaw,
  providerOpenAiImagesGenerations,
} as const;

export default _internalAiProvider;
