...
export async function POST(req: Request, ctx: RouteCtx) {
  const rid = `sa_close_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
  ...
  try {
    ...
    if (st === "closed") {
      ...
      return ok({ rid, company, meta: { alreadyClosed: true } });
    }
    ...
    if (up.error && isMissingColumn(up.error)) {
      ...
      if (up2.error) return fail(500, "db_error", "Kunne ikke close firma", { rid, upErr: up2.error });
      ...
      await tryAudit(sb, {
        ...
        entity_type: "company",
        entity_id: companyId,
        summary: `${company.status} -> closed`,
        detail: { rid, from: company.status, to: "closed", note },
        created_at: now,
      });
    ...
    if (up.error) return fail(500, "db_error", "Kunne ikke close firma", { rid, upErr: up.error });
    ...
    return fail(500, "server_error", "Uventet feil i close.", { rid, error: String(e?.message ?? e) });
  }
}
