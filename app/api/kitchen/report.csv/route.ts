...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const fileAnchor = mode === "week" ? report.period.weekStart : report.date;

    return new Response(csv, {
      status: 200,
      headers: {
        "content-type": "text/csv; charset=utf-8",
        "content-disposition": `attachment; filename="kjøkkenrapport_${mode}_${fileAnchor}.csv"`,
    ...
    return jsonErr(auth.ctx.rid, "Kunne ikke lage CSV for kjøkkenrapport.", 500, "KITCHEN_REPORT_CSV_FAILED");
  }
}
