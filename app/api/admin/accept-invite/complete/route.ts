...
async function findAuthUserByEmail(admin: ReturnType<typeof supabaseAdmin>, email: string) {
  ...
  const perPage = 200;
  const maxPages = 10; // 2000 users cap (kan økes ved behov)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const nameInput = safeText((body as any)?.name ?? (body as any)?.full_name ?? (body as any)?.fullName, 120);
    ...
    // 2) Opprett/oppdater auth-user (idempotent) med metadata som trigger bruker
    ...
    // 4) Sikkerhet: company må stemme (bundet av trigger)
    ...
    if (String(profile.company_id) !== String(company_id)) {
      return jsonErr(
        ...
        { existingCompany: profile.company_id, inviteCompany: company_id }
      );
    }
    ...
    return jsonErr(ctx, "server_error", "Uventet feil.", String(e?.message ?? e));
  }
}
