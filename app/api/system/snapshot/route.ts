...
export async function GET(req: NextRequest): Promise<Response> {
  return withApiAiEntrypoint(req, "GET", async () => {
    ...
    try {
      const body = await getSystemSnapshotPayload();
      opsLog("system_snapshot_served", { rid, healthStatus: body.health.status, healthScore: body.health.score });
      return jsonOk(rid, body, 200);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e);
      opsLog("system_snapshot_failed", { rid, message });
