...
const ROUTE = "/api/social/recommendations";

/** GET: anbefalinger fra samme aggregat som analytics (ingen intern HTTP — superadmin). */
export async function GET(req: NextRequest): Promise<Response> {
  const rid = makeRid("social_recommendations");
  try {
    ...
    if (bundle.ok === false) {
      return jsonErr(rid, bundle.error, 500, bundle.code, { detail: bundle.error });
    }

    const analytics = aggregateSocialAnalytics(
      mapSocialPostsFromDb(bundle.posts),
      mapSocialEventsFromDb(bundle.events),
    );
    const recommendations = getRecommendations(analytics);
    return jsonOk(rid, { recommendations }, 200);
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    return jsonErr(rid, message, 500, "SOCIAL_RECOMMENDATIONS_UNHANDLED");
  }
}
