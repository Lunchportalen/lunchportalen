...
const ROUTE = "/api/social/analytics";

/** GET: aggreger klikk/leds per post fra DB (superadmin). */
export async function GET(req: NextRequest): Promise<Response> {
  const rid = makeRid("social_analytics");
  try {
    ...
    const result = aggregateSocialAnalytics(
    ...
    return jsonErr(rid, message, 500, "SOCIAL_ANALYTICS_UNHANDLED");
  }
}
