...
import {
  parseVarmrettSharedBody,
  writeSharedVarmrettForProvider,
...
export async function POST(req: NextRequest) {
  ...
  const parsed = parseVarmrettSharedBody(body);
  if (!parsed) {
  ...
  const result = await writeSharedVarmrettForProvider(client, provider.id, parsed, {
    providerSlug: provider.slug,
    reconcileVisibleWindow: true,
  });
  ...
  if (result.ok === false) {
    ...
    return jsonErr(
      ...
      isLock ? { lockedDates: [parsed.date] } : undefined,
    );
  }
