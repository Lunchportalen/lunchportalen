...
export default async function SuperadminOverviewPage() {
  ...
  if (!auth.ok) {
    if (auth.reason === "UNAUTHENTICATED") {
      redirect("/login?next=/superadmin/overview&code=NO_SESSION");
    }
    return <BlockedAccess reason={auth.reason} />;
  }

  if (auth.role !== "superadmin") {
    redirect("/login?next=/superadmin/overview&code=ROLE_FORBIDDEN");
  }
  ...
  const [overlay, designSettings] = await Promise.all([
    getOverlayBySlug(APP_OVERLAYS.superadmin.slug, { locale: "nb", environment: "prod" }),
    getDesignSettings(),
  ]);
