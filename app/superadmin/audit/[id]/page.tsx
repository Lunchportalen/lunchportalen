...
async function getAudit(id: string): Promise<ApiOk | ApiErr> {
  // ✅ Server-side: intern relative fetch (bevarer cookies for auth)
  const r = await fetch(`/api/superadmin/audit/${encodeURIComponent(id)}`, { cache: "no-store" });
  return r.json();
}
      ...
      <header style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0 0", opacity: 0.75 }}>
            Revisjonslogg for Superadmin. Felt under er “read-only”.
          </p>
        </div>
