 ...
 * Hard system-e-poster er fasit og skal alltid vinne over profiles.role.
 */
...
function normalizeAllowed(allowed: ReadonlyArray<string>) {
  return allowed.map((x) => safeStr(x).toLowerCase()).filter(Boolean);
}
...
function ridFromReq(req: NextRequest) {
  ...
  // support both rid() and rid(req) in respond.ts
  try {
    const r = safeStr((makeRid as any)(req));
    if (r) return r;
  } catch {
    // ignore
  }
  try {
    const r = safeStr((makeRid as any)());
    if (r) return r;
  } catch {
    // ignore
  }
 ...
 * Dette er avgjørende for driver@lunchportalen.no dersom getScope() ikke leverer email/role.
 */
...
export async function scopeOr401(req: NextRequest): Promise<ScopeOr401Result> {
  ...
  try {
    ...
    const ctx: AuthedCtx = { rid, scope };

    // Minimum: userId må finnes for å regne som autentisert
    if (!scope.userId) {
      const r = jsonErr(401, ctx, "UNAUTHORIZED", "Du må være innlogget for å bruke dette endepunktet.");
      return { ok: false as const, res: r, response: r, ctx };
    }

    return { ok: true as const, ctx };
  } catch (e: any) {
    const r = jsonErr(401, emptyCtx, "UNAUTHORIZED", "Kunne ikke verifisere innlogging.", {
...
export function requireRoleOr403(...args: any[]): Response | null {
  try {
    ...
    if (typeof args[0] === "string") {
      ...
      if (!allowed.length)
        return jsonErr(500, r, "MISCONFIGURED_ROUTE", "Ingen tillatte roller er konfigurert for denne ruten.");
      if (!role) return jsonErr(403, r, "FORBIDDEN", "Mangler rolle (ikke autorisert).");
      if (!allowed.includes(role)) {
        return jsonErr(403, r, "FORBIDDEN", "Du har ikke tilgang til denne funksjonen.", { role, allowed });
      }
      return null;
    }
    ...
    if (args.length === 2 && Array.isArray(args[1])) {
      ...
      if (!allowed.length)
        return jsonErr(500, ctx, "MISCONFIGURED_ROUTE", "Ingen tillatte roller er konfigurert for denne ruten.");
      if (!ctxRole) return jsonErr(403, ctx, "FORBIDDEN", "Mangler rolle (ikke autorisert).");
      if (!allowed.includes(ctxRole)) {
        return jsonErr(403, ctx, "FORBIDDEN", "Du har ikke tilgang til denne funksjonen.", { role: ctxRole, allowed });
      }
      return null;
    }
    ...
    if (args.length === 3 && typeof args[1] === "string" && Array.isArray(args[2])) {
      ...
      if (!ctxRole) return jsonErr(403, ctx, "FORBIDDEN", "Mangler rolle (ikke autorisert).", { action });
      if (!allowed.includes(ctxRole)) {
        return jsonErr(403, ctx, "FORBIDDEN", "Du har ikke tilgang til denne funksjonen.", {
          action,
          role: ctxRole,
          allowed,
        });
    ...
    if (args.length === 3 && Array.isArray(args[2])) {
      ...
      if (!allowed.length)
        return jsonErr(500, ctx, "MISCONFIGURED_ROUTE", "Ingen tillatte roller er konfigurert for denne ruten.");
      if (!role) return jsonErr(403, ctx, "FORBIDDEN", "Mangler rolle (ikke autorisert).");
      if (!allowed.includes(role)) {
        return jsonErr(403, ctx, "FORBIDDEN", "Du har ikke tilgang til denne funksjonen.", { role, allowed });
      }
      return null;
    }

    // Feil bruk / ukjent signatur
    return jsonErr(500, ctxRidVal, "MISCONFIGURED_ROUTE", "Ugyldig bruk av requireRoleOr403().");
  } catch {
    const r = ridFallback();
    return jsonErr(500, r, "MISCONFIGURED_ROUTE", "requireRoleOr403 feilet uventet.");
  }
}
...
export function requireCompanyScopeOr403(...args: any[]): Response | null {
  try {
    ...
    if (typeof args[0] === "string") {
      ...
      if (!cid) return jsonErr(403, r, "FORBIDDEN", "Mangler company scope.");
      return null;
    }
    ...
    if (args.length === 1) {
      ...
      if (!cid) {
        return jsonErr(403, ctx, "FORBIDDEN", "Mangler company scope.", { companyId: ctx?.scope?.companyId ?? null });
      }
      return null;
    }
    ...
    // B) (ctx, companyId)
    const cid = safeStr(args[1]);
    if (!cid) return jsonErr(403, ctx, "FORBIDDEN", "Mangler company scope.");
    return null;
  } catch {
    const r = ridFallback();
    return jsonErr(500, r, "MISCONFIGURED_ROUTE", "requireCompanyScopeOr403 feilet uventet.");
  }
}
