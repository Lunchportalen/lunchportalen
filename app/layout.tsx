...
export const metadata: Metadata = {
  ...
  icons: {
    // ✅ Use existing file: /public/apple-touch-icon.png (180x180)
    // This is stable in Chrome/Edge and stops the tab “spinner” when /favicon.ico is missing.
...
async function resolvePathname(): Promise<string> {
  ...
  try {
    const raw =
      ...
      // referer is last resort (may point to previous page)
      h.get("referer") ||
      "";
...
/**
 * Admin areas have their own layout shells and must NOT be wrapped
 * by the global lp-page/lp-container wrapper (prevents double container/spacing).
 */
...
function navFor(pathname: string) {
  if (!pathname) return [];
  if (pathname.startsWith("/kitchen") || pathname.startsWith("/kjokken")) return [{ label: "Rapport", href: "/kitchen" }];
  if (pathname.startsWith("/driver")) return [{ label: "Ruter", href: "/driver" }];
  if (pathname.startsWith("/system")) return [{ label: "Status", href: "/system" }];
  ...
  return (
    <html lang="no" className={`${fontBody.variable} ${fontDisplay.variable} ${fontHeading.variable} h-full`}>
      <body className="min-h-full">
        ...
        {/* ✅ Auth + Admin/Superadmin have their own shells — no global wrapper */}
