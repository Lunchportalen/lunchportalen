...
vi.mock("@/lib/http/respond", async () => {
  // keep minimal semantics: jsonErr returns Response with ok:false
  return {
    rid: () => "rid_test",
    jsonErr: (_status: number, _ctx: any, error: string, message: string, detail?: any) =>
      new Response(JSON.stringify({ ok: false, rid: "rid_test", error, message, detail }), { status: _status }),
    jsonOk: (body: any, status = 200) => new Response(JSON.stringify(body), { status }),
  };
});
...
/* =========================================================
   Route under test
   (KORREKT STI FRA app/tests -> app/api)
========================================================= */
...
describe("tenant isolation – admin/orders", () => {
  test("company_admin: company_id i respons låses til scope.companyId (selv om query prøver annet)", async () => {
    ...
    expect(json.company_id).toBe("cA");
    expect(String(json.status)).toBe("ACTIVE");
    expect(String(json.dateISO)).toBe("2026-01-29");
  });
  ...
  test("superadmin: company_id kan filtreres fritt via query", async () => {
    ...
    const json = await readJson(res);
    expect(json.ok).toBe(true);
    expect(json.company_id).toBe("cB"); // ✅ superadmin får velge
  });
});
