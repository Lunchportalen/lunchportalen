...
type KitchenResp = {
  ...
  cutoff?: { isAfterCutoff: boolean; cutoffTime: string }; // valgfritt, hvis API sender
...
/** Oslo ISO-date (YYYY-MM-DD) fra en Date */
...
async function fetchKitchen(dateISO: string): Promise<KitchenResp> {
  // Bytt endpoint hvis ditt heter noe annet:
  // f.eks. /api/kitchen/day?date=...
  ...
  const data = (await r.json()) as KitchenResp;
  return data;
}
...
async function findNextActiveDate(startISO: string) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (res.ok && res.summary.orders > 0) return { date: cur, res };
    // Hvis API sier "NOT_DELIVERY_DAY", hopp direkte videre
    cur = nextBusinessDayISO(cur);
  }

  // Fant ingen aktive – returner start for å vise tom state
  ...
  return (
    <div className="rounded-2xl border bg-white p-5">
      ...
      <div className="mt-4 text-xs text-slate-500">
        Tips: Kjøkkenvisning er laget for drift — du skal alltid ha et tydelig
        neste steg, selv når det er tomt.
      </div>
    </div>
  );
}
...
export default function KitchenClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Hvis det er tomt (og ikke auth/error), autogå til neste aktive dag (1 gang)
        const isHardError =
          !first.ok &&
          (first.reason === "AUTH_REQUIRED" || first.reason === "ERROR");

        if (!isHardError && first.summary.orders === 0 && !autoPicked) {
  ...
  // Når dateISO endres via UI: last data
  useEffect(() => {
    // Unngå dobbel-load på init (når vi allerede har data)
    if (!data) return;
  ...
  const status = (() => {
    ...
    if (!data.ok) {
      return { kind: "crit" as const, label: "Problem" };
    }
    if (summary.orders === 0) {
      return { kind: "warn" as const, label: "Tom dag" };
    }
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium ${chipClass(
                status.kind
              )}`}
            >
              {status.label}
            ...
            {loading ? (
              <span className="text-xs text-slate-500">Henter…</span>
            ) : null}
          </div>
        </div>
      </div>
      ...
      <div className="mb-5 rounded-2xl border bg-white p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={setNextDelivery}
              className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium"
            >
              Neste leveringsdag
            </button>
            <button
              onClick={goNextActive}
              className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white"
            >
