...
import { useSectionSidebarContent } from "../_workspace/ContentWorkspaceLayout";
import {
  isForside, tryParseBlockListFromBody, documentTypes, type BlockList, type BlockType as AddModalBlockType,
  deriveBodyFromParse, looksJsonLike, normalizeBlock, parseBodyToBlocks, serializeBlocksToBody, toRawBodyString, type BodyMode,
...
import { buildContentWorkspaceModalShellPropsFromWorkspaceFlatFields } from "./contentWorkspaceTriPaneShellBundle";
...
export function ContentWorkspace({
  ...
  const onAfterAddBlock = useCallback(
    (block: Block) => {
  ...
  const flushVisualCanvasPatches = useCallback(() => {
    ...
    for (const [blockId, patch] of Object.entries(pending)) {
      ...
      setBlockById(blockId, (c) => ({ ...c, ...patch } as Block));
    }
  }, [setBlockById]);
  useEffect(() => {
    return () => {
  ...
  const applyImage = useCallback(
    (picked: { url: string; assetId?: string }) => {
      ...
      setExpandedBlockId(selectedBlock.id);
      setAiImages(null);
    },
  ...
  const onDuplicateBlock = useCallback(
    ...
    [queueBlockEnterAnimation, setBlocks, setSelectedBlockId, setExpandedBlockId, setBlockPulseId]
  );
  ...
  }, [closeInvalidBodyResetConfirm, setBodyMode, setBodyParseError, setLegacyBodyText, setInvalidBodyRaw, setBlocks, setExpandedBlockId]);
  ...
  return (
    <>
      <ContentWorkspaceDevDebugOverlays
        isDev={isDev}
        metrics={metrics}
      ...
      <ContentWorkspaceWorkspaceFrame
        hideLegacySidebar={hideLegacySidebar}
        legacySidebar={
          <ContentWorkspaceLegacySidebar
            hjemSingleClickTimerRef={hjemSingleClickTimerRef}
            hjemExpanded={hjemExpanded}
            setHjemExpanded={setHjemExpanded}
            items={items}
            ...
            mainView={mainView}
            goToGlobalWorkspace={goToGlobalWorkspace}
            goToDesignWorkspace={goToDesignWorkspace}
            embedded={embedded}
            ...
            createParentLoading={createParentLoading}
            allowedChildTypes={allowedChildTypes}
            onCreate={onCreate}
            createTitle={createTitle}
            setCreateTitle={setCreateTitle}
            createSlug={createSlug}
            setCreateSlug={setCreateSlug}
            setCreateSlugTouched={setCreateSlugTouched}
            createError={createError}
            creating={creating}
            createDocumentTypeAlias={createDocumentTypeAlias}
            setCreateDocumentTypeAlias={setCreateDocumentTypeAlias}
            setAllowedChildTypes={setAllowedChildTypes}
            setCreateParentLoading={setCreateParentLoading}
          />
        }
      >
        <ContentWorkspacePageEditorShell
          pendingNavigationHref={pendingNavigationHref}
          ...
          guardPush={guardPush}
          useEditor2={useEditor2}
          {...buildContentWorkspacePageEditorShellBundle({
            designTab,
            setDesignTab,
            colorsContentBg,
            colorsButtonBg,
            ...
            rightRailSlots,
            chrome: {
              shared: [canvasMode, previewDevice, title, slug, setTitle, setCanvasMode, setPreviewDevice, isOffline],
      ...
      <ContentWorkspaceModalShell
        {...buildContentWorkspaceModalShellPropsFromWorkspaceFlatFields({
          aiFullPageModalOpen, closeAiFullPageModal, aiFullPageModalPrompt, setAiFullPageModalPrompt, aiFullPageBusy, aiFullPageError, aiFullPagePreview, aiFullPagePreviewBlocks, aiFullPageReplaceOk, setAiFullPageReplaceOk, aiFullPageAlsoTitle, setAiFullPageAlsoTitle, onAiFullPageModalGenerate, onAiFullPageModalApply,
          addBlockModalOpen, setAddBlockModalOpen, onAddBlock, blockPickerOpen, page, slug, title, isForside, setBlockPickerOpen, addInsertIndexRef, setBodyMode, setBodyParseError, setLegacyBodyText, setInvalidBodyRaw, setBlocks, setExpandedBlockId, editOpen, blocks, editIndex, setEditModalLiveBlock, setEditOpen, setEditIndex, onDeleteBlock, mediaPickerOpen, mediaPickerTarget, setMediaPickerOpen, setMediaPickerTarget, setBlockById,
          onboardingStep, setOnboardingStep, onboardingDoneKey: EditorK.ONBOARDING_DONE_KEY, isPitch, pitchStep, runAiAction, runAiAudit, runAiImage, setShowAfter, setPitchStep,
        })}
      />
