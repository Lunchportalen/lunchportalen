...
import { OSLO_TZ } from "@/lib/kitchen/grouping";
...
export default function KitchenClient(props: {
  ...
  function go(dateISO: string) {
    router.push(`/admin/kjokken?date=${dateISO}`);
    router.refresh();
  }
      ...
      <div className="flex items-start justify-between gap-6 flex-wrap">
        <div>
          ...
          <p className="mt-2 text-sm opacity-80">
            Gruppert per leveringsvindu → firma → lokasjon. (Admin)
          </p>
        ...
        <div className="no-print flex items-end gap-3 flex-wrap">
          ...
          <a
            href={`/api/admin/orders?date=${props.dateISO}`}
            className="rounded-lg border border-white/20 px-4 py-2 text-sm hover:bg-white/5"
          >
            JSON
          </a>
          ...
          <section key={g.key} className="rounded-xl border border-white/15 p-4 card">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <div className="no-print flex items-center gap-3">
                ...
                <button
                  onClick={() => download(csvForGroup(props.dateISO, g), `kjokken-${props.dateISO}-${g.company.name}-${g.location.label}.csv`)}
                  className="rounded-lg border border-white/20 px-3 py-2 text-sm hover:bg-white/5"
                  disabled={g.count === 0}
                >
                  CSV (gruppe)
