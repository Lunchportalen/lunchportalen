...
async function main() {
  ...
  if (apiTarget.mode === "cloud") {
    if (dbTarget.classification !== "isolated_cloud") {
      throw new Error(`CLOUD_RECONCILIATION_DB_CLASS_FORBIDDEN:${dbTarget.classification}`);
    }
    if (!String(dbTarget.host || "").includes("pooler.supabase.com")) {
      throw new Error("CLOUD_RECONCILIATION_DB_POOLER_REQUIRED");
    }
  }
  ...
  const report = {
    ...
    CLOUD_RECONCILIATION_TARGET: apiTarget.mode === "cloud" ? "PASS" : "LOCAL",
