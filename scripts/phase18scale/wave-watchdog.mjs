 ...
 * after 5 minutes without progress. Does not depend on a Cursor terminal.
 */
  ...
  try {
    const out = execFileSync(
      ...
      [
        "-NoProfile",
        "-Command",
        "(Get-NetTCPConnection -LocalPort 3000 -State Listen -EA SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess) | ForEach-Object { $p=Get-Process -Id $_ -EA SilentlyContinue; if($p){ [math]::Round($p.WorkingSet64/1MB,1) } }",
      ],
...
function writeHeartbeat(extra) {
  ...
  const beat = {
    ...
    next_health: nextHealth(),
    next_rss_mb: nextRssMb(),
    postgres: pgStats(),
    progress,
    output_file: {
      path: outTxt,
...
function dumpDiagnostics(reason) {
  ...
  const dump = writeHeartbeat({ reason, stall_dump: true });
...
fs.writeSync(logFd, `# watchdog spawn ${startedAt}\n`);
...
child.on("exit", (code, signal) => {
  exitCode = code;
  writeHeartbeat({ exited: true, exit_code: code, signal });
  console.log(JSON.stringify({ watchdog: "WAVE_EXIT", exit_code: code, signal, stamped_at: new Date().toISOString() }));
  clearInterval(timer);
  process.exit(code === 0 ? 0 : 2);
});

writeHeartbeat({ phase: "spawned" });
console.log(JSON.stringify({ watchdog: "SPAWNED", wave_pid: child.pid, heartbeatPath, progressPath, outTxt }));
