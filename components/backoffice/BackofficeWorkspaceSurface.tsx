...
export function BackofficeWorkspaceSurface({
  ...
  const headerInner = (
    <>
      <h1 className="lp-h1 text-[rgb(var(--lp-text))]">{title}</h1>
      {lead ? <div className="lp-lead mt-2 max-w-3xl">{lead}</div> : null}
      ...
      {publishHistoryNote ? (
        <aside className="lp-empty mt-4 max-w-3xl px-3 py-2 text-xs leading-relaxed text-[rgb(var(--lp-text))]">
          {publishHistoryNote}
        </aside>
      ) : null}
    </>
  ...
  const footerBlock =
    footerApps != null ? (
      <footer
        className="border-t border-[rgb(var(--lp-border))]/75 bg-[rgba(var(--lp-surface-rgb),0.78)] px-4 py-3 text-[11px] leading-relaxed text-[rgb(var(--lp-muted))] sm:px-6"
        role="region"
  ...
  if (layout === "fullBleed") {
    return (
      ...
      >
        <header className="shrink-0 border-b border-[rgb(var(--lp-border))]/75 bg-[rgba(var(--lp-surface-rgb),0.84)] px-4 py-5 shadow-[var(--lp-shadow-sm)] backdrop-blur-sm sm:px-6">
  ...
  return (
    <div data-workspace={workspaceId}>
      <PageContainer className="max-w-[1440px] py-8">
        <header className="border-b border-[rgb(var(--lp-border))]/70 pb-6">{headerInner}</header>
        <div className="mt-8">{children}</div>
        {footerApps != null ? (
          <footer
            className="mt-10 border-t border-[rgb(var(--lp-border))]/75 pt-4 text-[11px] leading-relaxed text-[rgb(var(--lp-muted))]"
            role="region"
            aria-label="Workspace status og hurtighandlinger"
          >
            {footerApps}
...
>) {
  return (
    <header
      data-workspace={workspaceId}
      className="flex flex-col gap-2 border-b border-[rgb(var(--lp-border))]/70 pb-4"
    >
