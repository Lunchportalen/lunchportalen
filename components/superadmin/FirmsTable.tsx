...
function StatusPill({ status }: { status: CompanyStatus }) {
  if (status === "ACTIVE") return <span className="lp-chip lp-chip-ok">Active</span>;
  if (status === "PAUSED") return <span className="lp-chip lp-chip-warn">Paused</span>;
  return <span className="lp-chip lp-chip-crit">Closed</span>;
}
        ...
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((f: any) => {
                const created = f.created_at ? formatDateTimeNO(f.created_at) : "—";
